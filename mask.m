...
% HUMOR_MASK_MOTOR_SAFE_SAVE_PATCH_V1
% Motor / step-motor underlays can be large. Save underlay images as single.
anatomical_reference_raw = single(Ubase);
anatomical_reference = single(buildProcessedUnderlayForSave_native());
...
maskBundle.sliceUnderlayProcessed = sliceUnderlayProcessed;
        ...
        outFile = maskEditorShortOutputFile(visDir, filePrefix, datasetLabel, ts);

        try
            % HUMOR_MASK_MOTOR_SAFE_SAVE_PATCH_V1
            tmpOutFile = maskEditorTempSaveFile(outFile);
            if exist(tmpOutFile,'file') == 2
                delete(tmpOutFile);
            end
            ...
            try
                ...
                % Fallback: keep top-level underlays, compact duplicate underlays inside maskBundle.
                warning('HUMoR:MaskEditorSafeSave', ...
                    'Full maskBundle save failed. Retrying with compact maskBundle. Original error: %s', ...
                    ME_first_save.message);
                ...
                maskBundleFull = maskBundle;
                heavyFields = {'brainImage', 'anatomical_reference_raw', 'anatomical_reference', ...
                    'sliceUnderlayRaw', 'sliceUnderlayProcessed'};
                for hh = 1:numel(heavyFields)
                    if isfield(maskBundle, heavyFields{hh})
                        maskBundle = rmfield(maskBundle, heavyFields{hh});
                    end
                end
                maskEditorInfo.saveFallback = 'compact_maskBundle_top_level_underlays_preserved';
            ...
            [moveOK, moveMsg] = movefile(tmpOutFile, outFile, 'f');
            if ~moveOK
                error('Could not finalize saved mask file: %s', moveMsg);
            end

        catch ME
            try
                if exist('tmpOutFile','var') && exist(tmpOutFile,'file') == 2
                    delete(tmpOutFile);
                end
            catch
            end
...
% -------------------- String helpers --------------------
    ...
    function outFile = maskEditorShortOutputFile(visDir, filePrefix, datasetLabel, ts)
        % HUMOR_MASK_MOTOR_SAFE_SAVE_PATCH_V1
        try
            p = lower(char(filePrefix));
        catch
            p = '';
        end

        if ~isempty(strfind(p,'underlayandoverlay'))
            shortPrefix = 'ME_both';
        elseif ~isempty(strfind(p,'overlay'))
            shortPrefix = 'ME_overlay';
        elseif ~isempty(strfind(p,'underlay')) || ~isempty(strfind(p,'brain'))
            shortPrefix = 'ME_underlay';
        else
            shortPrefix = 'ME_mask';
        end

        stem = safeFileStem(datasetLabel);
        if numel(stem) > 18
            stem = stem(1:18);
            stem = regexprep(stem,'_+$','');
        end
        ...
        outFile = fullfile(visDir, sprintf('%s_%s_%s.mat', shortPrefix, stem, ts));

        % Windows/network paths can fail when too long. Drop dataset stem if needed.
        if numel(outFile) > 230
            outFile = fullfile(visDir, sprintf('%s_%s.mat', shortPrefix, ts));
        end

        if exist(outFile,'file') == 2
            outFile = fullfile(visDir, sprintf('%s_%s_%06d.mat', shortPrefix, ts, randi(999999)));
        end
    end

    function tmpOutFile = maskEditorTempSaveFile(outFile)
        % HUMOR_MASK_MOTOR_SAFE_SAVE_PATCH_V1
