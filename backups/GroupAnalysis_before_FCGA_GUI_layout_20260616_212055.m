...
for ii = 1:numel(FC.subjects)
    subj0 = FC.subjects(ii);
    fp = ''; nm = ''; grp = 'Unassigned';
    try, fp = strtrimSafe(subj0.sourceFile); catch, end
    try, nm = strtrimSafe(subj0.name); catch, end
    try, grp = strtrimSafe(subj0.group); catch, end
    if isempty(fp) && nargin >= 2 && ~isempty(fileList)
        try, fp = strtrimSafe(fileList{min(ii,numel(fileList))}); catch, end
    end
    if isempty(nm), nm = guessSubjectID(fp); end
    if isempty(grp), grp = inferGroupFromText_TARGETED([nm ' ' fp]); end
    if isempty(grp), grp = 'Unassigned'; end
    cond = mapConditionFromGroup(S,grp);
    if isempty(cond)
        cond = 'CondA';
        if contains(upper(grp),'VEH') || contains(upper(grp),'CONTROL') || contains(upper(grp),'GROUPB')
            cond = 'CondB';
        end
    end
    hit = findFCGARow_TARGETED(S,nm,fp);
    if isempty(hit)
        row = {true,nm,grp,cond,nm,'','','','OK'};
        S.subj(end+1,:) = row;
        S = ensureFCRowFilesSizeGA_TARGETED(S);
        hit = size(S.subj,1);
    end
    S.subj{hit,1} = true;
    if ~isempty(nm), S.subj{hit,2} = nm; end
    if ~isempty(grp), S.subj{hit,3} = grp; end
    if ~isempty(cond), S.subj{hit,4} = cond; end
    if isempty(strtrimSafe(S.subj{hit,5})), S.subj{hit,5} = S.subj{hit,2}; end
