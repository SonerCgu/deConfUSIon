...
S.paramPanel = uipanel(fig,'Title','Selected step settings', ...
    'Units','normalized', ...
    'Position',[0.735 0.405 0.250 0.425], ...
...
refreshRows(fig);
refreshSelectedPanel(fig);
setStatus(fig,'Option B loaded: detailed workflow restored.',[0.80 0.95 0.85]);
end
...
steps = repmat(emptyStep(),1,19);

steps(1)  = makeStep(true,  1,'Motor',                 '4 slices, split MAT folder, residual despike 4, correction none',4,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
steps(2)  = makeStep(false, 2,'Full QC',               'optional pre-QC after Motor; currently unticked',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
...
steps(5)  = makeStep(false, 5,'Filtering',             'optional filtering; default preset kept but unticked',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
...
steps(10) = makeStep(true, 10,'Full QC',               'Full QC after Imregdemons/preprocessing',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
...
steps(13) = makeStep(true, 13,'Time-Course Viewer',    'inspect ROI/time-course; workflow waits until closed',NaN,NaN,30,35,-100,100,NaN,NaN);
steps(14) = makeStep(true, 14,'SCM GUI',               'native-space SCM before atlas registration',NaN,NaN,30,35,-100,100,-20,20);
steps(15) = makeStep(true, 15,'Registration to Atlas', 'atlas/coregistration before segmentation',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
steps(16) = makeStep(true, 16,'SCM GUI',               'SCM again after atlas registration',NaN,NaN,30,35,-100,100,-20,20);
steps(17) = makeStep(true, 17,'Segmentation',          'segmentation after atlas registration',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
steps(18) = makeStep(true, 18,'Functional Connectivity','Functional Connectivity after Segmentation',NaN,NaN,NaN,NaN,-1,1,NaN,NaN);
steps(19) = makeStep(false,19,'Group Analysis',        'optional group-level analysis',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
