...
function refreshRows()
    ...
    for i = 1:n
        ...
        set(state.hName(i),'String',s.name,'BackgroundColor',bg,'ForegroundColor',fg,'FontWeight',fw);
...
function onRunWorkflow(~,~)
    ...
    answer = questdlg(sprintf('Run %d ticked workflow steps in displayed order?',numel(idx)), ...
        'Run standardized workflow','Run','Cancel','Run');
    if ~strcmp(answer,'Run'), return; end
    for ii = 1:numel(idx)
        r = idx(ii);
...
switch lower(strtrim(stepName))
    ...
    case 'video gui', candidates = {'Video GUI','Video','Open Video GUI'};
