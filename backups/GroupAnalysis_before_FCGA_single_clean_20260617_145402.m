...
pFCTop = uipanel(pFCBG,'Units','normalized','Position',[0.02 0.785 0.96 0.205], ...
...
%%% GA_FC_GUI_TARGETED_UI_20260616_START
% Re-layout existing FC controls into a 2-row control panel.
try
    set(S.hFCLoad,   'Position',[0.015 0.63 0.120 0.25]);
    set(S.hFCScan,   'Position',[0.145 0.63 0.105 0.25]);
    set(S.hFCGroupA, 'Position',[0.330 0.64 0.115 0.22]);
    set(S.hFCGroupB, 'Position',[0.535 0.64 0.115 0.22]);
    set(S.hFCDisplay,'Position',[0.735 0.64 0.120 0.22]);
    set(S.hFCThreshold,'Position',[0.120 0.18 0.060 0.20]);
    set(S.hFCCompute,'Position',[0.750 0.13 0.120 0.25]);
    set(S.hFCExport, 'Position',[0.875 0.13 0.110 0.25]);
catch
end

uicontrol(pFCTop,'Style','text','String','View:', ...
    'Units','normalized','Position',[0.015 0.42 0.060 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCView = uicontrol(pFCTop,'Style','popupmenu', ...
    'String',{'4 matrices','Group A mean','Group B mean','Difference A-B','p-values','Seed/region profile'}, ...
    'Units','normalized','Position',[0.075 0.405 0.150 0.19], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Active:', ...
    'Units','normalized','Position',[0.245 0.42 0.065 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCActiveGroup = uicontrol(pFCTop,'Style','popupmenu', ...
    'String',{'Group A','Group B'}, ...
    'Units','normalized','Position',[0.310 0.405 0.095 0.19], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Seed / region:', ...
    'Units','normalized','Position',[0.425 0.42 0.105 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCRegion1 = uicontrol(pFCTop,'Style','popupmenu', ...
    'String',{'Load FC first'}, ...
    'Units','normalized','Position',[0.535 0.405 0.145 0.19], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'Callback',@updateFCTabPreview);
S.hFCRegion2 = uicontrol(pFCTop,'Style','popupmenu', ...
    'String',{'Load FC first'}, ...
    'Units','normalized','Position',[0.690 0.405 0.145 0.19], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Matrix mode:', ...
    'Units','normalized','Position',[0.015 0.19 0.095 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCMatrixMode = uicontrol(pFCTop,'Style','popupmenu', ...
    'String',{'Full matrix','Seed row/profile','Region pair values'}, ...
    'Units','normalized','Position',[0.190 0.165 0.160 0.19], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Threshold:', ...
    'Units','normalized','Position',[0.365 0.19 0.080 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
try, set(S.hFCThreshold,'Position',[0.445 0.165 0.070 0.19]); catch, end
try, set(S.hFCDisplay,'Position',[0.590 0.165 0.130 0.19]); catch, end
%%% GA_FC_GUI_TARGETED_UI_20260616_END

S.hFCInfo = uicontrol(pFCBG,'Style','text', ...
    'String','Load FC_GroupBundle_*.mat files exported from FunctionalConnectivity.m.', ...
    'Units','normalized','Position',[0.02 0.018 0.96 0.038], ...
    'BackgroundColor',C.bg,'ForegroundColor',C.muted, ...
    'HorizontalAlignment','left','FontName','Consolas','FontSize',10);

pFCAx = uipanel(pFCBG,'Units','normalized','Position',[0.02 0.070 0.96 0.700], ...
    'Title','FC matrices', ...
    'BackgroundColor',bg2,'ForegroundColor','w','FontWeight','bold');
...
drawnow;
        try
            ...
            S0.FC = FC;
            S0.FC.loaded = true;
            S0 = attachFCGABundlesToTable_TARGETED(S0,fileList,FC);
            S0 = sanitizeTableStruct(S0);
            S0 = ensureRowPacapSideSize(S0);
            S0 = ensureFCRowFilesSizeGA_TARGETED(S0);
...
drawnow;
    ...
    function updateFCTabPreview(~,~)
        ...
        if ~isfield(S0,'lastFC') || isempty(fieldnames(S0.lastFC))
            try
                fcNoDataLocal(S0.axFCA,'Group A mean FC',S0.C);
                fcNoDataLocal(S0.axFCB,'Group B mean FC',S0.C);
                fcNoDataLocal(S0.axFCD,'Difference: A - B',S0.C);
                fcNoDataLocal(S0.axFCP,'p-value map',S0.C);
            catch
            end
            return;
        end
        try
            updateFCTabPreview_TARGETED(S0);
        catch ME
            try, GA_printErrorLocal(ME,'caught error in enhanced FC preview'); catch, end
...
% GA_FC_GUI_TARGETED_HELPERS_20260616_START
function refreshFCRegionPopups_TARGETED(hFig)
try
    S = guidata(hFig);
    ...
    if isempty(labels) && isfield(subj,'displayLabels'), labels = double(subj.displayLabels(:)); end
    if isempty(names) && isfield(subj,'displayNames'), names = subj.displayNames(:); end
    if isempty(labels)
    ...
    for ii = 1:numel(labels)
        nm = strtrimSafe(names{ii});
        if numel(nm) > 44, nm = [nm(1:41) '...']; end
        items{ii} = sprintf('%g | %s',labels(ii),nm);
    end
    if isempty(items), items = {'No ROI labels'}; end
    if isfield(S,'hFCRegion1') && ishghandle(S.hFCRegion1)
        oldVal = get(S.hFCRegion1,'Value');
        set(S.hFCRegion1,'String',items,'Value',max(1,min(oldVal,numel(items))));
    end
    if isfield(S,'hFCRegion2') && ishghandle(S.hFCRegion2)
        oldVal = get(S.hFCRegion2,'Value');
        set(S.hFCRegion2,'String',items,'Value',max(1,min(max(oldVal,2),numel(items))));
    end
catch
end
end

function updateFCTabPreview_TARGETED(S)
R = S.lastFC;
thr = 0;
try, thr = safeNum(get(S.hFCThreshold,'String'),0); catch, end
dispMode = GA_fc_popup_TARGETED(S,'hFCDisplay','Pearson r');
viewMode = GA_fc_popup_TARGETED(S,'hFCView','4 matrices');
matrixMode = GA_fc_popup_TARGETED(S,'hFCMatrixMode','Full matrix');
activeGroup = GA_fc_popup_TARGETED(S,'hFCActiveGroup','Group A');
...
if thr > 0
    A(abs(A) < thr) = 0; B(abs(B) < thr) = 0; D(abs(D) < thr) = 0;
end

idx1 = GA_fc_popup_index_TARGETED(S,'hFCRegion1',1);
idx2 = GA_fc_popup_index_TARGETED(S,'hFCRegion2',min(2,size(A,1)));
idx1 = max(1,min(idx1,size(A,1)));
idx2 = max(1,min(idx2,size(A,1)));

if strcmpi(matrixMode,'Seed row/profile') && ~strcmpi(viewMode,'Seed/region profile')
    viewMode = 'Seed/region profile';
end
if strcmpi(matrixMode,'Region pair values') && ~strcmpi(viewMode,'Seed/region profile')
    viewMode = 'Seed/region profile';
end

switch lower(viewMode)
    case '4 matrices'
        GA_fc_axes4_TARGETED(S);
        GA_fc_plot_matrix_TARGETED(S.axFCA,A,climMain,['Mean FC: ' R.groupA ' (' valTxt ')'],R.names,S.C);
        GA_fc_plot_matrix_TARGETED(S.axFCB,B,climMain,['Mean FC: ' R.groupB ' (' valTxt ')'],R.names,S.C);
        GA_fc_plot_matrix_TARGETED(S.axFCD,D,climDiff,[R.groupA ' - ' R.groupB],R.names,S.C);
        GA_fc_plot_p_TARGETED(S.axFCP,R.pMat,['p-values: ' R.groupA ' vs ' R.groupB],R.names,S.C);
    case 'group a mean'
        GA_fc_single_TARGETED(S);
        GA_fc_plot_matrix_TARGETED(S.axFCA,A,climMain,['Mean FC: ' R.groupA ' (' valTxt ')'],R.names,S.C);
    case 'group b mean'
        GA_fc_single_TARGETED(S);
        GA_fc_plot_matrix_TARGETED(S.axFCA,B,climMain,['Mean FC: ' R.groupB ' (' valTxt ')'],R.names,S.C);
    case 'difference a-b'
        GA_fc_single_TARGETED(S);
        GA_fc_plot_matrix_TARGETED(S.axFCA,D,climDiff,[R.groupA ' - ' R.groupB ' (' valTxt ')'],R.names,S.C);
    case 'p-values'
        GA_fc_single_TARGETED(S);
        GA_fc_plot_p_TARGETED(S.axFCA,R.pMat,['p-values: ' R.groupA ' vs ' R.groupB],R.names,S.C);
    otherwise
        GA_fc_seed_profile_TARGETED(S,A,B,D,R,idx1,idx2,dispMode,activeGroup,matrixMode);
end

try
    if isfield(S,'hFCInfo') && ishghandle(S.hFCInfo)
        set(S.hFCInfo,'String',sprintf('Loaded %d FC subject(s). %s | View: %s | Seed=%s | Region=%s',numel(R.subjectNames),R.note,viewMode,GA_fc_name_TARGETED(R,idx1),GA_fc_name_TARGETED(R,idx2)));
    end
catch
end
end

function GA_fc_axes4_TARGETED(S)
try, set(S.axFCA,'Visible','on','Position',[0.060 0.565 0.365 0.360]); catch, end
try, set(S.axFCB,'Visible','on','Position',[0.570 0.565 0.365 0.360]); catch, end
try, set(S.axFCD,'Visible','on','Position',[0.060 0.090 0.365 0.360]); catch, end
try, set(S.axFCP,'Visible','on','Position',[0.570 0.090 0.365 0.360]); catch, end
end

function GA_fc_single_TARGETED(S)
try, set(S.axFCA,'Visible','on','Position',[0.080 0.120 0.820 0.790]); catch, end
...
function GA_fc_seed_profile_TARGETED(S,A,B,D,R,idx1,idx2,dispMode,activeGroup,matrixMode)
GA_fc_axes4_TARGETED(S);
n = size(A,1); x = 1:n;
if strcmpi(activeGroup,'Group B'), seedVec = B(idx1,:); seedTitle = ['Seed profile: ' R.groupB]; else, seedVec = A(idx1,:); seedTitle = ['Seed profile: ' R.groupA]; end
cla(S.axFCA); plot(S.axFCA,x,seedVec,'LineWidth',1.8); hold(S.axFCA,'on'); plot(S.axFCA,idx2,seedVec(idx2),'o','MarkerSize',8,'LineWidth',2); hold(S.axFCA,'off');
set(S.axFCA,'Color',S.C.axisBg,'XColor',S.C.muted,'YColor',S.C.muted); grid(S.axFCA,'on');
title(S.axFCA,[seedTitle ' | seed ' GA_fc_name_TARGETED(R,idx1) ' | ' dispMode],'Color',S.C.txt,'Interpreter','none','FontWeight','bold');
ylabel(S.axFCA,dispMode,'Color',S.C.txt); xlabel(S.axFCA,'ROI index','Color',S.C.txt);

vals = [A(idx1,idx2), B(idx1,idx2), D(idx1,idx2)];
cla(S.axFCB); bar(S.axFCB,vals);
set(S.axFCB,'XTick',1:3,'XTickLabel',{R.groupA,R.groupB,'Diff'},'Color',S.C.axisBg,'XColor',S.C.muted,'YColor',S.C.muted);
title(S.axFCB,['Pair value: ' GA_fc_name_TARGETED(R,idx1) ' ↔ ' GA_fc_name_TARGETED(R,idx2)],'Color',S.C.txt,'Interpreter','none','FontWeight','bold');
ylabel(S.axFCB,dispMode,'Color',S.C.txt); grid(S.axFCB,'on');

GA_fc_plot_matrix_TARGETED(S.axFCD,D,[-1 1],[R.groupA ' - ' R.groupB],R.names,S.C);
GA_fc_plot_p_TARGETED(S.axFCP,R.pMat,'p-values',R.names,S.C);
try
    txt = sprintf('A=%.4g | B=%.4g | Diff=%.4g | p=%.4g',A(idx1,idx2),B(idx1,idx2),D(idx1,idx2),R.pMat(idx1,idx2));
    xlabel(S.axFCB,txt,'Color',S.C.txt,'Interpreter','none');
catch
end
end

function GA_fc_plot_matrix_TARGETED(ax,M,climVal,titleStr,names,C)
cla(ax);
if isempty(M), fcNoDataLocal(ax,titleStr,C); return; end
imagesc(ax,M); axis(ax,'image');
try, caxis(ax,climVal); catch, end
colormap(ax,GA_fc_bwr_TARGETED(256));
...
title(ax,titleStr,'Color',C.txt,'FontWeight','bold','Interpreter','none');
nR = size(M,1); ticks = GA_fc_ticks_TARGETED(nR);
set(ax,'XTick',ticks,'YTick',ticks,'XTickLabel',GA_fc_abbrev_TARGETED(names(ticks),10),'YTickLabel',GA_fc_abbrev_TARGETED(names(ticks),10));
try, xtickangle(ax,90); catch, end
end

function GA_fc_plot_p_TARGETED(ax,P,titleStr,names,C)
cla(ax);
if isempty(P), fcNoDataLocal(ax,titleStr,C); return; end
Plog = -log10(P); Plog(~isfinite(Plog)) = NaN;
imagesc(ax,Plog); axis(ax,'image'); caxis(ax,[0 3]); colormap(ax,hot(256));
cb = colorbar(ax); try, set(cb,'Color',[1 1 1]); catch, end
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontName','Arial','FontSize',8,'TickLength',[0 0]);
title(ax,[titleStr ' (-log10 p)'],'Color',C.txt,'FontWeight','bold','Interpreter','none');
nR = size(P,1); ticks = GA_fc_ticks_TARGETED(nR);
set(ax,'XTick',ticks,'YTick',ticks,'XTickLabel',GA_fc_abbrev_TARGETED(names(ticks),10),'YTickLabel',GA_fc_abbrev_TARGETED(names(ticks),10));
try, xtickangle(ax,90); catch, end
end

function s = GA_fc_popup_TARGETED(S,fieldName,fallback)
s = fallback;
try
...
function idx = GA_fc_popup_index_TARGETED(S,fieldName,fallback)
idx = fallback;
try, if isfield(S,fieldName) && ishghandle(S.(fieldName)), idx = get(S.(fieldName),'Value'); end, catch, end
end

function nm = GA_fc_name_TARGETED(R,idx)
...
function cmap = GA_fc_bwr_TARGETED(n)
...
function r = findExistingFCGARow_TARGETED(S,nm,fp)
r = [];
nm = lower(strtrimSafe(nm)); fp = strtrimSafe(fp);
try, S = ensureFCRowFilesSizeGA_TARGETED(S); catch, end
% Match by file first if already attached.
try
...
% Match by table animal/subject/pair ID.
if isempty(nm), return; end
try
    for ii = 1:size(S.subj,1)
        a = lower(strtrimSafe(S.subj{ii,2}));
        p = lower(strtrimSafe(S.subj{ii,5}));
        if strcmp(a,nm) || strcmp(p,nm) || contains(nm,a) || contains(a,nm)
            r = ii; return;
        end
    end
catch
...
% GA_FC_GUI_TARGETED_HELPERS_20260616_END
