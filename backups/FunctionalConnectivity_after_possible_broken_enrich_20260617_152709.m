...
try, fcBundle = deConfUSIon_FCGA_enrichBundleForExport_20260617(fcBundle); catch ME_enrich, try, disp(['FC GA enrich warning: ' ME_enrich.message]); catch, end, end
...
% FC_GA_EXPORT_ENRICH_20260617_START
function fcBundle = deConfUSIon_FCGA_enrichBundleForExport_20260617(fcBundle)
% Adds spatial ROI/label/segmentation maps to FC GA bundles when available in the export workspace.
try
    [map2,note] = deConfUSIon_FCGA_findSpatialMapInCaller_20260617();
    if ~isempty(map2)
        fcBundle.roiMap = map2;
        fcBundle.labelMap = map2;
        fcBundle.spatialMapNote = note;
        if isfield(fcBundle,'subjects')
            for ii=1:numel(fcBundle.subjects)
                fcBundle.subjects(ii).roiMap = map2;
                fcBundle.subjects(ii).labelMap = map2;
                fcBundle.subjects(ii).spatialMapNote = note;
            end
        end
    end
catch
...
function [map2,note] = deConfUSIon_FCGA_findSpatialMapInCaller_20260617()
map2=[]; note='';
    ...
    try
        if evalin('caller',sprintf('exist(''%s'',''var'')',names{ii}))
            ...
            if isnumeric(X) || islogical(X)
                map2=squeeze(double(X)); note=names{ii}; if ndims(map2)>2,map2=map2(:,:,round(size(map2,3)/2));end; return;
            end
        end
    catch
    end
...
try
    ...
    for vv=1:numel(vars)
        if vars(vv).bytes > 5e8, continue; end
        X = evalin('caller',vars(vv).name);
        [map2,note] = deConfUSIon_FCGA_findMapRecursive_20260617(X,vars(vv).name,0);
...
function [map2,note] = deConfUSIon_FCGA_findMapRecursive_20260617(X,prefix,depth)
map2=[]; note=''; if depth>3, return; end
keys={'roiMap','labelMap','parcelMap','labelMask','roiLabelMask','atlasLabels2D','maskLabels','segmentationMap','segMap','segmentation','labels2D'};
try
    if isstruct(X)
        ...
        for i=1:numel(fn)
            val=X.(fn{i});
            if any(strcmp(fn{i},keys)) && (isnumeric(val)||islogical(val))
                map2=squeeze(double(val)); note=[prefix '.' fn{i}]; if ndims(map2)>2,map2=map2(:,:,round(size(map2,3)/2));end; return;
            end
        end
        for i=1:numel(fn)
            val=X.(fn{i}); if isstruct(val), [map2,note]=deConfUSIon_FCGA_findMapRecursive_20260617(val,[prefix '.' fn{i}],depth+1); if ~isempty(map2), return; end, end
        end
    end
catch
end
end
% FC_GA_EXPORT_ENRICH_20260617_END
