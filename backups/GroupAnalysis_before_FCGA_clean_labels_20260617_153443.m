...
try
    ...
    S.hFCAnimals = mkBtn(pFCTop,'Animals...',[0.735 0.155 0.080 0.205],C.btnSecondary,@(~,~) selectFCAnimals_ADV_20260617(hFig));
    S.hFCNames   = mkBtn(pFCTop,'Names',[0.825 0.155 0.065 0.205],C.btnSecondary,@(~,~) showFCRegionNames_ADV_20260617(hFig));
...
drawnow;
        try
            ...
            try, refreshFCRegionPopups_ADV_20260617(hFig); catch, end
            try, refreshFCSubjectPopup_ADV_20260617(hFig); catch, end
                try, refreshFCSlicePopup_ADV_20260617(hFig); catch, end
...
function refreshFCRegionPopups_ADV_20260617(hFig)
try
...
function refreshFCSlicePopup_ADV_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
    nZ=1;
    for ii=1:numel(S.FC.subjects)
