...
uicontrol(state.fig,'Style','pushbutton','String','Recommended', ...
    ...
    'FontSize',15,'FontWeight','bold','BackgroundColor',[0.18 0.20 0.25],'ForegroundColor',[1 1 1], ...
...
uicontrol(state.fig,'Style','pushbutton','String','Run Workflow', ...
    ...
    'FontSize',18,'FontWeight','bold','BackgroundColor',[0.08 0.48 0.26],'ForegroundColor',[1 1 1], ...
...
uicontrol(state.fig,'Style','pushbutton','String','Run Step', ...
    ...
    'FontSize',18,'FontWeight','bold','BackgroundColor',[0.10 0.25 0.55],'ForegroundColor',[1 1 1], ...
...
function createRows()
    ...
    for i = 1:n
        ...
        S.hRow(i) = uipanel(S.listPanel,'Units','normalized','Position',[0.006 y 0.988 rowH], ...
            ...
            'Callback',@(src,evt) selectRowOnly(i),'ButtonDownFcn',@(src,evt) startDrag(i));
        S.hArrow(i) = uicontrol(S.hRow(i),'Style','text','String','', ...
            'Units','normalized','Position',[0.004 0.12 0.040 0.76], ...
            'FontSize',18,'FontWeight','bold','Callback',@(src,evt) selectRowOnly(i),'ButtonDownFcn',@(src,evt) startDrag(i));
...
function refreshRows()
    ...
    for i = 1:numel(S.steps)
        ...
        elseif mod(i,2)==0
            bg = [0.095 0.105 0.145]; fg = [0.92 0.96 1.00]; marker = '';
        else
            bg = [0.075 0.085 0.125]; fg = [0.92 0.96 1.00]; marker = '';
        end
...
function refreshSelectedPanel()
    ...
    uicontrol(S.paramPanel,'Style','text','String',['#' num2str(r) '  ' s.name], ...
        ...
        'BackgroundColor',[0.060 0.065 0.095],'ForegroundColor',[0.55 1.00 0.62], ...
        'FontSize',18,'FontWeight','bold','HorizontalAlignment','left');
    uicontrol(S.paramPanel,'Style','checkbox','String','Run this step','Value',s.run, ...
        'Units','normalized','Position',[0.04 0.80 0.90 0.07], ...
        'BackgroundColor',[0.060 0.065 0.095],'ForegroundColor',[0.92 0.96 1.00], ...
        'FontSize',18,'FontWeight','bold','Callback',@(src,evt) toggleRun(r,src));
    ...
    if isempty(fields)
        uicontrol(S.paramPanel,'Style','text','String','No numeric preset for this step yet.', ...
            ...
            'BackgroundColor',[0.060 0.065 0.095],'ForegroundColor',[0.80 0.88 0.95], ...
            'FontSize',15,'FontWeight','bold','HorizontalAlignment','left');
    else
        for k = 1:numel(fields)
            ...
            uicontrol(S.paramPanel,'Style','edit','String',num2str(s.(f)), ...
                ...
                'FontSize',15,'FontWeight','bold','Callback',@(src,evt) editParam(src,f));
            y = y - dy;
        end
    end
...
steps(3)  = makeStep(false, 3,'Frame Rejection',       'optional frame rejection before/after Motor',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
steps(4)  = makeStep(false, 4,'Scrubbing',             'optional scrubbing after correction',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
...
steps(15) = makeStep(true, 15,'Registration to Atlas', 'atlas/coregistration workflow',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
steps(16) = makeStep(true, 16,'Segmentation',          'segmentation before Functional Connectivity',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
steps(17) = makeStep(true, 17,'Functional Connectivity','Functional Connectivity after Segmentation',NaN,NaN,NaN,NaN,-1,1,NaN,NaN);
...
try
    ...
    mustWait = any(strcmp(nm,{'video gui','scm gui','registration to atlas','time-course viewer'}));
