...
R = S.lastFC;
C = fcDefaultColorsV14();
startDir = pwd;
try, if isfield(S,'outDir') && exist(S.outDir,'dir')==7, startDir = S.outDir; end, catch, end
...
names = R.names;
try
    if isfield(S,'fcAtlasLabelFile') && exist(S.fcAtlasLabelFile,'file')==2
        names = fcNamesFromAtlasFileV14(R.labels,names,S.fcAtlasLabelFile);
    end
catch
end
...
writeFCMatrixCSV(fullfile(outRoot,'meanZA_FisherZ_GroupA.csv'),R.meanZA,names);
...
if isempty(S) || ~isstruct(S), error('Missing GroupAnalysis state struct for FC export.'); end
if ~isfield(S,'lastFC') || isempty(fieldnames(S.lastFC)), error('Compute FC first.'); end
R=S.lastFC; C=fcDefaultColorsV15();
