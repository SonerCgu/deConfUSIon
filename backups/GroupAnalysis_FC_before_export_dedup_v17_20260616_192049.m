...
for i = 1:numel(fileList)
    ...
    for j = 1:numel(B.subjects)
        ...
        % FC_META_COPY_V14: preserve ROI/hemisphere/side/provenance fields when present.
        try, FC.subjects(idx).fcMeta = subj; catch, end
        try
            metaFieldsV14 = {'roiSet','roiMode','hemisphere','hemiMode','side','selectionMode','mergedHemisphere', ...
                'leftRightMode','roiSide','regionSide','analysisMode','atlasLabelFile','labelFile','TR','timeIdx', ...
                'nSlices','isStepMotor3D','analysisDir','sourceDir'};
            for mfV14 = 1:numel(metaFieldsV14)
                ffV14 = metaFieldsV14{mfV14};
                if isfield(subj,ffV14)
                    FC.subjects(idx).(ffV14) = subj.(ffV14);
                end
            end
        catch
        end

        FC.subjects(idx).condition = fcGetFieldCharCompatV14(subj,'condition','');
        FC.subjects(idx).animalID = fcGetFieldCharCompatV14(subj,'animalID','');
        FC.subjects(idx).rowIndex = fcGetFieldNumCompatV14(subj,'rowIndex',NaN);
        FC.subjects(idx).condition = fcGetFieldCharCompatV14(subj,'condition','');
        FC.subjects(idx).animalID = fcGetFieldCharCompatV14(subj,'animalID','');
        FC.subjects(idx).rowIndex = fcGetFieldNumCompatV14(subj,'rowIndex',NaN);
...
% Flexible FC stats: two-group comparison if possible, otherwise single-group summary.
if nargin < 2 || isempty(groupA), groupA = ''; end
if nargin < 3, groupB = ''; end
groups = G.groups(:);
u = uniqueStableFCV14(groups);
...
if ~isempty(groupB) && ~strcmpi(groupA,groupB) && any(strcmpi(groups,groupB))
    ...
    if numel(u) >= 2
        groupB = u{find(~strcmpi(u,groupA),1,'first')};
        idxB = strcmpi(groups,groupB);
    else
        groupB = 'None';
    end
...
try, R.conditions = G.conditions; catch, R.conditions = {}; end
try, R.rowIndex = G.rowIndex; catch, R.rowIndex = []; end
R.note = 'Single-group mode is allowed. Two-group p-values require both groups and enough subjects.';
end

function fcPlotFCAdvancedOneViewV14(ax,FC,R,seedText,viewMode,activeGroup,dispMode,thr,C)
if nargin < 9 || isempty(C), C = fcDefaultColorsV14(); end
if nargin < 8 || isempty(thr), thr = 0; end
if nargin < 7 || isempty(dispMode), dispMode = 'Pearson r'; end
if nargin < 6 || isempty(activeGroup), activeGroup = R.groupA; end
if nargin < 5 || isempty(viewMode), viewMode = 'Matrix summary'; end
cla(ax);
set(ax,'Visible','on','Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',11);
viewLower = lower(strtrimSafe(viewMode));
if contains(viewLower,'seed')
    fcPlotSeedOneV14(ax,R,seedText,activeGroup,dispMode,thr,C);
elseif contains(viewLower,'max')
    fcPlotMaxConnectionsOneV14(ax,R,activeGroup,dispMode,thr,C);
elseif contains(viewLower,'time')
    fcPlotROITimeCourseOneV14(ax,FC,R,seedText,activeGroup,C);
elseif contains(viewLower,'heat') || contains(viewLower,'subject')
    fcPlotSubjectHeatmapOneV14(ax,FC,R,seedText,activeGroup,dispMode,thr,C);
else
    fcPlotMatrixOneV14(ax,R,activeGroup,dispMode,thr,C);
end
end

function fcPlotMatrixOneV14(ax,R,activeGroup,dispMode,thr,C)
[M,climVal,titlePart,ok,msg] = fcGetMatrixForGroupV14(R,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
Mplot = M;
if thr > 0, Mplot(abs(Mplot) < thr) = 0; end
imagesc(ax,Mplot);
axis(ax,'image');
colormap(ax,fcBlueWhiteRed(256));
caxis(ax,climVal);
cb = colorbar(ax);
try, cb.Color = C.txt; catch, end
title(ax,[titlePart ' | Hide |r| < ' num2str(thr)],'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',10);
tickIdx = fcTickIdx(numel(R.names));
set(ax,'XTick',tickIdx,'XTickLabel',fcAbbrevNames(R.names(tickIdx),14), ...
    'YTick',tickIdx,'YTickLabel',fcAbbrevNames(R.names(tickIdx),14));
try, xtickangle(ax,90); catch, end
end

function fcPlotSeedOneV14(ax,R,seedText,activeGroup,dispMode,thr,C)
[M,~,titlePart,ok,msg] = fcGetMatrixForGroupV14(R,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
seedIdx = fcFindSeedIndexV14(R,seedText);
v = M(seedIdx,:);
v(seedIdx) = NaN;
...
ord = ord(1:N);
vals = v(ord);
labels = fcAbbrevNames(R.names(ord),28);
cla(ax);
if isempty(vals)
    fcNoData(ax,'No seed connections pass the visual threshold.',C); return;
end
barh(ax,flipud(vals(:)));
set(ax,'YTick',1:N,'YTickLabel',flipud(labels(:)));
grid(ax,'on');
xlim(ax,[-max(1e-6,max(abs(vals))) max(1e-6,max(abs(vals)))]);
title(ax,sprintf('Seed profile: %g | %s | %s',R.labels(seedIdx),R.names{seedIdx},titlePart), ...
    'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
xlabel(ax,'Connection strength');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',10);
end

function fcPlotMaxConnectionsOneV14(ax,R,activeGroup,dispMode,thr,C)
[M,~,titlePart,ok,msg] = fcGetMatrixForGroupV14(R,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
n = size(M,1); rows = {};
for i = 1:n
    for j = i+1:n
        ...
        if isfinite(v) && abs(v) >= thr
            rows(end+1,:) = {abs(v),v,[fcShortNameV14(R.names{i}) '  ↔  ' fcShortNameV14(R.names{j})]}; %#ok<AGROW>
        end
    end
end
if isempty(rows), fcNoData(ax,'No edges pass the visual threshold.',C); return; end
s = cell2mat(rows(:,1));
[~,ord] = sort(s,'descend');
...
grid(ax,'on');
title(ax,['Strongest connections | ' titlePart],'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
xlabel(ax,'Connection strength');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',9);
end

function fcPlotROITimeCourseOneV14(ax,FC,R,seedText,activeGroup,C)
seedIdx = fcFindSeedIndexV14(R,seedText);
label = R.labels(seedIdx);
[T,subNames,ok,msg] = fcCollectSeedTimeCoursesV14(FC,label,activeGroup,R);
if ~ok
    fcNoData(ax,msg,C); return;
end
cla(ax); hold(ax,'on');
plot(ax,T,'Color',[0.45 0.45 0.45]);
m = nanmean_local(T,2);
plot(ax,m,'LineWidth',3);
hold(ax,'off'); grid(ax,'on');
title(ax,sprintf('ROI time course: %g | %s | %s | %d stored frames | n=%d', ...
    label,R.names{seedIdx},activeGroup,size(T,1),size(T,2)), ...
    'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
xlabel(ax,'Frame stored in FC bundle'); ylabel(ax,'ROI signal');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',11);
try, legend(ax,[subNames(:); {'mean'}],'Interpreter','none','TextColor',C.txt,'Color',C.axisBg,'Location','bestoutside'); catch, end
end

function fcPlotSubjectHeatmapOneV14(ax,FC,R,seedText,activeGroup,dispMode,thr,C)
seedIdx = fcFindSeedIndexV14(R,seedText);
label = R.labels(seedIdx);
[H,subNames,ok,msg] = fcCollectSeedRowsV14(FC,R,label,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
if thr > 0, H(abs(H)<thr) = 0; end
imagesc(ax,H);
colormap(ax,fcBlueWhiteRed(256));
if strcmpi(strtrimSafe(dispMode),'Fisher z'), caxis(ax,[-2.5 2.5]); else, caxis(ax,[-1 1]); end
cb = colorbar(ax);
try, cb.Color = C.txt; catch, end
title(ax,sprintf('Subject heatmap: seed %g | %s | %s',label,R.names{seedIdx},activeGroup), ...
    'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',9);
tickIdx = fcTickIdx(size(H,2));
set(ax,'XTick',tickIdx,'XTickLabel',fcAbbrevNames(R.names(tickIdx),14), ...
    'YTick',1:numel(subNames),'YTickLabel',subNames);
try, xtickangle(ax,90); catch, end
xlabel(ax,'Target region'); ylabel(ax,'Subject');
end

function [M,climVal,titlePart,ok,msg] = fcGetMatrixForGroupV14(R,activeGroup,dispMode)
ok = true; msg = '';
if nargin < 3 || isempty(dispMode), dispMode = 'Pearson r'; end
if nargin < 2 || isempty(activeGroup), activeGroup = R.groupA; end
...
if isDiff
    if isfield(R,'singleGroup') && R.singleGroup
        ok = false; msg = 'Difference is unavailable in single-group mode.';
        M = []; climVal = []; titlePart = ''; return;
    end
    if useZ, M = R.diffZ; climVal = [-1 1]; unitTxt = 'Fisher z'; else, M = R.diffR; climVal = [-1 1]; unitTxt = 'Pearson r'; end
    titlePart = [R.groupA ' - ' R.groupB ' (' unitTxt ')'];
elseif strcmpi(strtrimSafe(activeGroup),strtrimSafe(R.groupB))
    if isfield(R,'singleGroup') && R.singleGroup
        ok = false; msg = 'Group B is unavailable in single-group mode.';
        M = []; climVal = []; titlePart = ''; return;
    end
    if useZ, M = R.meanZB; climVal = [-2.5 2.5]; unitTxt = 'Fisher z'; else, M = R.meanRB; climVal = [-1 1]; unitTxt = 'Pearson r'; end
    titlePart = [R.groupB ' mean FC (' unitTxt ')'];
else
    if useZ, M = R.meanZA; climVal = [-2.5 2.5]; unitTxt = 'Fisher z'; else, M = R.meanRA; climVal = [-1 1]; unitTxt = 'Pearson r'; end
    titlePart = [R.groupA ' mean FC (' unitTxt ')'];
end
end

function idx = fcFindSeedIndexV14(R,seedText)
...
for ii = 1:numel(R.names)
    if contains(lower(strtrimSafe(R.names{ii})),s)
        idx = ii; return;
    end
end
end

function [T,subNames,ok,msg] = fcCollectSeedTimeCoursesV14(FC,label,activeGroup,R)
T = []; subNames = {}; ok = false;
msg = 'No ROI time courses found. The FC bundle must contain meanTS/roiTS/timeCourses for this view.';
if ~isfield(FC,'subjects') || isempty(FC.subjects), return; end
for ii = 1:numel(FC.subjects)
    s = FC.subjects(ii);
    g = strtrimSafe(s.group);
    if contains(lower(strtrimSafe(activeGroup)),'diff')
        useThis = true;
    else
        useThis = strcmpi(g,activeGroup);
        if ~useThis && strcmpi(activeGroup,R.groupA), useThis = strcmpi(g,R.groupA); end
        if ~useThis && strcmpi(activeGroup,R.groupB), useThis = strcmpi(g,R.groupB); end
    end
    if ~useThis, continue; end
    [X,labs] = fcGetSubjectTimeCourseMatrixV14(s);
    if isempty(X) || isempty(labs), continue; end
    hit = find(double(labs)==double(label),1,'first');
    if isempty(hit), continue; end
    if size(X,2)==numel(labs)
        tc = X(:,hit);
    elseif size(X,1)==numel(labs)
        tc = X(hit,:)';
    else
        continue;
    end
    T(:,end+1) = double(tc(:)); %#ok<AGROW>
    subNames{end+1,1} = strtrimSafe(s.name); %#ok<AGROW>
end
ok = ~isempty(T);
if ok, msg = ''; end
end

function [X,labs] = fcGetSubjectTimeCourseMatrixV14(s)
X = []; labs = [];
...
for ii = 1:numel(fields)
    ff = fields{ii};
    try
        if isfield(s,ff) && ~isempty(s.(ff)) && isnumeric(s.(ff))
            X = double(s.(ff));
            return;
        end
    catch
...
function [H,subNames,ok,msg] = fcCollectSeedRowsV14(FC,R,label,activeGroup,dispMode)
H = []; subNames = {}; ok = false; msg = 'No subject FC matrices found.';
if ~isfield(FC,'subjects') || isempty(FC.subjects), return; end
for ii = 1:numel(FC.subjects)
    s = FC.subjects(ii);
    g = strtrimSafe(s.group);
    ...
    if ~useThis, continue; end
    try, labs = double(s.labels(:)); catch, continue; end
    hit = find(double(labs)==double(label),1,'first'); if isempty(hit), continue; end
    if strcmpi(strtrimSafe(dispMode),'Fisher z') && isfield(s,'Z'), M = s.Z; else, M = s.R; end
    [~,idx] = ismember(double(R.labels),double(labs));
    if any(idx < 1), continue; end
    H(end+1,:) = double(M(hit,idx)); %#ok<AGROW>
    subNames{end+1,1} = strtrimSafe(s.name); %#ok<AGROW>
end
ok = ~isempty(H);
if ok, msg = ''; end
...
function u = uniqueStableFCV14(c)
u = {};
for ii = 1:numel(c)
    s = strtrimSafe(c{ii});
    if isempty(s), s = 'Unassigned'; end
    if ~any(strcmpi(u,s)), u{end+1,1} = s; end %#ok<AGROW>
end
end

function s = fcShortNameV14(s)
s = strtrimSafe(s);
if numel(s) > 34, s = [s(1:31) '...']; end
end

function C = fcDefaultColorsV14()
C = struct();
C.bg = [0.04 0.04 0.04];
...
ZA_FisherZ_GroupA.csv'),R.meanZA,names);
writeFCMatrixCSV(fullfile(outRoot,'meanZB_FisherZ_GroupB.csv'),R.meanZB,names);
writeFCMatrixCSV(fullfile(outRoot,'diffZ_GroupA_minus_GroupB.csv'),R.diffZ,names);
writeFCMatrixCSV(fullfile(outRoot,'p_values.csv'),R.pMat,names);
fcWriteRegionListCSV_V14(fullfile(outRoot,'region_labels_and_names.csv'),R.labels,names);
fcWriteSubjectMetadataCSV_V14(fullfile(outRoot,'subject_metadata_fc.csv'),S,R);
fcWriteTopConnectionsCSV_V14(fullfile(outRoot,'top_connections_GroupA.csv'),R,names,R.groupA,250);
if ~isfield(R,'singleGroup') || ~R.singleGroup
    fcWriteTopConnectionsCSV_V14(fullfile(outRoot,'top_connections_GroupB.csv'),R,names,R.groupB,250);
    fcWriteTopConnectionsCSV_V14(fullfile(outRoot,'top_differences_A_minus_B.csv'),R,names,'Difference A-B',250);
end
seedText = ''; try, seedText = S.fcSeedRegion; catch, end
if ~isempty(seedText), fcWriteSeedSummaryCSV_V14(fullfile(outRoot,'seed_region_summary.csv'),R,names,seedText); end
try
    FC = S.FC;
    views = {'Matrix summary','Seed profile','Max connections','ROI time course','Subject heatmap'};
    groups = {R.groupA};
    if ~isfield(R,'singleGroup') || ~R.singleGroup
        groups = {R.groupA,R.groupB,'Difference A-B'};
    end
    f = figure('Visible','off','Color',C.bg,'InvertHardcopy','off','Position',[100 100 1500 1000]);
    ax = axes('Parent',f,'Position',[0.08 0.10 0.82 0.80]);
    for gi = 1:numel(groups)
        for vi = 1:numel(views)
            cla(ax);
            fcPlotFCAdvancedOneViewV14(ax,FC,R,seedText,views{vi},groups{gi},'Pearson r',0,C);
            outFig = fullfile(outRoot,[sanitizeFilename(groups{gi}) '_' sanitizeFilename(views{vi}) '.png']);
            print(f,outFig,'-dpng','-r220');
        end
    end
    close(f);
catch
...
function namesOut = fcNamesFromAtlasFileV14(labels,namesIn,labelFile)
namesOut = namesIn;
try
    T = fileread(labelFile);
    L = regexp(T,'\r\n|\n|\r','split');
    ...
    for i = 1:numel(labels)
        id = double(labels(i));
        if isKey(mp,id), namesOut{i}=mp(id); end
    end
catch
end
end

function fcWriteRegionListCSV_V14(outFile,labels,names)
fid = fopen(outFile,'w'); if fid < 0, error('Could not write %s',outFile); end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'label,name\n');
for i = 1:numel(labels), fprintf(fid,'%g,%s\n',labels(i),csvEscapeFC(names{i})); end
end

function fcWriteSubjectMetadataCSV_V14(outFile,S,R)
fid = fopen(outFile,'w'); if fid < 0, error('Could not write %s',outFile); end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'subject,group,condition,rowIndex,rowSide_or_hemi,sourceFile,roiSet,roiMode,hemisphere,hemiMode,side,selectionMode,mergedHemisphere,nLabels,nSlices,isStepMotor3D,analysisDir\n');
FC = struct(); try, FC = S.FC; catch, end
if ~isfield(FC,'subjects'), return; end
for i = 1:numel(FC.subjects)
    s = FC.subjects(i);
    rowSide = '';
    rowIndex = NaN;
    try, rowIndex = s.rowIndex; catch, end
    try
        if isfinite(rowIndex) && isfield(S,'rowPacapSide') && numel(S.rowPacapSide) >= rowIndex
            rowSide = strtrimSafe(S.rowPacapSide{rowIndex});
        end
    catch
    end
    fprintf(fid,'%s,%s,%s,%.0f,%s,%s,%s,%s,%s,%s,%s,%s,%s,%d,%s,%s,%s\n', ...
        csvEscapeFC(fcGetMetaStringV14(s,'name')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'group')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'condition')), ...
        rowIndex, ...
        csvEscapeFC(rowSide), ...
        csvEscapeFC(fcGetMetaStringV14(s,'sourceFile')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'roiSet')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'roiMode')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'hemisphere')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'hemiMode')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'side')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'selectionMode')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'mergedHemisphere')), ...
        numel(fcGetLabelsV14(s)), ...
        csvEscapeFC(fcGetMetaStringV14(s,'nSlices')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'isStepMotor3D')), ...
        csvEscapeFC(fcGetMetaStringV14(s,'analysisDir')));
end
end

function labels = fcGetLabelsV14(s)
labels = []; try, labels = s.labels(:); catch, end
end

function v = fcGetMetaStringV14(s,fieldName)
v = '';
try
    if isstruct(s) && isfield(s,fieldName) && ~isempty(s.(fieldName))
        ...
        elseif isnumeric(x) || islogical(x), v = mat2str(x);
        else, v = class(x);
        end
    end
catch
...
function fcWriteTopConnectionsCSV_V14(outFile,R,names,activeGroup,N)
if nargin < 5, N = 250; end
[M,~,~,ok,~] = fcGetMatrixForGroupV14(R,activeGroup,'Pearson r');
if ~ok, return; end
...
for i = 1:n
    for j = i+1:n
        v = M(i,j);
        if isfinite(v)
            rows(end+1,:) = {abs(v),v,R.labels(i),names{i},R.labels(j),names{j}}; %#ok<AGROW>
        end
    end
end
...
fid = fopen(outFile,'w'); if fid < 0, error('Could not write %s',outFile); end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'rank,abs_value,value,label_i,name_i,label_j,name_j\n');
for k = 1:size(rows,1)
    fprintf(fid,'%d,%.10g,%.10g,%g,%s,%g,%s\n',k,rows{k,1},rows{k,2},rows{k,3},csvEscapeFC(rows{k,4}),rows{k,5},csvEscapeFC(rows{k,6}));
end
end

function fcWriteSeedSummaryCSV_V14(outFile,R,names,seedText)
seedIdx = fcFindSeedIndexV14(R,seedText);
fid = fopen(outFile,'w'); if fid < 0, error('Could not write %s',outFile); end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'seed_label,seed_name,target_label,target_name,meanR_A,meanR_B,diffR,p,t\n');
for j = 1:numel(names)
    if j == seedIdx, continue; end
    fprintf(fid,'%g,%s,%g,%s,%.10g,%.10g,%.10g,%.10g,%.10g\n', ...
        R.labels(seedIdx),csvEscapeFC(names{seedIdx}),R.labels(j),csvEscapeFC(names{j}), ...
        R.meanRA(seedIdx,j),R.meanRB(seedIdx,j),R.diffR(seedIdx,j),R.pMat(seedIdx,j),R.tMat(seedIdx,j));
end
end
...
set(ax,'Visible','on','Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',11);
viewLower = lower(strtrimSafe(viewMode));
if contains(viewLower,'pair')
    fcPlotPairCorrelationV15(ax,FC,R,reg1,reg2,activeGroup,dispMode,C);
elseif contains(viewLower,'seed')
    fcPlotSeedOneV15(ax,R,reg1,activeGroup,hemiMode,dispMode,thr,C);
elseif contains(viewLower,'max')
    fcPlotMaxConnectionsOneV15(ax,R,activeGroup,hemiMode,dispMode,thr,C);
elseif contains(viewLower,'time')
    fcPlotROITimeCourseOneV15(ax,FC,R,reg1,activeGroup,C);
elseif contains(viewLower,'heat') || contains(viewLower,'subject')
    fcPlotSubjectHeatmapOneV15(ax,FC,R,reg1,activeGroup,hemiMode,dispMode,thr,C);
elseif contains(viewLower,'graph') || contains(viewLower,'overlay')
    fcPlotRegionGraphOneV15(ax,R,reg1,activeGroup,hemiMode,dispMode,thr,C);
else
    fcPlotMatrixOneV15(ax,R,activeGroup,hemiMode,dispMode,thr,C);
end
end

function fcPlotMatrixOneV15(ax,R,activeGroup,hemiMode,dispMode,thr,C)
[M,climVal,titlePart,ok,msg] = fcGetMatrixForGroupV15(R,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
[Mplot,namesPlot,labelsPlot,msg2] = fcApplyHemisphereModeV15(M,R.names,R.labels,hemiMode);
if isempty(Mplot), fcNoData(ax,msg2,C); return; end
if thr > 0, Mplot(abs(Mplot)<thr)=0; end
imagesc(ax,Mplot);
axis(ax,'image');
colormap(ax,fcBlueWhiteRed(256));
...
title(ax,[titlePart ' | ' hemiMode ' | visual hide |r|<' num2str(thr)], ...
    'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',10);
tickIdx = fcTickIdx(numel(namesPlot));
set(ax,'XTick',tickIdx,'XTickLabel',fcAbbrevNames(namesPlot(tickIdx),14), ...
    'YTick',tickIdx,'YTickLabel',fcAbbrevNames(namesPlot(tickIdx),14));
try, xtickangle(ax,90); catch, end
xlabel(ax,'Target regions'); ylabel(ax,'Seed/source regions');
end

function fcPlotSeedOneV15(ax,R,reg1,activeGroup,hemiMode,dispMode,thr,C)
[M,~,titlePart,ok,msg] = fcGetMatrixForGroupV15(R,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
seedIdx = fcFindRegionIndexV15(R,reg1);
v = M(seedIdx,:); v(seedIdx)=NaN;
keep = fcHemisphereKeepMaskV15(R.names,hemiMode);
if contains(lower(hemiMode),'left vs right')
    keep = fcHemisphereMaskV15(R.names,'right');
end
v(~keep) = NaN;
if thr > 0, v(abs(v)<thr)=NaN; end
[~,ord] = sort(abs(v),'descend');
...
ord = ord(1:N);
vals = v(ord);
labels = fcAbbrevNames(R.names(ord),30);
cla(ax);
if isempty(vals), fcNoData(ax,'No seed connections pass the visual threshold / hemisphere filter.',C); return; end
barh(ax,flipud(vals(:)));
set(ax,'YTick',1:N,'YTickLabel',flipud(labels(:)));
grid(ax,'on');
mx = max(abs(vals)); if ~isfinite(mx) || mx<=0, mx=1; end
xlim(ax,[-mx mx]);
title(ax,sprintf('Seed profile: %g | %s | %s | %s',R.labels(seedIdx),R.names{seedIdx},titlePart,hemiMode), ...
    'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
xlabel(ax,'Connection strength');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',10);
end

function fcPlotPairCorrelationV15(ax,FC,R,reg1,reg2,activeGroup,dispMode,C)
idx1 = fcFindRegionIndexV15(R,reg1);
idx2 = fcFindRegionIndexV15(R,reg2);
[M,~,titlePart,ok,msg] = fcGetMatrixForGroupV15(R,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
val = M(idx1,idx2);
cla(ax); axis(ax,'off'); set(ax,'Color',C.axisBg);
lines = {};
lines{end+1} = 'Pair correlation';
lines{end+1} = '';
lines{end+1} = sprintf('Region 1: %g | %s',R.labels(idx1),R.names{idx1});
lines{end+1} = sprintf('Region 2: %g | %s',R.labels(idx2),R.names{idx2});
lines{end+1} = '';
lines{end+1} = sprintf('%s',titlePart);
lines{end+1} = sprintf('Value: %.4f',val);
lines{end+1} = '';
if strcmpi(strtrimSafe(dispMode),'Fisher z')
    lines{end+1} = 'Displayed value is Fisher z.';
else
    lines{end+1} = 'Displayed value is Pearson r.';
end
text(ax,0.05,0.92,sprintf('%s\n',lines{:}),'Units','normalized', ...
    'VerticalAlignment','top','Color',C.txt,'FontName','Consolas','FontSize',15,'Interpreter','none');
end

function fcPlotMaxConnectionsOneV15(ax,R,activeGroup,hemiMode,dispMode,thr,C)
[M,~,titlePart,ok,msg] = fcGetMatrixForGroupV15(R,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
[Mf,namesPlot,~,msg2] = fcApplyHemisphereModeV15(M,R.names,R.labels,hemiMode);
if isempty(Mf), fcNoData(ax,msg2,C); return; end
n = size(Mf,1); rows = {};
for i=1:n
    for j=i+1:n
        v = Mf(i,j);
        if isfinite(v) && abs(v) >= thr
            rows(end+1,:) = {abs(v),v,[fcShortNameV15(namesPlot{i}) '  <->  ' fcShortNameV15(namesPlot{j})]}; %#ok<AGROW>
        end
    end
end
if isempty(rows), fcNoData(ax,'No edges pass the visual threshold.',C); return; end
s = cell2mat(rows(:,1)); [~,ord]=sort(s,'descend'); ord=ord(1:min(35,numel(ord)));
vals = cell2mat(rows(ord,2)); labs = rows(ord,3);
barh(ax,flipud(vals(:)));
set(ax,'YTick',1:numel(vals),'YTickLabel',flipud(labs(:)));
grid(ax,'on');
title(ax,['Strongest connections | ' titlePart ' | ' hemiMode],'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
xlabel(ax,'Connection strength');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',9);
end

function fcPlotROITimeCourseOneV15(ax,FC,R,reg1,activeGroup,C)
seedIdx = fcFindRegionIndexV15(R,reg1);
label = R.labels(seedIdx);
[T,subNames,ok,msg] = fcCollectSeedTimeCoursesV15(FC,label,activeGroup,R);
if ~ok, fcNoData(ax,msg,C); return; end
cla(ax); hold(ax,'on');
hLines = plot(ax,T,'Color',[0.45 0.45 0.45]); %#ok<NASGU>
m = nanmean_local(T,2);
hMean = plot(ax,m,'LineWidth',3); %#ok<NASGU>
hold(ax,'off'); grid(ax,'on');
title(ax,sprintf('ROI time course: %g | %s | %s | %d frames | n=%d',label,R.names{seedIdx},activeGroup,size(T,1),size(T,2)), ...
    'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
xlabel(ax,'Frame stored in FC bundle'); ylabel(ax,'ROI signal from FC bundle');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',11);
try
    if numel(subNames) <= 10
        legend(ax,[subNames(:); {'group mean'}],'Interpreter','none','TextColor',C.txt,'Color',C.axisBg,'Location','bestoutside');
    else
        legend(ax,{'individual subjects','group mean'},'TextColor',C.txt,'Color',C.axisBg,'Location','best');
    end
catch
end
end

function fcPlotSubjectHeatmapOneV15(ax,FC,R,reg1,activeGroup,hemiMode,dispMode,thr,C)
seedIdx = fcFindRegionIndexV15(R,reg1);
label = R.labels(seedIdx);
[H,subNames,namesUse,ok,msg] = fcCollectSeedRowsV15(FC,R,label,activeGroup,hemiMode,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
if thr > 0, H(abs(H)<thr)=0; end
imagesc(ax,H);
colormap(ax,fcBlueWhiteRed(256));
if strcmpi(strtrimSafe(dispMode),'Fisher z'), caxis(ax,[-2.5 2.5]); else, caxis(ax,[-1 1]); end
cb=colorbar(ax); try, cb.Color=C.txt; catch, end
title(ax,sprintf('Subject heatmap: seed %g | %s | %s | %s',label,R.names{seedIdx},activeGroup,hemiMode), ...
    'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',9);
tickIdx = fcTickIdx(size(H,2));
set(ax,'XTick',tickIdx,'XTickLabel',fcAbbrevNames(namesUse(tickIdx),14),'YTick',1:numel(subNames),'YTickLabel',subNames);
try, xtickangle(ax,90); catch, end
xlabel(ax,'Target regions'); ylabel(ax,'Subjects');
end

function fcPlotRegionGraphOneV15(ax,R,reg1,activeGroup,hemiMode,dispMode,thr,C)
[M,~,titlePart,ok,msg] = fcGetMatrixForGroupV15(R,activeGroup,dispMode);
if ~ok, fcNoData(ax,msg,C); return; end
[Mf,namesPlot,labelsPlot,msg2] = fcApplyHemisphereModeV15(M,R.names,R.labels,hemiMode);
if isempty(Mf), fcNoData(ax,msg2,C); return; end
seedIdx0 = fcFindRegionIndexV15(R,reg1);
seedLabel = R.labels(seedIdx0);
seedIdx = find(double(labelsPlot)==double(seedLabel),1,'first');
if isempty(seedIdx), seedIdx = 1; end
v = Mf(seedIdx,:); v(seedIdx)=NaN;
if thr > 0, v(abs(v)<thr)=NaN; end
[~,ord]=sort(abs(v),'descend'); ord=ord(isfinite(v(ord))); ord=ord(1:min(20,numel(ord)));
nNodes = numel(ord)+1;
nodeIdx = [seedIdx ord(:)'];
...
text(ax,x(1)*1.10,y(1)*1.10,['SEED: ' fcShortNameV15(namesPlot{seedIdx})],'Color',C.txt,'FontWeight','bold','Interpreter','none');
for k=2:nNodes
    text(ax,x(k)*1.12,y(k)*1.12,fcShortNameV15(namesPlot{nodeIdx(k)}),'Color',C.txt,'FontSize',8,'Interpreter','none');
end
title(ax,['Region graph / network view | ' titlePart ' | ' hemiMode],'Color',C.txt,'Interpreter','none','FontSize',15,'FontWeight','bold');
hold(ax,'off');
end

function [M,climVal,titlePart,ok,msg] = fcGetMatrixForGroupV15(R,activeGroup,dispMode)
ok=true; msg='';
if nargin<3 || isempty(dispMode), dispMode='Pearson r'; end
if nargin<2 || isempty(activeGroup), activeGroup=R.groupA; end
useZ = strcmpi(strtrimSafe(dispMode),'Fisher z');
...
if isDiff
    if isfield(R,'singleGroup') && R.singleGroup
        ok=false; msg='Difference unavailable in single-subject/single-group mode.'; M=[]; climVal=[]; titlePart=''; return;
    end
    if useZ, M=R.diffZ; unitTxt='Fisher z'; else, M=R.diffR; unitTxt='Pearson r'; end
    climVal=[-1 1]; titlePart=[R.groupA ' - ' R.groupB ' (' unitTxt ')'];
elseif strcmpi(strtrimSafe(activeGroup),strtrimSafe(R.groupB))
    if isfield(R,'singleGroup') && R.singleGroup
        ok=false; msg='Group B unavailable in single-group mode.'; M=[]; climVal=[]; titlePart=''; return;
    end
    if useZ, M=R.meanZB; climVal=[-2.5 2.5]; unitTxt='Fisher z'; else, M=R.meanRB; climVal=[-1 1]; unitTxt='Pearson r'; end
    titlePart=[R.groupB ' mean FC (' unitTxt ')'];
else
...
function [Mf,namesF,labelsF,msg] = fcApplyHemisphereModeV15(M,names,labels,hemiMode)
msg='';
hemi = lower(strtrimSafe(hemiMode));
namesF = names; labelsF = labels; Mf = M;
if isempty(hemi) || contains(hemi,'all') || contains(hemi,'merged')
    return;
end
L = fcHemisphereMaskV15(names,'left');
R = fcHemisphereMaskV15(names,'right');
if contains(hemi,'left vs right')
    if ~any(L) || ~any(R), Mf=[]; namesF={}; labelsF=[]; msg='No left/right labels detected in region names.'; return; end
    Mf = M(L,R);
    namesF = names(R);
    labelsF = labels(R);
elseif contains(hemi,'left')
    if ~any(L), Mf=[]; namesF={}; labelsF=[]; msg='No left labels detected in region names.'; return; end
    Mf = M(L,L); namesF = names(L); labelsF = labels(L);
elseif contains(hemi,'right')
    if ~any(R), Mf=[]; namesF={}; labelsF=[]; msg='No right labels detected in region names.'; return; end
    Mf = M(R,R); namesF = names(R); labelsF = labels(R);
end
end

function keep = fcHemisphereKeepMaskV15(names,hemiMode)
hemi = lower(strtrimSafe(hemiMode));
if contains(hemi,'left') && ~contains(hemi,'left vs right')
    keep = fcHemisphereMaskV15(names,'left');
elseif contains(hemi,'right')
    keep = fcHemisphereMaskV15(names,'right');
else
    keep = true(numel(names),1);
end
end

function mask = fcHemisphereMaskV15(names,side)
n = numel(names); mask = false(n,1); side = lower(side);
for ii=1:n
    s = lower(strtrimSafe(names{ii}));
    if strcmp(side,'left')
        mask(ii) = ~isempty(regexp(s,'(^|[\s_\-\(\[])(l|left|lh)([\s_\-\)\]]|$)','once')) || contains(s,'_l_') || contains(s,' left') || contains(s,'(left');
    else
        mask(ii) = ~isempty(regexp(s,'(^|[\s_\-\(\[])(r|right|rh)([\s_\-\)\]]|$)','once')) || contains(s,'_r_') || contains(s,' right') || contains(s,'(right');
    end
end
end

function idx = fcFindRegionIndexV15(R,regionText)
...
s = lower(regionText);
s = regexprep(s,'^\s*[+-]?\d+\.?\d*\s*\|\s*','');
for ii=1:numel(R.names)
    if contains(lower(strtrimSafe(R.names{ii})),s), idx=ii; return; end
end
end

function [T,subNames,ok,msg] = fcCollectSeedTimeCoursesV15(FC,label,activeGroup,R)
T=[]; subNames={}; ok=false;
msg='No ROI time courses found. The FC bundle must contain meanTS / roiTS / timeCourses for this view.';
if ~isfield(FC,'subjects') || isempty(FC.subjects), return; end
for ii=1:numel(FC.subjects)
    s=FC.subjects(ii);
    g=strtrimSafe(s.group);
    if ~contains(lower(strtrimSafe(activeGroup)),'diff') && ~strcmpi(g,activeGroup), continue; end
    [X,labs]=fcGetSubjectTimeCourseMatrixV15(s);
    if isempty(X) || isempty(labs), continue; end
    hit=find(double(labs)==double(label),1,'first');
    if isempty(hit), continue; end
    if size(X,2)==numel(labs)
...
function [X,labs] = fcGetSubjectTimeCourseMatrixV15(s)
X=[]; labs=[];
try, labs=double(s.labels(:)); catch, labs=[]; end
fields={'meanTS','roiTS','timeCourses','roiTimeCourses','TS','tc'};
for ii=1:numel(fields)
...
function [H,subNames,namesUse,ok,msg] = fcCollectSeedRowsV15(FC,R,label,activeGroup,hemiMode,dispMode)
H=[]; subNames={}; namesUse=R.names; ok=false; msg='No subject FC matrices found.';
if ~isfield(FC,'subjects') || isempty(FC.subjects), return; end
keep = fcHemisphereKeepMaskV15(R.names,hemiMode);
if contains(lower(strtrimSafe(hemiMode)),'left vs right'), keep = fcHemisphereMaskV15(R.names,'right'); end
...
function s = fcShortNameV15(s)
s=strtrimSafe(s); if numel(s)>36, s=[s(1:33) '...']; end
end
...
writeFCMatrixCSV(fullfile(outRoot,'diffZ_GroupA_minus_GroupB.csv'),R.diffZ,names);
writeFCMatrixCSV(fullfile(outRoot,'p_values.csv'),R.pMat,names);
fcWriteSubjectMetadataCSV_V15(fullfile(outRoot,'subject_metadata_fc.csv'),S,R);
try
    FC=S.FC;
    reg1=''; reg2=''; hemiMode='All / merged'; activeGroup=R.groupA;
    try, reg1=S.fcRegion1; catch, end
    try, reg2=S.fcRegion2; catch, end
    try, hemiMode=S.fcHemiMode; catch, end
    views={'Matrix summary','Seed profile','Pair correlation','Max connections','ROI time course','Subject heatmap','Region graph'};
    groups={R.groupA};
    if ~isfield(R,'singleGroup') || ~R.singleGroup, groups={R.groupA,R.groupB,'Difference A-B'}; end
    f=figure('Visible','off','Color',C.bg,'InvertHardcopy','off','Position',[100 100 1600 1050]);
    ax=axes('Parent',f,'Position',[0.08 0.10 0.82 0.80]);
    for gi=1:numel(groups)
        activeGroup=groups{gi};
        for vi=1:numel(views)
            cla(ax);
            fcPlotFCAdvancedOneViewV15(ax,FC,R,reg1,reg2,views{vi},activeGroup,hemiMode,'Pearson r',0,C);
            print(f,fullfile(outRoot,[sanitizeFilename(activeGroup) '_' sanitizeFilename(views{vi}) '.png']),'-dpng','-r220');
        end
    end
    close(f);
catch
...
function fcWriteSubjectMetadataCSV_V15(outFile,S,R)
fid=fopen(outFile,'w'); if fid<0, error('Could not write %s',outFile); end
cleanup=onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'subject,group,condition,rowIndex,rowSide_or_hemi,sourceFile,roiSet,roiMode,hemisphere,hemiMode,side,selectionMode,mergedHemisphere,nLabels,nSlices,isStepMotor3D,analysisDir\n');
FC=struct(); try, FC=S.FC; catch, end
if ~isfield(FC,'subjects'), return; end
for i=1:numel(FC.subjects)
    s=FC.subjects(i); rowIndex=NaN; rowSide='';
    try, rowIndex=s.rowIndex; catch, end
    try
        if isfinite(rowIndex) && isfield(S,'rowPacapSide') && numel(S.rowPacapSide)>=rowIndex
            rowSide=strtrimSafe(S.rowPacapSide{rowIndex});
    ...
    fprintf(fid,'%s,%s,%s,%.0f,%s,%s,%s,%s,%s,%s,%s,%s,%s,%d,%s,%s,%s\n', ...
        csvEscapeFC(fcGetMetaStringV15(s,'name')),csvEscapeFC(fcGetMetaStringV15(s,'group')),csvEscapeFC(fcGetMetaStringV15(s,'condition')),rowIndex, ...
        csvEscapeFC(rowSide),csvEscapeFC(fcGetMetaStringV15(s,'sourceFile')),csvEscapeFC(fcGetMetaStringV15(s,'roiSet')),csvEscapeFC(fcGetMetaStringV15(s,'roiMode')), ...
        csvEscapeFC(fcGetMetaStringV15(s,'hemisphere')),csvEscapeFC(fcGetMetaStringV15(s,'hemiMode')),csvEscapeFC(fcGetMetaStringV15(s,'side')), ...
        csvEscapeFC(fcGetMetaStringV15(s,'selectionMode')),csvEscapeFC(fcGetMetaStringV15(s,'mergedHemisphere')),numel(fcGetLabelsV15(s)), ...
        csvEscapeFC(fcGetMetaStringV15(s,'nSlices')),csvEscapeFC(fcGetMetaStringV15(s,'isStepMotor3D')),csvEscapeFC(fcGetMetaStringV15(s,'analysisDir')));
end
end

function labels=fcGetLabelsV15(s)
labels=[]; try, labels=s.labels(:); catch, end
end

function v=fcGetMetaStringV15(s,fieldName)
v='';
try
    if isstruct(s) && isfield(s,fieldName) && ~isempty(s.(fieldName))
        x=s.(fieldName);
        if ischar(x), v=x; elseif isnumeric(x)||islogical(x), v=mat2str(x); else, v=class(x); end
    end
catch
end
end
...
% Clean single FC export function inserted by V16 duplicate-fix patch.
S = [];
if nargin >= 1 && isstruct(varargin{1})
    S = varargin{1};
end
if isempty(S) || ~isstruct(S)
    error('Missing GroupAnalysis state struct for FC export.');
end
if ~isfield(S,'lastFC') || isempty(fieldnames(S.lastFC))
    error('Compute FC first.');
end
R = S.lastFC;

startDir = pwd;
try
    if isfield(S,'outDir') && exist(S.outDir,'dir') == 7
        startDir = S.outDir;
    end
catch
end

outDir = uigetdir(startDir,'Select folder for FC export');
...
try, writeFCMatrixCSV(fullfile(outRoot,'meanZA_FisherZ_GroupA.csv'),R.meanZA,names); catch ME, warning(ME.message); end
try, writeFCMatrixCSV(fullfile(outRoot,'meanZB_FisherZ_GroupB.csv'),R.meanZB,names); catch ME, warning(ME.message); end
try, writeFCMatrixCSV(fullfile(outRoot,'diffZ_GroupA_minus_GroupB.csv'),R.diffZ,names); catch ME, warning(ME.message); end
...
try
    ...
    warning(['Could not export FC subject metadata: ' ME.message]);
end

% Export current/standard figures if the plotting helper exists.
try
    C = fcDefaultColorsV15();
catch
    C = struct();
    C.bg = [0.04 0.04 0.04];
    C.axisBg = [0.02 0.02 0.02];
...
try
    ...
    for gi = 1:numel(groups)
        for vi = 1:numel(views)
            cla(ax);
            try
                fcPlotFCAdvancedOneViewV15(ax,FC,R,reg1,reg2,views{vi},groups{gi},hemiMode,'Pearson r',0,C);
                print(f,fullfile(outRoot,[sanitizeFilename(groups{gi}) '_' sanitizeFilename(views{vi}) '.png']),'-dpng','-r220');
            catch MEplot
                warning(['Could not export FC view ' views{vi} ': ' MEplot.message]);
            end
        end
    end
    close(f);
    ...
    try, close(f); catch, end
    warning(['Could not export FC figures: ' ME.message]);
end

FC = struct();
try, FC = S.FC; catch, end
try
    save(fullfile(outRoot,'FC_GroupAnalysis_Results.mat'),'R','FC','names','-v7.3');
catch ME
    warning(['Could not save FC_GroupAnalysis_Results.mat: ' ME.message]);
end
...
function s = fcGetFieldCharCompatV14(S,fieldName,fb)
% Safe field-to-char helper used by FC bundle loading/export patches.
s = fb;
try
    if isstruct(S) && isfield(S,fieldName) && ~isempty(S.(fieldName))
        x = S.(fieldName);
...
function v = fcGetFieldNumCompatV14(S,fieldName,fb)
% Safe field-to-number helper used by FC bundle loading/export patches.
v = fb;
try
    if isstruct(S) && isfield(S,fieldName) && ~isempty(S.(fieldName))
        x = S.(fieldName);
        if isnumeric(x) || islogical(x)
            ...
            v = x(1);
        elseif ischar(x)
            tmp = str2double(strtrim(x));
            if isfinite(tmp), v = tmp; end
        elseif iscell(x) && ~isempty(x)
            tmp = str2double(strtrim(char(x{1})));
            if isfinite(tmp), v = tmp; end
        end
    end
catch
