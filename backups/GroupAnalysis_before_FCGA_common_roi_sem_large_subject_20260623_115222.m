...
% FC-GA preview update with correct Group mean vs selected-animal logic.
...
% Use compact/top ROI selection if available, otherwise all ROIs.
selNote = 'all ROIs';
selIdx = 1:size(M0,1);
try
    [tmpSel,tmpNote] = fcGASelectedOrTopROIIdx_20260622(S,Rplot,M0,seedIdx0,roi2Idx0,20);
    if ~isempty(tmpSel)
        selIdx = tmpSel(:)';
        selNote = tmpNote;
    end
...
switch lower(viewMode)
    ...
    case {'roi pair summary','roi pair'}
        plotROIPairSummary_ADV_20260617(S.axFCA,stackSmall,seedSmall,roi2Small,Rsmall,valTxt,S.C,S,isIndividual,subjectNote);
...
if isIndividual || numel(vals) == 1
    plot(ax,1,vals(1),'o','MarkerSize',10,'LineWidth',2.0,'Color',lineCol,'MarkerFaceColor',lineCol);
    nTxt = 1;
else
    good = vals(isfinite(vals));
    nTxt = numel(good);
    if isempty(good), good = NaN; end
    mu = mean(good,'omitnan');
    if isnan(mu), mu = nanmean_local_ADV_20260617(good); end
    sd = std(good,'omitnan');
    if isnan(sd), sd = nanstd_local_ADV_20260617(good,0,1); end
    sem = sd ./ sqrt(max(numel(good),1));
    if numel(vals) == 1, xj = 1; else, xj = 1 + linspace(-0.11,0.11,numel(vals)); end
    plot(ax,xj,vals,'o','MarkerSize',7,'LineWidth',1.5,'Color',lineCol,'MarkerFaceColor',lineCol);
    plot(ax,[0.78 1.22],[mu mu],'-','LineWidth',3.0,'Color',lineCol);
    if isfinite(sem)
        plot(ax,[1 1],[mu-sem mu+sem],'-','LineWidth',2.0,'Color',lineCol);
        plot(ax,[0.94 1.06],[mu-sem mu-sem],'-','LineWidth',2.0,'Color',lineCol);
        plot(ax,[0.94 1.06],[mu+sem mu+sem],'-','LineWidth',2.0,'Color',lineCol);
    end
end
hold(ax,'off');
set(ax,'Color',C.axisBg,'XColor',C.txt,'YColor',C.txt,'FontSize',11);
grid(ax,'on'); box(ax,'on'); xlim(ax,[0.55 1.45]);
set(ax,'XTick',1,'XTickLabel',{'Selected pair'});
fcGAApplyValueAxis_20260624(ax,S,valTxt);
seedName = fcNiceName_SAFE_20260617(R.names{seedIdx},R.labels(seedIdx),'Abbrev',false);
...
n = numel(vals); x = 1:n;
subjLabs = fcGASubjectLabelsFromR_20260624(R);
lineCol = fcGAGetPlotColor_20260624(S,[0.10 0.45 0.95]);
cols = lines(max(n,1));
hold(ax,'on');
for ii = 1:n
    ...
    plot(ax,x(ii),vals(ii),'o','Color',cols(ii,:),'MarkerFaceColor',cols(ii,:),'MarkerSize',ms,'LineWidth',lw, ...
        'DisplayName',subjLabs{ii});
end
good = vals(isfinite(vals));
if ~isempty(good)
    mu = mean(good);
    plot(ax,[0.75 n+0.25],[mu mu],'-','LineWidth',2.2,'Color',lineCol,'DisplayName','Mean');
end
hold(ax,'off');
set(ax,'Color',C.axisBg,'XColor',C.txt,'YColor',C.txt,'FontSize',10);
grid(ax,'on'); box(ax,'on'); xlim(ax,[0.5 max(1,n+0.5)]);
fcGAApplyValueAxis_20260624(ax,S,valTxt);
seedName = fcNiceName_SAFE_20260617(R.names{seedIdx},R.labels(seedIdx),'Abbrev',false);
roiName  = fcNiceName_SAFE_20260617(R.names{roi2Idx},R.labels(roi2Idx),'Abbrev',false);
title(ax,sprintf('Animal pair values: %s ↔ %s | n=%d',seedName,roiName,n), ...
    'Color',C.txt,'FontWeight','bold','Interpreter','none');
ylabel(ax,fcGAValueLabel_20260624(valTxt),'Color',C.txt,'Interpreter','none');
xlabel(ax,'Animal / scan', 'Color',C.txt,'Interpreter','none');
shortLabs = fcGATickLabelsCompact_20260624(subjLabs,18);
set(ax,'XTick',x,'XTickLabel',shortLabs);
...
labs = cell(max(n,1),1);
for ii = 1:numel(labs)
    labs{ii} = fcGASubjectDisplayName_20260624(R,ii);
end
end

function labs = fcGATickLabelsCompact_20260624(labs,maxN)
if nargin < 2, maxN = 18; end
try
    for ii = 1:numel(labs)
        s = labs{ii};
        parts = regexp(s,'\s*\|\s*','split');
        if numel(parts) >= 2, s = strtrim(parts{2}); end
        if numel(parts) >= 4, s = [s '_' strtrim(parts{4})]; end
