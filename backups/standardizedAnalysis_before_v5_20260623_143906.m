...
function state = getStdState()
    fig0 = gcbf;
    if isempty(fig0) || ~ishandle(fig0)
        fig0 = findobj(0,'Type','figure','Tag','deconfusion_standardized_analysis_v4');
        if isempty(fig0)
            fig0 = findobj(0,'Type','figure','Tag','deconfusion_standardized_analysis_v3');
        end
        if isempty(fig0)
            state = [];
            return;
