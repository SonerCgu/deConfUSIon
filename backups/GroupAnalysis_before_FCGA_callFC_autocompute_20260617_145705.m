...
pFCAx = uipanel(pFCBG,'Units','normalized','Position',[0.02 0.065 0.96 0.755], ...
    'Title','FC matrices', ...
    'BackgroundColor',bg2,'ForegroundColor','w','FontWeight','bold');
...
drawnow;
        try
            ...
            try, refreshFCRegionPopups_SINGLE_20260616(hFig); catch, end
            try, refreshFCSubjectPopup_SINGLE_20260616(hFig); catch, end
    ...
    function updateFCTabPreview(~,~)
        ...
        if ~isfield(S0,'lastFC') || isempty(fieldnames(S0.lastFC))
            try
                ...
                fcNoDataLocal(S0.axFCA,'Compute FC first',S0.C);
            catch
            end
            return;
        end
        try
            updateFCTabPreview_SINGLE_20260616(S0);
        catch ME
            try, GA_printErrorLocal(ME,'caught error in single FC preview'); catch, end
            try, set(S0.hFCInfo,'String',['FC preview failed: ' ME.message]); catch, end
        end
    end
...
G = callFC('alignFCSubjectsToCommonROIs',S.FC);
groupSel = 'All loaded';
