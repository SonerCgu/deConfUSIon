...
set(ax,'Color',C.axisBg,'XColor',C.txt,'YColor',C.txt,'YTickMode','auto','YTickLabelMode','auto'); grid(ax,'on');
seedName = fcNiceName_SAFE_20260617(R.names{seedIdx},R.labels(seedIdx),'Full',false);
title(ax,sprintf('Seed profile: seed [%s] → displayed target ROIs | mean ± SD | n=%d',seedName,size(stack,3)),'Color',C.txt,'FontWeight','bold','Interpreter','none');
xlabel(ax,'Target ROIs: Names-table selection or auto seed +10/-10','Color',C.txt,'Interpreter','none');
...
if strcmpi(labelMode,'Full')
    if isempty(full), base = fcFullName_SAFE_20260617(acr); else, base = full; end
else
    base = upper(strtrimSafe(acr));
end
...
maxN = 11; if strcmpi(labelMode,'Full'), maxN = 34; end
...
if ~isempty(strfind(s,'||'))
    ...
    acr = strtrimSafe(parts{1});
    full = strtrimSafe(parts{min(2,numel(parts))});
...
if isfield(S,'fcSelectedROIIdx') && ~isempty(S.fcSelectedROIIdx), sel=fc_region_keep_indices_GA_20260622(S.fcSelectedROIIdx,n); note=sprintf('manual %d ROI(s)',numel(sel)); return; end
row=double(M(seedIdx,:)); row(seedIdx)=NaN; [~,posOrd]=sort(row,'descend'); [~,negOrd]=sort(row,'ascend'); posOrd=posOrd(isfinite(row(posOrd))); negOrd=negOrd(isfinite(row(negOrd))); posSel=posOrd(1:min(nHalf,numel(posOrd))); negSel=negOrd(1:min(nHalf,numel(negOrd))); sel=unique([seedIdx;roi2Idx;posSel(:);negSel(:)],'stable'); sel=sel(sel>=1&sel<=n); note=sprintf('seed top +%d / -%d ROI(s)',numel(posSel),numel(negSel));
...
f=figure('Name','FC-GA large interactive view','Color',[0.08 0.08 0.08],'NumberTitle','off','MenuBar','none','ToolBar','figure','Position',[40 35 1580 930]);
...
y=0.890; hView=uicontrol(f,'Style','popupmenu','String',viewItems,'Value',get(S.hFCView,'Value'),'Units','normalized','Position',[0.835 y 0.140 0.040]);
y=y-0.055; hDisp=uicontrol(f,'Style','popupmenu','String',{'Pearson r','Fisher z'},'Value',max(1,min(2,get(S.hFCDisplay,'Value'))),'Units','normalized','Position',[0.835 y 0.140 0.040]);
y=y-0.055; hHemi=uicontrol(f,'Style','popupmenu','String',{'All','Left only','Right only','Merged L/R','Left vs Right','Right vs Left'},'Value',max(1,min(6,get(S.hFCHemi,'Value'))),'Units','normalized','Position',[0.835 y 0.140 0.040]);
y=y-0.055; hColor=uicontrol(f,'Style','popupmenu','String',{'Blue-White-Red','Blue-White','Red-White-Blue','parula','hot','jet','gray'},'Units','normalized','Position',[0.835 y 0.140 0.040]);
y=y-0.055; hLab=uicontrol(f,'Style','popupmenu','String',{'Abbrev','Full','Label ID'},'Value',max(1,min(3,get(S.hFCLabelMode,'Value'))),'Units','normalized','Position',[0.835 y 0.140 0.040]);
y=y-0.055; hROISet=uicontrol(f,'Style','popupmenu','String',{'Main selection/top','All ROIs','Seed +10/-10','Manual Names selection'},'Units','normalized','Position',[0.835 y 0.140 0.040]);
y=y-0.055; hSeed=uicontrol(f,'Style','popupmenu','String',seedItems,'Value',get(S.hFCRegion1,'Value'),'Units','normalized','Position',[0.835 y 0.140 0.040]);
y=y-0.055; hROI2=uicontrol(f,'Style','popupmenu','String',roiItems,'Value',get(S.hFCRegion2,'Value'),'Units','normalized','Position',[0.835 y 0.140 0.040]);
y=y-0.060; hAutoY=uicontrol(f,'Style','checkbox','String','Y auto','Value',1,'Units','normalized','Position',[0.835 y 0.070 0.035],'BackgroundColor',[0.08 0.08 0.08],'ForegroundColor',[1 1 1]); hY0=uicontrol(f,'Style','edit','String','0','Units','normalized','Position',[0.908 y 0.030 0.035]); hY1=uicontrol(f,'Style','edit','String','1.5','Units','normalized','Position',[0.942 y 0.030 0.035]);
uicontrol(f,'Style','pushbutton','String','Update','Units','normalized','Position',[0.835 0.245 0.065 0.045],'BackgroundColor',[0.10 0.45 0.95],'ForegroundColor','w','FontWeight','bold','Callback',@(src,evt)fcGALargeReplot_20260622(f));
uicontrol(f,'Style','pushbutton','String','Export PNG','Units','normalized','Position',[0.910 0.245 0.065 0.045],'BackgroundColor',[0.10 0.60 0.25],'ForegroundColor','w','FontWeight','bold','Callback',@(src,evt)fcExportLargeView_GA_20260622(f));
D=struct('mainFig',hFig,'ax',ax,'hView',hView,'hDisp',hDisp,'hHemi',hHemi,'hColor',hColor,'hLab',hLab,'hROISet',hROISet,'hSeed',hSeed,'hROI2',hROI2,'hAutoY',hAutoY,'hY0',hY0,'hY1',hY1); setappdata(f,'D',D);
set([hView hDisp hHemi hColor hLab hROISet hSeed hROI2 hAutoY hY0 hY1],'Callback',@(src,evt)fcGALargeReplot_20260622(f)); fcGALargeReplot_20260622(f);
...
seedOrig=max(1,min(get(D.hSeed,'Value'),numel(Rfull.labels))); roi2Orig=max(1,min(get(D.hROI2,'Value'),numel(Rfull.labels)));
try, set(S.hFCRegion1,'Value',seedOrig); set(S.hFCRegion2,'Value',roi2Orig); catch, end
if strcmpi(dispMode,'Fisher z'), M0full=Rfull.meanZ; stack0full=Rfull.Zstack; clim=[-2.5 2.5]; valTxt='Fisher z'; else, M0full=Rfull.meanR; stack0full=Rfull.Rstack; clim=[-1 1]; valTxt='Pearson r'; end
switch lower(roiMode), case 'all rois', selIdx=1:numel(Rfull.labels); selNote='all ROIs'; case 'manual names selection', selIdx=fc_region_keep_indices_GA_20260622(getfield(S,'fcSelectedROIIdx'),numel(Rfull.labels)); if isempty(selIdx), selIdx=1:numel(Rfull.labels); end; selNote=sprintf('manual %d ROI(s)',numel(selIdx)); otherwise, [selIdx,selNote]=fcGASelectedOrTopROIIdx_20260622(S,Rfull,M0full,seedOrig,roi2Orig,20); if isempty(selIdx), selIdx=1:numel(Rfull.labels); end; end
...
otherwise, plotROIOverlay_ADV_20260617(D.ax,S,Rfull,cmapName);
end
...
try, ax=ancestor(src,'axes'); fig=ancestor(ax,'figure'); S=guidata(fig); if isempty(S), fig=getappdata(ancestor(ax,'figure'),'mainFig'); S=guidata(fig); end; labelSlice=getappdata(ax,'fcGAOverlayLabelSlice'); labs=getappdata(ax,'fcGAOverlayLabels'); cp=get(ax,'CurrentPoint'); x=round(cp(1,1)); y=round(cp(1,2)); if y<1||x<1||y>size(labelSlice,1)||x>size(labelSlice,2), return; end; lab=double(labelSlice(y,x)); if lab==0||~isfinite(lab), return; end; idx=find(labs==lab,1,'first'); if isempty(idx), idx=find(abs(labs)==abs(lab),1,'first'); end; if isempty(idx), return; end; if isfield(S,'hFCRegion1')&&ishghandle(S.hFCRegion1), set(S.hFCRegion1,'Value',idx); guidata(ancestor(S.hFCRegion1,'figure'),S); updateFCTabPreview_ADV_20260617(S); end; catch, end
end
