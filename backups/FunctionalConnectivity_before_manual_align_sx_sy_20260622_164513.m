...
try
    ...
    if exist('pSeedDisplayFinal','var') && ishghandle(pSeedDisplayFinal)
        uicontrol('Parent',pSeedDisplayFinal,'Style','pushbutton','Units','normalized', ...
            'Position',[0.160 0.035 0.270 0.060], ...
            'String','Warp labels','Tag','FC_WarpLabelsButton_20260622', ...
            'BackgroundColor',C.orange,'ForegroundColor','w', ...
            'FontName',C.font,'FontWeight','bold','FontSize',7.0, ...
...
try
    ...
    if exist('pSeedDisplayFinal','var') && ishghandle(pSeedDisplayFinal)
        uicontrol('Parent',pSeedDisplayFinal,'Style','pushbutton','Units','normalized', ...
            'Position',[0.500 0.035 0.310 0.060], ...
...
f = figure('Name','Manual ROI-label alignment', ...
    ...
    'Position',[60 40 1380 900]);

ax = axes('Parent',f,'Units','normalized','Position',[0.035 0.075 0.705 0.880]);
...
Ushow = fc_manual_contrast_20260622(D.U);
axes(D.ax); cla(D.ax);
hImg = imagesc(D.ax,Ushow,[0 1]);
colormap(D.ax,gray(256));
axis(D.ax,'image'); axis(D.ax,'ij');
xlim(D.ax,[1 size(Ushow,2)]); ylim(D.ax,[1 size(Ushow,1)]);
set(D.ax,'Color',[0 0 0],'XTick',[],'YTick',[]);
hold(D.ax,'on');
[x,y] = fc_manual_align_boundary_xy_20260622(A2);
hPts = plot(D.ax,x,y,'.','Color',[1 1 1],'MarkerSize',4);
hold(D.ax,'off');
...
setappdata(f,'dragStartPoint',[]);
setappdata(f,'dragStartDxDy',[]);
try
    set(hImg,'ButtonDownFcn',@(src,evt)fc_manual_align_drag_start_20260622(f));
    set(hPts,'ButtonDownFcn',@(src,evt)fc_manual_align_drag_start_20260622(f));
    set(D.ax,'ButtonDownFcn',@(src,evt)fc_manual_align_drag_start_20260622(f));
    set(f,'WindowButtonMotionFcn',@(src,evt)fc_manual_align_drag_move_20260622(f));
    set(f,'WindowButtonUpFcn',@(src,evt)fc_manual_align_drag_stop_20260622(f));
catch
end
end
...
v = U(:); v = v(isfinite(v));
if isempty(v)
    Ushow = zeros(size(U)); return;
end
v = sort(v);
n = numel(v);
lo = v(max(1,round(0.01*n)));
hi = v(max(1,round(0.995*n)));
if hi <= lo
    lo = min(v); hi = max(v);
end
if hi <= lo
...
try
    D = getappdata(f,'D');
    cp = get(D.ax,'CurrentPoint');
    [dx,dy,~,~] = fc_manual_align_params_20260622(D);
    setappdata(f,'dragStartPoint',cp(1,1:2));
    setappdata(f,'dragStartDxDy',[dx dy]);
catch
end
end
...
try
    D = getappdata(f,'D');
    p0 = getappdata(f,'dragStartPoint');
    d0 = getappdata(f,'dragStartDxDy');
    if isempty(p0) || isempty(d0), return; end
    cp = get(D.ax,'CurrentPoint');
    dp = cp(1,1:2) - p0;
    set(D.hDx,'String',sprintf('%.1f',d0(1)+dp(1)));
    set(D.hDy,'String',sprintf('%.1f',d0(2)+dp(2)));
    fc_manual_align_update_20260622(f);
catch
end
end
