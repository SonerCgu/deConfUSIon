...
% TRUE_LR_DISPLAY_FIX_20260623
% All              = full matrix
% Merged L+R       = L/R homologues collapsed by base region
% Left only        = left-left matrix
% Right only       = right-right matrix
% Left vs Right    = left rows vs right columns
% Important: never silently show the full matrix for Left/Right if no L/R labels exist.
try
    n = size(M,1);
    ...
    if ~isempty(strfind(mode,'left vs right')) || ~isempty(strfind(mode,'left-right')) || ~isempty(strfind(mode,'l vs r')) || ~isempty(strfind(mode,'l/r'))
        if any(isL) && any(isR)
            ...
            M2 = NaN(1,1);
            names2 = {'NO_TRUE_LR_LABELS'};
            labels2 = 0;
            rowIdx = [];
            colIdx = [];
            hemiTitle = 'Left vs Right - no true L/R labels in bundle';
            return;
        end
    end
    ...
    if strcmp(mode,'left') || strcmp(mode,'l') || ~isempty(strfind(mode,'left only')) || ~isempty(strfind(mode,'left hemi')) || ~isempty(strfind(mode,'left hemisphere'))
        if any(isL)
            ...
            M2 = NaN(1,1);
            names2 = {'NO_LEFT_LABELS'};
            labels2 = 0;
            rowIdx = [];
            colIdx = [];
            hemiTitle = 'Left hemisphere - no true left labels in bundle';
            return;
        end
    end
    ...
    if strcmp(mode,'right') || strcmp(mode,'r') || ~isempty(strfind(mode,'right only')) || ~isempty(strfind(mode,'right hemi')) || ~isempty(strfind(mode,'right hemisphere'))
        if any(isR)
            ...
            M2 = NaN(1,1);
            names2 = {'NO_RIGHT_LABELS'};
            labels2 = 0;
            rowIdx = [];
            colIdx = [];
            hemiTitle = 'Right hemisphere - no true right labels in bundle';
            return;
        end
    end
catch ME
...
% Heatmap labels. Strip side prefixes whenever the selected view already defines the side.
if nargin < 4, hemiTitle = ''; end
n = numel(labels);
ht = lower(strtrim(char(hemiTitle)));
stripSide = ~isempty(strfind(ht,'merged')) || ~isempty(strfind(ht,'bilateral')) || ...
    ~isempty(strfind(ht,'left hemisphere')) || ~isempty(strfind(ht,'right hemisphere')) || ...
