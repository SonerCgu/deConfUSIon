...
try
    ...
    try
        S.hFCZoom = mkBtn(pFCTop,'Large',[0.900 0.020 0.065 0.105],C.btnAction,@(~,~) showFCLargeView_GA_20260622(hFig));
        set(S.hFCZoom,'TooltipString','Open current FC-GA view in a large exportable window');
    catch
        try, S.hFCZoom = mkBtn(pFCTop,'Large',[0.900 0.020 0.065 0.105],C.btnAction,@(~,~) showFCLargeView_GA_20260622(hFig)); catch, end
    end
...
maxTicks = 32; if strcmpi(strtrimSafe(labelMode),'Full'), maxTicks = 18; end; try, if isappdata(ax,'FCGA_FORCE_ALL_TICKS'), maxTicks = max(size(M)); end, catch, end
...
set(ax,'Color',S.C.axisBg); axis(ax,'image'); axis(ax,'ij'); axis(ax,'off'); xlim(ax,[1 size(valMap,2)]); ylim(ax,[1 size(valMap,1)]);
...
try, set(ax,'UserData',struct('sliceUpdated',true)); catch, end
end
...
try
    for x = 0.5:1:(nC+0.5)
        line(ax,[x x],[0.5 nR+0.5],'Color',[0 0 0],'LineWidth',0.55,'HitTest','off');
    end
    for y = 0.5:1:(nR+0.5)
        line(ax,[0.5 nC+0.5],[y y],'Color',[0 0 0],'LineWidth',0.55,'HitTest','off');
    end
    for x = 0.5:5:(nC+0.5)
        line(ax,[x x],[0.5 nR+0.5],'Color',[0 0 0],'LineWidth',1.50,'HitTest','off');
    end
    for y = 0.5:5:(nR+0.5)
        line(ax,[0.5 nC+0.5],[y y],'Color',[0 0 0],'LineWidth',1.50,'HitTest','off');
    end
catch
end
end
...
try
    S = guidata(hFig);
    if isempty(S) || ~isfield(S,'axFCA') || ~ishghandle(S.axFCA), return; end
    f = figure('Name','FC-GA large view','Color',[0.08 0.08 0.08], ...
        'NumberTitle','off','MenuBar','none','ToolBar','figure', ...
        'Position',[50 40 1450 920]);
    axNew = axes('Parent',f,'Units','normalized','Position',[0.055 0.095 0.840 0.840]);
    try, setappdata(axNew,'FCGA_FORCE_ALL_TICKS',true); catch, end
    oldFig = ancestor(S.axFCA,'figure');
    oldUnits = get(S.axFCA,'Units');
    set(S.axFCA,'Units','normalized');
    ch = copyobj(allchild(S.axFCA),axNew); %#ok<NASGU>
    try, colormap(axNew,colormap(S.axFCA)); catch, end
    try, set(axNew,'CLim',get(S.axFCA,'CLim')); catch, end
    try, title(axNew,get(get(S.axFCA,'Title'),'String'),'Color',[1 1 1],'Interpreter','none','FontWeight','bold'); catch, end
    try, xlabel(axNew,get(get(S.axFCA,'XLabel'),'String'),'Color',[1 1 1]); catch, end
    try, ylabel(axNew,get(get(S.axFCA,'YLabel'),'String'),'Color',[1 1 1]); catch, end
    set(axNew,'Color',get(S.axFCA,'Color'),'XColor',[1 1 1],'YColor',[1 1 1]);
    try, set(S.axFCA,'Units',oldUnits); catch, end
    uicontrol(f,'Style','pushbutton','String','Export PNG','Units','normalized', ...
        'Position',[0.910 0.880 0.075 0.045],'BackgroundColor',[0.10 0.45 0.95], ...
        'ForegroundColor','w','FontWeight','bold', ...
        'Callback',@(src,evt)fcExportLargeView_GA_20260622(f));
    uicontrol(f,'Style','text','String','Large view mirrors current selected FC-GA view. Change view in main panel, then press Large again.','Units','normalized', ...
        'Position',[0.905 0.730 0.085 0.120],'BackgroundColor',[0.08 0.08 0.08], ...
        'ForegroundColor',[0.88 0.88 0.88],'HorizontalAlignment','left');
catch ME
...
try
    [fn,pn] = uiputfile({'*.png','PNG image (*.png)'},'Export FC-GA large view',fullfile(pwd,'FCGA_large_view.png'));
