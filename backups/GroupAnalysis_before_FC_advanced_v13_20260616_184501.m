...
S.hFCLoadAtlas = mkBtn(pFCTop,'Atlas TXT',[0.015 0.10 0.145 0.34],C.btnSecondary,@onLoadFCAtlasLabels);
uicontrol(pFCTop,'Style','text','String','Seed:', ...
    'Units','normalized','Position',[0.175 0.16 0.045 0.25], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCSeedRegion = uicontrol(pFCTop,'Style','edit','String','', ...
    'Units','normalized','Position',[0.220 0.14 0.085 0.28], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w');
...
%%% =====================================================================
    ...
    function onSaveList(~,~)
        ...
        fcRowFiles = localGA_getFCRowFilesV12(S0);
        FC = struct();
        lastFC = struct();
        fcAtlasLabelFile = '';
        fcSeedRegion = '';
        ...
        S0.opt.startDir = p;
        guidata(hFig,S0);
        setStatusText('Saved list including FC row files and FC data.');
    end
    ...
    function onLoadList(~,~)
        ...
        if isfield(L,'FC') && isstruct(L.FC)
            S0.FC = L.FC;
            try, S0.FC.loaded = isfield(S0.FC,'subjects') && ~isempty(S0.FC.subjects); catch, end
        else
            S0.FC = struct('files',{{}},'subjects',struct([]),'loaded',false);
        end
        ...
        setStatusText('Loaded list including FC row files and FC data.');
    end
    ...
    function loadFCFileListIntoState(fileList)
        ...
        try
            ...
            if ~isempty(selectedRows)
                ...
                if numel(selectedRows) ~= numel(fileList)
                    try
                        warndlg(sprintf(['Selected rows: %d | FC files: %d\n\n' ...
                            'Only the first %d pair(s) will be assigned one-to-one.'], ...
                            numel(selectedRows),numel(fileList),nAssign),'FC row assignment');
                    catch
                    end
                end
                for ii = 1:nAssign
                ...
                [FC,cacheOut] = localGA_loadFCFilesV12(fileList,S0.cache);
                S0.cache = cacheOut;
                S0.FC = FC;
                S0.FC.loaded = true;
                S0 = localGA_autoAttachFCFilesToRowsV12(S0,FC);
                msg = sprintf('Loaded %d FC subject(s) from %d file(s). No row selected, so files were auto-matched where possible.',FC.nSubjects,numel(fileList));
            end
    ...
    function onComputeGroupFC(~,~)
        ...
        try
            ...
            if ~isfield(S0,'FC') || ~isfield(S0.FC,'loaded') || ~S0.FC.loaded
                errordlg(['No active table rows with existing FC files found. ' MEbuild.message],'Functional Connectivity');
                return;
            end
        end
        if ~isfield(S0,'FC') || ~isfield(S0.FC,'loaded') || ~S0.FC.loaded
            errordlg('Load or assign FC_GroupBundle files first.','Functional Connectivity');
            return;
        end
        ...
        setStatus(false);
        setStatusText('Computing group FC from table-assigned FC rows...');
        drawnow;
        try
            ...
            updateManualTabs();
            updateFCTabPreview();
            setStatusText(sprintf('Functional Connectivity complete: %s n=%d vs %s n=%d.',R.groupA,R.nA,R.groupB,R.nB));
        catch ME
            try, GA_printErrorLocal(ME,'caught error in GroupAnalysis.m'); catch, end
...
for i = 1:n
    ...
    V{i,9} = fcBundlePresenceLabelV12(fcRowFiles{i});
    V{i,10} = deriveRowStatusWithFCV12(subj(i,:),fcRowFiles{i});
end
end
...
function S = ensureFCRowFilesSizeV12(S)
n = size(S.subj,1);
...
for ii = 1:n
    S.fcRowFiles{ii,1} = strtrimSafe(S.fcRowFiles{ii,1});
end
end

function fcRowFiles = localGA_getFCRowFilesV12(S)
try
    S = ensureFCRowFilesSizeV12(S);
    fcRowFiles = S.fcRowFiles;
catch
    fcRowFiles = cell(size(S.subj,1),1);
end
end

function s = fcBundlePresenceLabelV12(fp)
fp = strtrimSafe(fp);
if isempty(fp)
    s = '';
...
function s = deriveRowStatusWithFCV12(row,fcFile)
...
elseif ~use
    s = 'Not used';
elseif ~isempty(fcFile) && exist(fcFile,'file') == 2
    s = 'FC OK';
elseif ~isempty(strtrimSafe(row{7})) || ~isempty(strtrimSafe(row{8}))
    s = 'OK';
else
    s = 'Not set';
end
...
function [FC,cache] = localGA_loadFCFilesV12(fileList,cache)
[FC,cache] = GroupAnalysis_FC('loadFCGroupBundlesFromFiles',fileList,cache);
end

function S = localGA_buildFCFromTableRowsV12(S)
S = ensureFCRowFilesSizeV12(S);
rows = [];
files = {};
for r = 1:size(S.subj,1)
...
if isempty(rows)
    error('No active rows with existing FC files.');
end
FCout = struct();
FCout.files = unique(files,'stable');
FCout.subjects = struct([]);
FCout.nSubjects = 0;
cache = S.cache;
for ii = 1:numel(rows)
    r = rows(ii);
    fp = files{ii};
    [FCtmp,cache] = localGA_loadFCFilesV12({fp},cache);
    if ~isfield(FCtmp,'subjects') || isempty(FCtmp.subjects)
        continue;
    end
    pick = localGA_pickFCSubjectForRowV12(FCtmp.subjects,S.subj(r,:),fp);
    subj = FCtmp.subjects(pick);
    animal = strtrimSafe(S.subj{r,2});
    if isempty(animal) || strcmpi(animal,'N/A'), animal = guessSubjectID(fp); end
    subj.name = animal;
    subj.animalID = animal;
    subj.group = strtrimSafe(S.subj{r,3});
    subj.condition = strtrimSafe(S.subj{r,4});
    subj.rowIndex = r;
    subj.sourceFile = fp;
    FCout.nSubjects = FCout.nSubjects + 1;
    if FCout.nSubjects == 1
        FCout.subjects = subj;
    else
        FCout.subjects(FCout.nSubjects) = subj;
    end
end
if FCout.nSubjects < 1
    error('FC files were found but no valid FC subject could be read.');
end
FCout.loaded = true;
S.cache = cache;
S.FC = FCout;
...
function pick = localGA_pickFCSubjectForRowV12(subjects,row,fp)
pick = 1;
animal = lower(strtrimSafe(row{2}));
...
function S = localGA_autoAttachFCFilesToRowsV12(S,FC)
S = ensureFCRowFilesSizeV12(S);
...
for ii = 1:numel(FC.subjects)
    ...
    hit = localGA_findMatchingTableRowForFCV12(S,nm,fp);
    if ~isempty(hit)
...
function hit = localGA_findMatchingTableRowForFCV12(S,nameText,fp)
hit = [];
nameText = lower(strtrimSafe(nameText));
fpLow = lower(strtrimSafe(fp));
for r = 1:size(S.subj,1)
    animal = lower(strtrimSafe(S.subj{r,2}));
    if isempty(animal) || strcmpi(animal,'n/a'), continue; end
    if strcmp(animal,nameText) || contains(nameText,animal) || contains(fpLow,animal)
        hit = r;
        return;
    end
end
