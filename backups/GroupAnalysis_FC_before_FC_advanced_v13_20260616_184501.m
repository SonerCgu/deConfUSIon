...
% Robust FC group export: matrices, top/max correlations, node summary, seed summary.
S = [];
if nargin >= 1 && isstruct(varargin{1})
    S = varargin{1};
else
    for ii = 1:2:numel(varargin)
        if ii+1 <= numel(varargin) && ischar(varargin{ii})
            key = lower(strtrim(varargin{ii}));
            if any(strcmp(key,{'s','state'})) && isstruct(varargin{ii+1})
                S = varargin{ii+1};
                break;
            end
        end
...
if isempty(S) || ~isstruct(S)
    error('Missing S/state struct for exportGroupFCResults.');
end
...
outDir = uigetdir(startDir,'Select folder for FC group export');
if isequal(outDir,0), return; end
tag = datestr(now,'yyyymmdd_HHMMSS');
baseName = ['FC_GroupAnalysis_' sanitizeFilename([R.groupA '_vs_' R.groupB]) '_' tag];
outRoot = fullfile(outDir,baseName);
if exist(outRoot,'dir') ~= 7, mkdir(outRoot); end

names = R.names;
try
    labelFile = '';
    if isfield(S,'fcAtlasLabelFile'), labelFile = S.fcAtlasLabelFile; end
    if ~isempty(labelFile) && exist(labelFile,'file')==2
        names = fcNamesFromAtlasFileV12(R.labels,names,labelFile);
    end
catch
end
...
fcWriteTopConnectionsCSV_V12(fullfile(outRoot,'top_connections_by_abs_difference.csv'),R,names,250,'diff');
fcWriteTopConnectionsCSV_V12(fullfile(outRoot,'top_connections_by_max_abs_correlation.csv'),R,names,250,'maxcorr');
fcWriteNodeSummaryCSV_V12(fullfile(outRoot,'region_node_summary_max_correlations.csv'),R,names);
fcWriteRegionListCSV_V12(fullfile(outRoot,'region_labels_and_names.csv'),R.labels,names);

seedText = '';
try, if isfield(S,'fcSeedRegion') && ~isempty(S.fcSeedRegion), seedText = S.fcSeedRegion; end, catch, end
try
    if isfield(S,'hFCSeedRegion') && ishghandle(S.hFCSeedRegion)
        seedText = strtrim(char(get(S.hFCSeedRegion,'String')));
    end
catch
end
if ~isempty(seedText)
    fcWriteSeedSummaryCSV_V12(fullfile(outRoot,'seed_region_summary.csv'),R,names,seedText);
end

try
    C = fcDefaultColorsV12();
    f = figure('Visible','off','Color',C.bg,'InvertHardcopy','off','Position',[100 100 1200 900]);
    ax = axes('Parent',f,'Position',[0.10 0.12 0.72 0.75]);
    fcPlotMatrix(ax,R.meanRA,[-1 1],['Mean FC: ' R.groupA ' Pearson r'],names,C);
    print(f,fullfile(outRoot,'meanFC_GroupA.png'),'-dpng','-r250');
    clf(f); ax = axes('Parent',f,'Position',[0.10 0.12 0.72 0.75]);
    fcPlotMatrix(ax,R.meanRB,[-1 1],['Mean FC: ' R.groupB ' Pearson r'],names,C);
    print(f,fullfile(outRoot,'meanFC_GroupB.png'),'-dpng','-r250');
    clf(f); ax = axes('Parent',f,'Position',[0.10 0.12 0.72 0.75]);
    fcPlotMatrix(ax,R.diffR,[-1 1],[R.groupA ' - ' R.groupB ' Pearson r'],names,C);
    print(f,fullfile(outRoot,'diffFC_GroupA_minus_GroupB.png'),'-dpng','-r250');
    clf(f); ax = axes('Parent',f,'Position',[0.10 0.12 0.72 0.75]);
    fcPlotPMatrix(ax,R.pMat,['p-values: ' R.groupA ' vs ' R.groupB],names,C);
    print(f,fullfile(outRoot,'p_values_neglog10.png'),'-dpng','-r250');
    close(f);
catch
...
msgbox(sprintf('FC group export complete:\n\n%s',outRoot),'FC export');
fprintf('\n[FC export] Saved:\n%s\n',outRoot);
end

function C = fcDefaultColorsV12()
C = struct();
C.bg = [0.04 0.04 0.04];
...
function fcWriteTopConnectionsCSV_V12(outFile,R,names,N,modeName)
if nargin < 4 || isempty(N), N = 250; end
if nargin < 5, modeName = 'diff'; end
nR = numel(R.labels);
rows = {};
for i = 1:nR
    for j = i+1:nR
        ...
        p = R.pMat(i,j);
        t = R.tMat(i,j);
        if strcmpi(modeName,'maxcorr')
            score = max(abs([a b]));
        else
            score = abs(d);
        end
        rows(end+1,:) = {score,R.labels(i),names{i},R.labels(j),names{j},a,b,d,p,t}; %#ok<AGROW>
    end
end
if isempty(rows), return; end
scores = cell2mat(rows(:,1));
[~,ord] = sort(scores,'descend');
ord = ord(1:min(N,numel(ord)));
rows = rows(ord,:);
fid = fopen(outFile,'w');
if fid < 0, error('Could not write %s',outFile); end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'rank,score,label_i,name_i,label_j,name_j,meanR_%s,meanR_%s,diffR,p,t\n',csvEscapeFC(R.groupA),csvEscapeFC(R.groupB));
for k = 1:size(rows,1)
    fprintf(fid,'%d,%.10g,%g,%s,%g,%s,%.10g,%.10g,%.10g,%.10g,%.10g\n', ...
        k,rows{k,1},rows{k,2},csvEscapeFC(rows{k,3}),rows{k,4},csvEscapeFC(rows{k,5}),rows{k,6},rows{k,7},rows{k,8},rows{k,9},rows{k,10});
end
end

function fcWriteNodeSummaryCSV_V12(outFile,R,names)
nR = numel(R.labels);
fid = fopen(outFile,'w');
if fid < 0, error('Could not write %s',outFile); end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'label,name,maxAbsR_%s,maxAbsR_%s,maxAbsDiffR,nSigEdges_pLT005,meanAbsR_%s,meanAbsR_%s\n',csvEscapeFC(R.groupA),csvEscapeFC(R.groupB),csvEscapeFC(R.groupA),csvEscapeFC(R.groupB));
for i = 1:nR
    idx = true(nR,1); idx(i) = false;
    a = R.meanRA(i,idx); b = R.meanRB(i,idx); d = R.diffR(i,idx); p = R.pMat(i,idx);
    fprintf(fid,'%g,%s,%.10g,%.10g,%.10g,%d,%.10g,%.10g\n', ...
        R.labels(i),csvEscapeFC(names{i}),fcMaxFiniteV12(abs(a)),fcMaxFiniteV12(abs(b)),fcMaxFiniteV12(abs(d)),sum(p<0.05 & isfinite(p)),nanmean_local(abs(a(:)),1),nanmean_local(abs(b(:)),1));
end
end

function m = fcMaxFiniteV12(x)
x = x(isfinite(x));
if isempty(x), m = NaN; else, m = max(x); end
end

function fcWriteSeedSummaryCSV_V12(outFile,R,names,seedText)
seedText0 = lower(strtrim(seedText));
idx = [];
lab = str2double(seedText0);
if isfinite(lab)
    idx = find(double(R.labels)==lab,1,'first');
end
if isempty(idx)
    for i=1:numel(names)
        if ~isempty(strfind(lower(names{i}),seedText0))
            idx = i; break;
        end
    end
end
if isempty(idx)
    warning('Seed/region not found for FC export: %s',seedText);
    return;
end
fid = fopen(outFile,'w');
if fid < 0, error('Could not write %s',outFile); end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'seed_label,seed_name,target_label,target_name,meanR_%s,meanR_%s,diffR,p,t\n',csvEscapeFC(R.groupA),csvEscapeFC(R.groupB));
for j=1:numel(names)
...
function fcWriteRegionListCSV_V12(outFile,labels,names)
fid = fopen(outFile,'w');
if fid < 0, error('Could not write %s',outFile); end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'label,name\n');
for i=1:numel(labels)
    fprintf(fid,'%g,%s\n',labels(i),csvEscapeFC(names{i}));
end
end

function namesOut = fcNamesFromAtlasFileV12(labels,namesIn,labelFile)
namesOut = namesIn;
try
    txt0 = fileread(labelFile);
    lines0 = regexp(txt0,'\r\n|\n|\r','split');
    map = containers.Map('KeyType','double','ValueType','char');
    for ii=1:numel(lines0)
        L = strtrim(lines0{ii});
        if isempty(L) || L(1)=='#', continue; end
        tok = regexp(L,'^\s*(\d+)\s+(.+?)\s*$','tokens','once');
        if isempty(tok)
            tok = regexp(L,'^\s*(\d+)[,\t;]\s*(.+?)\s*$','tokens','once');
        end
        if ~isempty(tok)
            id = str2double(tok{1});
            nm = strtrim(tok{2});
            if isfinite(id) && ~isempty(nm), map(id)=nm; end
        end
    end
    for i=1:numel(labels)
        id = double(labels(i));
        if isKey(map,id), namesOut{i} = map(id); end
    end
catch
end
end

function s = fcGetFieldCharV12(S,fieldName,fb)
s = fb;
try
    if isstruct(S) && isfield(S,fieldName) && ~isempty(S.(fieldName))
        s = strtrim(char(S.(fieldName)));
    end
catch
end
end

function v = fcGetFieldNumV12(S,fieldName,fb)
v = fb;
try
    if isstruct(S) && isfield(S,fieldName) && ~isempty(S.(fieldName))
