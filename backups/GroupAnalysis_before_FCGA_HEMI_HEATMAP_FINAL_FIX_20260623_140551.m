...
% Make sure changing Hemi/View/Display really refreshes the FC-GA plot.
try
...
% Merged = L/R homologues collapsed into base ROI names.
try
    n = size(M,1);
    ...
    if ~isempty(strfind(mode,'left vs right')) || ~isempty(strfind(mode,'left-right')) || ~isempty(strfind(mode,'l vs r')) || ~isempty(strfind(mode,'l/r'))
        if any(isL) && any(isR)
            ...
            hemiTitle = 'Left vs Right - no L/R labels detected';
            M2 = M;
            return;
        end
    end
    ...
    if strcmp(mode,'left') || strcmp(mode,'l') || ~isempty(strfind(mode,'left hemisphere'))
        if any(isL)
            ...
            hemiTitle = 'Left hemisphere - no left labels detected';
            M2 = M;
            return;
        end
    end
    ...
    if strcmp(mode,'right') || strcmp(mode,'r') || ~isempty(strfind(mode,'right hemisphere'))
        if any(isR)
            ...
            hemiTitle = 'Right hemisphere - no right labels detected';
            M2 = M;
            return;
        end
    end
...
labsX = fcGAHeatLabels_20260625(namesX,labelsX,labelMode);
labsY = fcGAHeatLabels_20260625(namesY,labelsY,labelMode);
nx = numel(labsX);
ny = numel(labsY);
nMax = max(nx,ny);
...
function labs = fcGAHeatLabels_20260625(names,labels,labelMode)
% Labels for heatmap. Merged names are base-only, so they display as AAA.
n = numel(labels);
labs = cell(n,1);
for ii = 1:n
    try
