...
try
    ...
    try, if isfield(S,'hFCSubject') && ishghandle(S.hFCSubject), set(S.hFCSubject,'Parent',pTop,'Units','normalized','Position',[0.095 row2+0.010 0.175 hCtl],'FontSize',fsCtl); end, catch, end
    fcGASetLabelPos_20260624(pTop,'View:',[0.290 row2 0.050 hLab],fsLab);
    try, if isfield(S,'hFCView') && ishghandle(S.hFCView), set(S.hFCView,'Parent',pTop,'Units','normalized','Position',[0.345 row2+0.010 0.155 hCtl],'FontSize',fsCtl); end, catch, end
    fcGASetLabelPos_20260624(pTop,'Hemi:',[0.520 row2 0.055 hLab],fsLab);
    try, if isfield(S,'hFCHemi') && ishghandle(S.hFCHemi), set(S.hFCHemi,'Parent',pTop,'Units','normalized','Position',[0.580 row2+0.010 0.125 hCtl],'FontSize',fsCtl); end, catch, end
    fcGASetLabelPos_20260624(pTop,'Color:',[0.700 row2 0.070 hLab],fsLab);
    try, if isfield(S,'hFCColorMap') && ishghandle(S.hFCColorMap), set(S.hFCColorMap,'Parent',pTop,'Units','normalized','Position',[0.775 row2+0.010 0.115 hCtl],'FontSize',fsCtl); end, catch, end
    fcGASetLabelPos_20260624(pTop,'Labels:',[0.865 row2 0.065 hLab],fsLab);
    try, if isfield(S,'hFCLabelMode') && ishghandle(S.hFCLabelMode), set(S.hFCLabelMode,'Parent',pTop,'Units','normalized','Position',[0.930 row2+0.010 0.065 hCtl],'FontSize',fsCtl); end, catch, end
