...
% Adds/restores ROI subset selector in the normal FC-GA GUI.
try
    ...
    if isempty(hLab) || ~ishghandle(hLab(1))
        hLab = uicontrol('Parent',pStrip,'Style','text','Units','normalized', ...
            'Position',[0.445 0.170 0.045 0.650], ...
            'String','ROIs:', ...
            'HorizontalAlignment','right', ...
            'FontSize',9.5, ...
            'FontWeight','bold', ...
        ...
        hLab = hLab(1);
        set(hLab,'Parent',pStrip,'Units','normalized','Position',[0.445 0.170 0.045 0.650],'Visible','on');
    end
    if isempty(hPop) || ~ishghandle(hPop(1))
        hPop = uicontrol('Parent',pStrip,'Style','popupmenu','Units','normalized', ...
            'Position',[0.495 0.165 0.105 0.650], ...
            'String',items, ...
            'Value',2, ...
            'FontSize',9.0, ...
            'BackgroundColor',[0.12 0.12 0.13], ...
        ...
        set(hPop,'Parent',pStrip,'Units','normalized','Position',[0.495 0.165 0.105 0.650], ...
...
% ROI subset selector. All returned indices are alphabetically sorted for clean heatmaps.
...
if fcGAIsAlphaChoice_20260624(choice)
    ...
    idx = fcGAAlphaSortIdx_20260624(R,unique([seedIdx roi2Idx idx(:)'],'stable'));
...
% Clean scientific heatmap: alphabetic order is preserved, all selected boxes are labeled.
try, cla(ax,'reset'); catch, cla(ax); end
imagesc(ax,M);
set(ax,'YDir','reverse');
axis(ax,'tight');
...
title(ax,ttl,'Color',C.txt,'FontWeight','bold','Interpreter','none');
xlabel(ax,'Regions', 'Color',C.txt,'Interpreter','none');
ylabel(ax,'Regions', 'Color',C.txt,'Interpreter','none');
box(ax,'on');
end
...
% Kept for compatibility; plotFCMatrix now controls dense heatmap labels directly.
try
    if isempty(ax) || ~ishghandle(ax), return; end
    vm = lower(strtrim(viewMode));
    if isempty(strfind(vm,'heatmap')) && isempty(strfind(vm,'matrix')), return; end
    nLab = max(numel(get(ax,'XTickLabel')),numel(get(ax,'YTickLabel')));
    if nLab > 120, fs = 3.2; elseif nLab > 90, fs = 4; elseif nLab > 60, fs = 5; elseif nLab > 35, fs = 6; else, fs = 8; end
    set(ax,'FontSize',fs,'TickLabelInterpreter','none');
catch
end
end
...
try
    ...
    if xData < min(xl) || xData > max(xl) || yData < min(yl) || yData > max(yl)
        return;
    end
