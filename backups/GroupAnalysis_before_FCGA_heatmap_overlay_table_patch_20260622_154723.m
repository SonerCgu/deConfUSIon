...
roi2Orig = popupIndex_SINGLE_20260616(S,'hFCRegion2',min(2,size(M0,1)));
seedOrig = max(1,min(seedOrig,size(M0,1)));
roi2Orig = max(1,min(roi2Orig,size(M0,1)));

% Default: show top 20 relevant ROIs unless user selected regions in Names table.
[selIdx,selNote] = fcGASelectedOrTopROIIdx_20260622(S,R,M0,seedOrig,roi2Orig,20);
if ~isempty(selIdx) && numel(selIdx) < size(M0,1)
    M0 = M0(selIdx,selIdx);
    stack0 = stack0(selIdx,selIdx,:);
    R.names = R.names(selIdx);
    R.labels = R.labels(selIdx);
...
% Important: for Left-vs-Right, row names and column names are different.
namesY = R.names(rowIdx); labelsY = R.labels(rowIdx);
namesX = R.names(colIdx); labelsX = R.labels(colIdx);

switch lower(viewMode)
    case 'heatmap'
        plotFCMatrix_CLEAN_20260617(S.axFCA,M,clim,sprintf('%s FC heatmap | %s | %s | n=%d | %s',R.groupName,hemiTitle,valTxt,R.n,selNote),namesX,namesY,labelsX,labelsY,S.C,cmapName,labelMode,hemiTitle);
    ...
    case 'subject matrix'
        if subjIdx <= 1
            plotFCMatrix_CLEAN_20260617(S.axFCA,M,clim,sprintf('%s mean subject matrix | %s | n=%d | %s',R.groupName,hemiTitle,R.n,selNote),namesX,namesY,labelsX,labelsY,S.C,cmapName,labelMode,hemiTitle);
        else
            si = max(1,min(subjIdx-1,size(stack0,3)));
            Ms0 = stack0(:,:,si);
            if thr > 0, Ms0(abs(Ms0) < thr) = 0; end
            [Ms,nms,lbs,rowS,colS,ht] = applyHemisphereMode_ADV_20260617(Ms0,R.names,R.labels,hemiMode);
            plotFCMatrix_CLEAN_20260617(S.axFCA,Ms,clim,sprintf('Subject matrix: %s | %s | %s',strtrimSafe(R.subjectNames{si}),ht,selNote),R.names(colS),R.names(rowS),R.labels(colS),R.labels(rowS),S.C,cmapName,labelMode,ht);
        end
    otherwise
...
try
    set(S.hFCInfo,'String',sprintf('Loaded %d FC subject(s). Showing %s, n=%d | %s | %s | Slice=%s | Seed=%s | ROI2=%s | %s',numel(S.FC.subjects),R.groupName,R.n,viewMode,hemiMode,R.sliceMode,roiName_SINGLE_20260616(R,seedIdx0),roiName_SINGLE_20260616(R,roi2Idx0),selNote));
catch
end
end
...
% Readable FC heatmap plotter with correct labels and live colormap update.
...
try
    fig = ancestor(ax,'figure'); S = guidata(fig);
    sl = popupStr_SAFE_20260617(S,'hFCSlice','');
    if ~isempty(sl) && isempty(strfind(lower(titleStr),'slice'))
        titleStr = [titleStr ' | ' sl];
    end
catch
end
title(ax,titleStr,'Color',C.txt,'FontWeight','bold','Interpreter','none');
...
maxTicks = 32; if fullMode, maxTicks = 18; end
...
hold(ax,'on'); plot(ax,x,mu,'LineWidth',2.2); hold(ax,'off');
...
title(ax,sprintf('Seed profile: FC from seed [%s] to each target ROI | mean ± SD | n=%d',seedName,size(stack,3)),'Color',C.txt,'FontWeight','bold','Interpreter','none');
xlabel(ax,'Target regions selected for display','Color',C.txt);
ylabel(ax,valTxt,'Color',C.txt);
if numel(roiKeep) <= 35
...
if ndims(map2) > 2
    try
        ...
        if ~isempty(tok), z = str2double(tok{1}); else, z = round(size(map2,3)/2); end
    catch
        z = round(size(map2,3)/2);
    end
    z = max(1,min(round(z),size(map2,3)));
    map2 = map2(:,:,z);
    note = sprintf('%s | slice %d',note,z);
end
imagesc(ax,map2); axis(ax,'image'); axis(ax,'off');
u = unique(double(map2(:))); u = u(isfinite(u) & u ~= 0);
try, colormap(ax,lines(max(16,numel(u)+1))); catch, colormap(ax,jet(max(16,numel(u)+1))); end
cb = colorbar(ax);
try, set(cb,'Color',S.C.txt); ylabel(cb,'Atlas/ROI label ID - not FC value','Color',S.C.txt,'Interpreter','none'); catch, end
mini = '';
try
    nShow = min(12,numel(u));
    parts = cell(nShow,1);
    for ii=1:nShow
        hit = find(double(R.labels(:)) == u(ii),1,'first');
        if isempty(hit), parts{ii}=sprintf('%g',u(ii)); else, parts{ii}=sprintf('%g:%s',u(ii),fcNiceName_SAFE_20260617(R.names{hit},R.labels(hit),'Abbrev',true)); end
    end
    if nShow > 0, mini = [' | labels: ' strjoin(parts,', ')]; end
catch
end
title(ax,['ROI overlay map | ' note mini],'Color',S.C.txt,'FontWeight','bold','Interpreter','none');
end
...
try
    ...
    try
        if isfield(S,'fcSelectedROIIdx') && ~isempty(S.fcSelectedROIIdx)
            include(fc_region_keep_indices_GA_20260622(S.fcSelectedROIIdx,n)) = true;
        else
            include(fcGASelectedOrTopROIIdx_20260622(S,struct('labels',labels,'names',{names}),zeros(n),1,min(2,n),20)) = true;
        end
    catch
        include(:) = true;
    end
    abbr = cell(n,1); full = cell(n,1);
    for kk = 1:n
        abbr{kk} = fcNiceName_SAFE_20260617(names{kk},labels(kk),'Abbrev',true);
        full{kk} = fcNiceName_SAFE_20260617(names{kk},labels(kk),'Full',true);
    end
    dataCell = [num2cell(include(:)), num2cell(labels(:)), abbr(:), full(:)];
    f = figure('Name','FC ROI names / select regions','Color',[0.08 0.08 0.08],'MenuBar','none','ToolBar','none','NumberTitle','off','Position',[180 100 980 760]);
    uicontrol(f,'Style','text','String','Select/unselect regions for FC-GA display. Label signs still encode hemisphere; names below are shown without L/R.','Units','normalized','Position',[0.03 0.935 0.94 0.045],'BackgroundColor',[0.08 0.08 0.08],'ForegroundColor',[1 1 1],'FontName','Arial','FontWeight','bold','FontSize',12,'HorizontalAlignment','left');
    t = uitable('Parent',f,'Units','normalized','Position',[0.03 0.095 0.94 0.825],'Data',dataCell,'ColumnName',{'Include','Label','Abbrev','Full region name'},'ColumnEditable',[true false false false],'RowName',[],'ColumnWidth',{70 90 180 590},'FontName','Arial','FontSize',12);
    uicontrol(f,'Style','pushbutton','String','Apply selection','Units','normalized','Position',[0.03 0.025 0.18 0.050],'BackgroundColor',[0.10 0.45 0.95],'ForegroundColor','w','FontWeight','bold','Callback',@(src,evt)fcApplyRegionNameTableSelection_GA_20260622(hFig,t,f));
    uicontrol(f,'Style','pushbutton','String','All','Units','normalized','Position',[0.23 0.025 0.10 0.050],'Callback',@(src,evt)set(t,'Data',fcSetIncludeColumn_GA_20260622(get(t,'Data'),true)));
    uicontrol(f,'Style','pushbutton','String','None','Units','normalized','Position',[0.35 0.025 0.10 0.050],'Callback',@(src,evt)set(t,'Data',fcSetIncludeColumn_GA_20260622(get(t,'Data'),false)));
...
function dataCell = fcSetIncludeColumn_GA_20260622(dataCell,val)
try
    for ii=1:size(dataCell,1), dataCell{ii,1} = logical(val); end
catch
end
end

function fcApplyRegionNameTableSelection_GA_20260622(hFig,tFig,f)
try
    S = guidata(hFig);
    D = get(tFig,'Data');
    inc = false(size(D,1),1);
    for ii=1:size(D,1), inc(ii) = logical(D{ii,1}); end
    sel = find(inc);
    if isempty(sel)
        warndlg('At least one region must be selected.','FC names'); return;
    end
    S.fcSelectedROIIdx = sel(:);
    S.lastFC = struct();
    guidata(hFig,S);
...
try
    ...
    nTop = max(10,min(20,round(nTop)));
    if isfield(S,'fcSelectedROIIdx') && ~isempty(S.fcSelectedROIIdx)
        sel = fc_region_keep_indices_GA_20260622(S.fcSelectedROIIdx,n);
        note = sprintf('selected %d ROI(s)',numel(sel));
        return;
    end
    A = abs(double(M));
    A(1:n+1:end) = NaN;
    score = max(A,[],2,'omitnan');
    if any(~isfinite(score)), score(~isfinite(score)) = 0; end
    try
        if seedIdx >= 1 && seedIdx <= n
            score = max(score,abs(double(M(seedIdx,:)))');
        end
    catch
    end
    [~,ord] = sort(score,'descend');
    sel = unique([seedIdx; roi2Idx; ord(1:min(nTop,n))],'stable');
    sel = sel(sel >= 1 & sel <= n);
    note = sprintf('auto top %d relevant ROI(s)',numel(sel));
catch
    sel = [];
    note = 'all ROIs';
end
