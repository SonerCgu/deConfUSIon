...
%%% INITIALIZE
%%% =====================================================================
try, fcGACleanFinalLayout_20260624(S); catch, end%INIT_FCGA_LAYOUT
guidata(hFig,S);
...
% Clean final FC-GA layout.
% Upper box = only essential FC controls.
% Matrix panel = large again.
% Status = below matrix panel, original style.
% Secondary controls = lower detached strip below status.
try
    ...
    try
        if isfield(S,'hFCView') && ishghandle(S.hFCView)
            pTop = get(S.hFCView,'Parent');
        end
    catch
    end
    if isempty(pTop) || ~ishghandle(pTop)
        try
            if isfield(S,'hFCDisplay') && ishghandle(S.hFCDisplay)
                pTop = get(S.hFCDisplay,'Parent');
            end
        catch
        end
    end
    ...
    % Top FC box is smaller; matrix panel keeps large height; status and controls below.
    set(pTop,'Position',[0.02 0.785 0.96 0.205]);
    set(pMat,'Position',[0.02 0.130 0.96 0.640]);

    % Restore FC matrix axes, leaving space for x labels but NOT for the status line.
    try
        set(S.axFCA,'Units','normalized','Position',[0.060 0.105 0.810 0.835]);
    catch
    end

    % Restore status line to the original global place, below FC matrices.
    ...
    % Detached bottom strip below the status line.
    ...
    % ------------------------------------------------------------------
    % Upper FC box: hide Threshold and make Recompute/Export readable.
    % ------------------------------------------------------------------
    ...
    % Row 1: load/group/display/recompute/export.
    try, set(S.hFCLoad,'Parent',pTop,'Units','normalized','Position',[0.020 0.610 0.135 0.290],'FontSize',10.5); catch, end
    try, set(S.hFCScan,'Parent',pTop,'Units','normalized','Position',[0.165 0.610 0.105 0.290],'FontSize',10.5); catch, end
    fcGASetLabelPos_20260624(pTop,'Group:',[0.300 0.675 0.055 0.150]);
    try, set(S.hFCGroupA,'Parent',pTop,'Units','normalized','Position',[0.355 0.635 0.120 0.230]); catch, end
    fcGASetLabelPos_20260624(pTop,'Display:',[0.505 0.675 0.070 0.150]);
    try, set(S.hFCDisplay,'Parent',pTop,'Units','normalized','Position',[0.575 0.635 0.120 0.230]); catch, end
    try, set(S.hFCCompute,'Parent',pTop,'Units','normalized','Position',[0.765 0.595 0.105 0.320],'String','Recompute','FontSize',9.0); catch, end
    try, set(S.hFCExport,'Parent',pTop,'Units','normalized','Position',[0.880 0.595 0.100 0.320],'String','Export GA','FontSize',9.0); catch, end

    % Row 2: subject/view/hemi/color/labels.
    fcGASetLabelPos_20260624(pTop,'Subject:',[0.020 0.360 0.065 0.145]);
    try, set(S.hFCSubject,'Parent',pTop,'Units','normalized','Position',[0.085 0.330 0.155 0.230]); catch, end
    fcGASetLabelPos_20260624(pTop,'View:',[0.270 0.360 0.045 0.145]);
    try, set(S.hFCView,'Parent',pTop,'Units','normalized','Position',[0.315 0.330 0.145 0.230]); catch, end
    fcGASetLabelPos_20260624(pTop,'Hemi:',[0.485 0.360 0.050 0.145]);
    try, if isfield(S,'hFCHemi') && ishghandle(S.hFCHemi), set(S.hFCHemi,'Parent',pTop,'Units','normalized','Position',[0.535 0.330 0.125 0.230]); end, catch, end
    fcGASetLabelPos_20260624(pTop,'Color:',[0.680 0.360 0.055 0.145]);
    try, if isfield(S,'hFCColorMap') && ishghandle(S.hFCColorMap), set(S.hFCColorMap,'Parent',pTop,'Units','normalized','Position',[0.735 0.330 0.115 0.230]); end, catch, end
    fcGASetLabelPos_20260624(pTop,'Labels:',[0.865 0.360 0.060 0.145]);
    try, if isfield(S,'hFCLabelMode') && ishghandle(S.hFCLabelMode), set(S.hFCLabelMode,'Parent',pTop,'Units','normalized','Position',[0.925 0.330 0.060 0.230]); end, catch, end

    % Row 3: seed/roi/slice only.
    fcGASetLabelPos_20260624(pTop,'Seed:',[0.020 0.100 0.045 0.145]);
    try, set(S.hFCRegion1,'Parent',pTop,'Units','normalized','Position',[0.065 0.075 0.235 0.235]); catch, end
    fcGASetLabelPos_20260624(pTop,'ROI 2:',[0.330 0.100 0.055 0.145]);
    try, set(S.hFCRegion2,'Parent',pTop,'Units','normalized','Position',[0.385 0.075 0.235 0.235]); catch, end
    fcGASetLabelPos_20260624(pTop,'Slice:',[0.650 0.100 0.050 0.145]);
    try, if isfield(S,'hFCSlice') && ishghandle(S.hFCSlice), set(S.hFCSlice,'Parent',pTop,'Units','normalized','Position',[0.700 0.075 0.100 0.235]); end, catch, end

    % ------------------------------------------------------------------
    % Bottom strip: Y/plot/action controls.
    % ------------------------------------------------------------------
    ...
    if isfield(S,'hFCPlotColor') && ~isempty(S.hFCPlotColor) && ishghandle(S.hFCPlotColor)
        fcGASetCtrlPos_20260624(S.hFCPlotColor,pStrip,[0.345 0.165 0.090 0.650]);
    end

    hAnimals = [];
    try, if isfield(S,'hFCAnimals') && ishghandle(S.hFCAnimals), hAnimals = S.hFCAnimals; end, catch, end
    ...
    hNames = [];
    try, if isfield(S,'hFCNames') && ishghandle(S.hFCNames), hNames = S.hFCNames; end, catch, end
    ...
    hPNG = [];
    try, if isfield(S,'hFCExportPNG') && ishghandle(S.hFCExportPNG), hPNG = S.hFCExportPNG; end, catch, end
    if isempty(hPNG), hPNG = fcGAFindButtonExact_20260624(fig,'PNG'); end
    fcGASetCtrlPos_20260624(hPNG,pStrip,[0.785 0.120 0.070 0.760]);

    hLarge = fcGAFindButtonExact_20260624(fig,'Large');
    if isempty(hLarge), hLarge = fcGAFindButtonExact_20260624(fig,'⛶'); end
    if ~isempty(hLarge) && ishghandle(hLarge)
        fcGASetCtrlPos_20260624(hLarge,pStrip,[0.865 0.120 0.085 0.760]);
        try
            set(hLarge,'String','Large', ...
                'Callback',@(src,evt)showFCLargeView_GA_20260622(ancestor(src,'figure')));
        catch
        end
    end
    ...
    try, fprintf('FC-GA clean final layout warning: %s\n',ME.message); catch, end
end
end

function fcGASetLabelPos_20260624(parentHandle,labelText,pos)
try
