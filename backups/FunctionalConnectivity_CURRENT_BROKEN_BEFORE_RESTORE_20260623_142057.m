...
%% TARGETED_FC_USE_SCM_ATLAS_BUTTON_20260623_V2
try
    delete(findall(pSeedView,'Tag','FC_WarpLabelsButton_20260622'));
    delete(findall(pSeedView,'Tag','FC_UseSCMAtlasButton_20260623'));
    if exist('pSeedDisplayFinal','var') && ishghandle(pSeedDisplayFinal)
        uicontrol('Parent',pSeedDisplayFinal,'Style','pushbutton','Units','normalized', ...
            'Position',[0.105 0.030 0.355 0.055], ...
            'String','Use SCM atlas','Tag','FC_UseSCMAtlasButton_20260623', ...
            'BackgroundColor',[0.10 0.55 0.35],'ForegroundColor','w', ...
            'FontName',C.font,'FontWeight','bold','FontSize',6.8, ...
            'TooltipString','Import slice-wise warped atlas labels from SCM bundle into FC ROI atlas', ...
            'Callback',@onUseSCMAtlasWarp);
    end
catch ME_scmbtn
    try, fprintf('FC Use SCM atlas button warning: %s\n',ME_scmbtn.message); catch, end
end
...
function onMapClick(~,~)
    ...
    function onUseSCMAtlasWarp(~,~)
        s = guidata(fig);
        try
            cs = s.currentSubject;
            if cs < 1 || cs > numel(s.subjects)
                errordlg('No current subject selected.','Use SCM atlas');
                return;
            end
            ...
            if isempty(A)
                choice = questdlg({'No SCM warped atlas map was found automatically.','','Do you want to select an SCM bundle MAT file manually?'}, ...
                    'Use SCM atlas','Select MAT','Cancel','Select MAT');
                if strcmpi(choice,'Select MAT')
                    startDir = fc_first_existing_dir_20260623(fc_scm_candidate_dirs_20260623(subj,s));
                    [f0,p0] = uigetfile({'*.mat','MAT files (*.mat)'},'Select SCM bundle / atlas MAT',startDir);
                    if ~isequal(f0,0)
                        srcFile = fullfile(p0,f0);
                        [A,meta,~,report] = fc_try_load_scm_file_20260623(srcFile,targetSize);
                    end
                end
            end

            if isempty(A)
                msg = [{'Could not find slice-wise SCM warped atlas labels.'; ''; 'Searched / inspected:'}; report(:)];
                try, setStatus('Use SCM atlas failed: no warped atlas labels found',C.warn); catch, end
                errordlg(msg,'Use SCM atlas');
                return;
            end

            A = fc_fit_label_volume_20260623(A,targetSize);
            A = int32(round(double(A)));

            s.subjects(cs).roiAtlas = A;
            s.subjects(cs).labelMap = A;
            s.subjects(cs).roiMap = A;
            s.subjects(cs).scmAtlasWarp = meta;
            s.subjects(cs).scmAtlasWarp.sourceFile = srcFile;
            s.subjects(cs).atlasSourceFile = srcFile;
            s.subjects(cs).roiSourceFile = srcFile;

            try, if isfield(meta,'colorTable'), s.subjects(cs).colorTable = meta.colorTable; end, catch, end
            try, if isfield(meta,'labelColors'), s.subjects(cs).labelColors = meta.labelColors; end, catch, end
            try, if isfield(meta,'rgbVolume'), s.subjects(cs).rgbMap = meta.rgbVolume; end, catch, end
            try, if isfield(meta,'abbrNames'), s.subjects(cs).abbrNames = meta.abbrNames; end, catch, end
            try, if isfield(meta,'fullNames'), s.subjects(cs).fullNames = meta.fullNames; end, catch, end
            try, if isfield(meta,'labels'), s.subjects(cs).labels = meta.labels; end, catch, end

            try
                for zz = 1:size(A,3)
                    s.subjects(cs).sliceResults(zz).roiAtlas = A(:,:,zz);
                    s.subjects(cs).sliceResults(zz).labelMap = A(:,:,zz);
                    s.subjects(cs).sliceResults(zz).roiMap = A(:,:,zz);
                    s.subjects(cs).sliceResults(zz).source = 'SCM warped atlas imported into FC';
                    s.subjects(cs).sliceResults(zz).sourceFile = srcFile;
                    if isfield(meta,'rgbVolume') && ndims(meta.rgbVolume) >= 4
                        try, s.subjects(cs).sliceResults(zz).rgbMap = meta.rgbVolume(:,:,zz,:); catch, end
                    end
                end
            catch
            end
            ...
            try
                if iscell(s.roiResults)
                    s.roiResults(cs,:) = cell(1,size(s.roiResults,2));
                end
            catch
            end
            ...
            try, setStatus(sprintf('Imported SCM warped atlas labels: %s',srcFile),C.good); catch, end

            try
                ep = s.currentEpoch;
                s = computeROI(s,cs,ep);
                guidata(fig,s);
                setStatus('SCM atlas imported and ROI current recomputed. Export a NEW GA bundle.',C.good);
            catch MEroi
                try, warning('ROI recompute after SCM atlas import failed: %s',MEroi.message); catch, end
            end

            try, refreshAll(); catch, end

            msgbox({'SCM warped atlas labels imported into FC.','', ...
                sprintf('Source: %s',srcFile), ...
                sprintf('Label volume size: %d x %d x %d',size(A,1),size(A,2),size(A,3)), ...
                '','Now export a NEW FC GroupAnalysis bundle and reload it in GroupAnalysis.'}, ...
                'Use SCM atlas');

        catch ME
            try, setStatus(['Use SCM atlas failed: ' ME.message],C.warn); catch, end
            errordlg(ME.message,'Use SCM atlas failed');
        end
    end
...
useLR = hasLR; % TRUE_LR_EXPORT_FIX_20260623: use Seg.Left/Right traces whenever available
    if useLR && ~hasSignedMap
        try, fprintf('FC true-LR note: Seg.Left/Right traces found, but labelMap is unsigned. Heatmap/export will use true LR traces; spatial overlay may remain bilateral/unsigned.\n'); catch, end
    end
...
info.note = 'True LR FC uses Seg.Left/Right traces whenever available. If labelMap is unsigned, heatmap/export can still be true LR, but spatial overlay may remain bilateral/unsigned.';
end
...
for i = 1:nSub
    ...
    try, rec.atlasSourceFile = fc_getc_auto_v4(subj,'atlasSourceFile',''); catch, end
    try, rec.colorTable = fc_get_auto_v4(subj,'colorTable',[]); catch, end
    try, rec.labelColors = fc_get_auto_v4(subj,'labelColors',[]); catch, end
    try, rec.rgbMap = fc_get_auto_v4(subj,'rgbMap',[]); catch, end
    try, rec.abbrNames = fc_get_auto_v4(subj,'abbrNames',{}); catch, end
    try, rec.fullNames = fc_get_auto_v4(subj,'fullNames',{}); catch, end
...
function targetSize = fc_target_size_from_fc_20260623(s,subj)
targetSize = [];
try
    if isfield(s,'Y') && isfield(s,'X') && isfield(s,'Z') && ~isempty(s.Y) && ~isempty(s.X) && ~isempty(s.Z)
        targetSize = [double(s.Y) double(s.X) double(s.Z)];
    end
catch
end
if isempty(targetSize)
    try
        if isfield(subj,'I4') && ~isempty(subj.I4)
            sz = size(subj.I4);
            if numel(sz) >= 3, targetSize = sz(1:3); end
        end
    catch
    end
end
...
if isempty(targetSize), targetSize = [1 1 1]; end
targetSize = round(double(targetSize(1:3)));
end

function [A,meta,srcFile,report] = fc_load_scm_warped_atlas_20260623(subj,s,targetSize)
A = [];
meta = struct();
srcFile = '';
report = {};
dirs = fc_scm_candidate_dirs_20260623(subj,s);
dirs = fc_unique_existing_dirs_20260623(dirs);
for dd = 1:numel(dirs)
    d0 = dirs{dd};
    report{end+1,1} = ['DIR: ' d0]; %#ok<AGROW>
    files = fc_find_scm_candidate_files_20260623(d0);
    for ff = 1:numel(files)
        [A0,meta0,~,r0] = fc_try_load_scm_file_20260623(files{ff},targetSize);
        report = [report; r0(:)]; %#ok<AGROW>
        if ~isempty(A0)
            A = A0;
            meta = meta0;
            srcFile = files{ff};
...
function dirs = fc_scm_candidate_dirs_20260623(subj,s)
dirs = {};
try, dirs{end+1}=fc_getc_any_20260623(subj,'analysisDir',''); catch, end
try, dirs{end+1}=fc_getc_any_20260623(subj,'saveRoot',''); catch, end
try, dirs{end+1}=fc_getc_any_20260623(subj,'sourceFile',''); catch, end
try, dirs{end+1}=fc_getc_any_20260623(subj,'roiSourceFile',''); catch, end
try, dirs{end+1}=fc_getc_any_20260623(subj,'atlasSourceFile',''); catch, end
try, dirs{end+1}=fc_getc_any_20260623(s,'saveRoot',''); catch, end
try, dirs{end+1}=fc_getc_any_20260623(s,'rootDir',''); catch, end
try, dirs{end+1}=fc_getc_any_20260623(s,'loadedPath',''); catch, end
more = {};
for i=1:numel(dirs)
    d = dirs{i};
    if isempty(d), continue; end
    if exist(d,'file')==2, d = fileparts(d); end
    if exist(d,'dir')==7
        more{end+1}=d; %#ok<AGROW>
        more{end+1}=fullfile(d,'SCM Bundle'); %#ok<AGROW>
        more{end+1}=fullfile(d,'SCM_Bundle'); %#ok<AGROW>
        more{end+1}=fullfile(d,'GroupAnalysis','SCM Bundle'); %#ok<AGROW>
        more{end+1}=fullfile(d,'GroupAnalysis','SCM_Bundle'); %#ok<AGROW>
        more{end+1}=fullfile(d,'SCM'); %#ok<AGROW>
        p = fileparts(d);
        if ~isempty(p) && exist(p,'dir')==7
            more{end+1}=fullfile(p,'SCM Bundle'); %#ok<AGROW>
            more{end+1}=fullfile(p,'GroupAnalysis','SCM Bundle'); %#ok<AGROW>
        end
    end
end
dirs = [dirs(:); more(:)];
...
function dirs = fc_unique_existing_dirs_20260623(dirsIn)
dirs = {};
for i=1:numel(dirsIn)
    d = dirsIn{i};
    if isempty(d), continue; end
    if exist(d,'file')==2, d = fileparts(d); end
    if exist(d,'dir')==7 && ~any(strcmp(dirs,d))
        dirs{end+1,1}=d; %#ok<AGROW>
    end
end
end

function d = fc_first_existing_dir_20260623(dirs)
d = pwd;
try
    dd = fc_unique_existing_dirs_20260623(dirs);
    if ~isempty(dd), d = dd{1}; end
catch
end
end

function files = fc_find_scm_candidate_files_20260623(rootDir)
files = {};
if exist(rootDir,'dir') ~= 7, return; end
files = fc_recursive_mat_files_20260623(rootDir,0,4);
if isempty(files), return; end
score = zeros(numel(files),1);
for i=1:numel(files)
    nm = lower(files{i});
    if ~isempty(strfind(nm,'scm')), score(i)=score(i)+12; end
    if ~isempty(strfind(nm,'bundle')), score(i)=score(i)+8; end
    if ~isempty(strfind(nm,'warp')), score(i)=score(i)+8; end
    if ~isempty(strfind(nm,'atlas')), score(i)=score(i)+6; end
    if ~isempty(strfind(nm,'label')), score(i)=score(i)+4; end
    if ~isempty(strfind(nm,'groupanalysis')), score(i)=score(i)+3; end
    if ~isempty(strfind(nm,'backup')), score(i)=score(i)-20; end
end
[~,ord] = sort(score,'descend');
files = files(ord);
end

function files = fc_recursive_mat_files_20260623(rootDir,depth,maxDepth)
files = {};
if depth > maxDepth || exist(rootDir,'dir') ~= 7, return; end
try
    d = dir(rootDir);
    for i=1:numel(d)
        nm = d(i).name;
        ...
        if d(i).isdir
            lnm = lower(nm);
            if ~strcmp(lnm,'backups') && ~strcmp(lnm,'backup') && ~strcmp(lnm,'archive')
                sub = fc_recursive_mat_files_20260623(fp,depth+1,maxDepth);
                files = [files; sub(:)]; %#ok<AGROW>
            end
        else
            [~,~,e] = fileparts(nm);
...
function [A,meta,srcFile,report] = fc_try_load_scm_file_20260623(matFile,targetSize)
A = [];
meta = struct();
srcFile = matFile;
report = {['FILE: ' matFile]};
try
    W = whos('-file',matFile);
    nms = {W.name};
    report{end+1,1} = ['  vars: ' strjoin(nms,', ')];
    L = load(matFile);
    [A,why] = fc_extract_label_volume_20260623(L,targetSize);
    report{end+1,1} = ['  atlas result: ' why];
    if ~isempty(A)
        meta = fc_extract_scm_meta_20260623(L,A);
        meta.sourceFile = matFile;
        meta.extractNote = why;
    end
catch ME
    report{end+1,1} = ['  ERROR: ' ME.message];
end
end

function [A,why] = fc_extract_label_volume_20260623(S,targetSize)
A = [];
why = 'not found';
candidates = {};
paths = {};
[candidates,paths] = fc_collect_numeric_label_candidates_20260623(S,'',0,candidates,paths);
bestScore = -Inf;
bestA = [];
bestWhy = '';
for i=1:numel(candidates)
    C = candidates{i};
    pth = paths{i};
    [ok,score,C2,reason] = fc_score_label_candidate_20260623(C,pth,targetSize);
    if ok && score > bestScore
        bestScore = score;
        bestA = C2;
        bestWhy = sprintf('%s | %s | score %.1f',pth,reason,score);
    end
end
if ~isempty(bestA)
    A = fc_fit_label_volume_20260623(bestA,targetSize);
    why = bestWhy;
end
end

function [candidates,paths] = fc_collect_numeric_label_candidates_20260623(x,path,depth,candidates,paths)
if depth > 7, return; end
try
    if isnumeric(x) && ~isempty(x)
        candidates{end+1,1} = x; %#ok<AGROW>
        paths{end+1,1} = path; %#ok<AGROW>
        return;
    end
    if isstruct(x)
        if numel(x) > 1
            [candidates,paths] = fc_collect_slice_array_candidates_20260623(x,path,candidates,paths);
        end
        f = fieldnames(x);
        for ii=1:numel(f)
            try
                p2 = f{ii};
                if ~isempty(path), p2 = [path '.' f{ii}]; end
                [candidates,paths] = fc_collect_numeric_label_candidates_20260623(x(1).(f{ii}),p2,depth+1,candidates,paths);
            catch
            end
        end
    elseif iscell(x)
        for ii=1:min(numel(x),200)
            p2 = sprintf('%s{%d}',path,ii);
            [candidates,paths] = fc_collect_numeric_label_candidates_20260623(x{ii},p2,depth+1,candidates,paths);
        end
    end
catch
end
...
function [candidates,paths] = fc_collect_slice_array_candidates_20260623(S,path,candidates,paths)
try
    fPref = {'labelMap','roiMap','roiAtlas','atlasMap','atlasLabels','regionMap','warpedAtlas','warpedLabelMap','labels'};
    for ff=1:numel(fPref)
        if isfield(S,fPref{ff})
            tmp = {};
            for zz=1:numel(S)
                try
                    a = S(zz).(fPref{ff});
                    if isnumeric(a) && ismatrix(squeeze(a))
                        tmp{zz} = squeeze(a); %#ok<AGROW>
                    end
                catch
                end
            end
            ...
            if ~isempty(good)
                ...
                for zz=1:Z
                    if zz<=numel(tmp) && ~isempty(tmp{zz})
                        a = double(tmp{zz});
                        if ~isequal(size(a),[Y X])
                            try, a = imresize(a,[Y X],'nearest'); catch, end
                        end
                        A(:,:,zz)=round(a);
                    end
                end
                candidates{end+1,1}=A; %#ok<AGROW>
                paths{end+1,1}=[path '.' fPref{ff} ' slice-array']; %#ok<AGROW>
            end
        end
    end
catch
...
function [ok,score,A,reason] = fc_score_label_candidate_20260623(C,path,targetSize)
ok = false;
score = -Inf;
A = [];
reason = '';
try
    C = squeeze(C);
    if isempty(C) || ~isnumeric(C), return; end
    nd = ndims(C);
    sz = size(C);
    p = lower(path);
    if nd < 2 || nd > 4, return; end
    if nd == 4 && sz(end) == 3, return; end
    vals = double(C(:));
    ...
    if fracInt < 0.95, return; end
    score = 0;
    key = {'warped','warp','scm','atlas','label','roi','region','slice'};
    w = [12 12 10 8 7 7 5 4];
    for kk=1:numel(key)
        if ~isempty(strfind(p,key{kk})), score = score + w(kk); end
    end
    if nd == 3, score = score + 10; end
    if numel(targetSize) == 3
        if numel(sz)>=3 && sz(3) == targetSize(3), score = score + 8; end
        if sz(1) == targetSize(1) && sz(2) == targetSize(2), score = score + 8; end
    end
    if max(abs(u)) > 10000, score = score - 20; end
    if numel(u) > 5000, score = score - 10; end
    ok = true;
    A = round(double(C));
    reason = sprintf('candidate size [%s], unique=%d',num2str(size(C)),numel(u));
catch
end
end

function A = fc_fit_label_volume_20260623(A,targetSize)
try
    A = squeeze(round(double(A)));
    if ndims(A) == 2
        A = repmat(A,[1 1 targetSize(3)]);
    end
    if ndims(A) ~= 3, return; end
    if isequal(size(A),targetSize), return; end
    B = zeros(targetSize);
    for zz=1:targetSize(3)
        srcZ = min(zz,size(A,3));
        a = A(:,:,srcZ);
        if ~isequal(size(a),targetSize(1:2))
            try, a = imresize(a,targetSize(1:2),'nearest'); catch, end
        end
        B(:,:,zz) = round(double(a));
    end
    A = B;
...
function meta = fc_extract_scm_meta_20260623(S,A)
meta = struct();
meta.importTime = datestr(now);
try, meta.labels = unique(double(A(:))); meta.labels = meta.labels(isfinite(meta.labels) & meta.labels ~= 0); catch, end
try, meta.colorTable = fc_find_meta_field_20260623(S,{'colorTable','colors','labelColors','atlasColors','rgbTable'},0); catch, end
try, meta.labelColors = fc_find_meta_field_20260623(S,{'labelColors','atlasColors','rgbTable','colorTable'},0); catch, end
try, meta.rgbVolume = fc_find_rgb_volume_20260623(S,0); catch, end
try, meta.abbrNames = fc_find_meta_field_20260623(S,{'abbrNames','acronyms','abbrev','abbreviations','shortNames'},0); catch, end
try, meta.fullNames = fc_find_meta_field_20260623(S,{'fullNames','regionNames','names','labelNames','atlasNames'},0); catch, end
end

function val = fc_find_meta_field_20260623(x,names,depth)
val = [];
...
try
    if isstruct(x)
        ...
        for ii=1:numel(f)
            val = fc_find_meta_field_20260623(x(1).(f{ii}),names,depth+1);
            if ~isempty(val), return; end
        end
    elseif iscell(x)
        for ii=1:min(numel(x),50)
            val = fc_find_meta_field_20260623(x{ii},names,depth+1);
            if ~isempty(val), return; end
        end
    end
catch
...
function rgb = fc_find_rgb_volume_20260623(x,depth)
rgb = [];
if depth > 5, return; end
try
    if isnumeric(x) && ndims(x) == 4 && size(x,4) == 3
        rgb = x;
        return;
    end
    if isstruct(x)
        ...
        for ii=1:numel(f)
            lf = lower(f{ii});
            if ~isempty(strfind(lf,'rgb')) || ~isempty(strfind(lf,'color'))
                rgb = fc_find_rgb_volume_20260623(x(1).(f{ii}),depth+1);
    ...
    elseif iscell(x)
        for ii=1:min(numel(x),50)
            rgb = fc_find_rgb_volume_20260623(x{ii},depth+1);
            if ~isempty(rgb), return; end
        end
    end
catch
...
function val = fc_getc_any_20260623(s,field,defaultVal)
val = defaultVal;
try
