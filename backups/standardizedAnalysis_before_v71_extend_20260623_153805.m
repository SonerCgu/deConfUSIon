...
uicontrol(state.fig,'Style','text', ...
    'String','Drag a row by holding left click on the step name/preset text. Green row = selected. Ticked rows run in shown order.', ...
    'Units','normalized', ...
...
function startDrag(r)
    ...
    refreshRows();
    refreshSelectedPanel();
    setStatus(['Dragging row #' num2str(r) '. Release mouse on destination row.'],[0.85 0.95 0.80]);
end
...
steps(4)  = makeStep(false, 4,'Filtering',             'filterType 1=band, low=0.001 Hz, high=0.20 Hz, order=4',NaN,NaN,NaN,NaN,NaN,NaN,NaN,NaN);
...
steps(11) = makeStep(true, 11,'Video GUI',             'baseline 30-35 s, signed PSC, caxis -100..100, alpha 20..100',NaN,NaN,30,35,-100,100,20,100);
steps(12) = makeStep(false,12,'Time-Course Viewer',    'optional inspect ROI/time-course',NaN,NaN,30,35,-100,100,NaN,NaN);
steps(13) = makeStep(true, 13,'SCM GUI',               'baseline 30-35 s, signed PSC, caxis -100..100, alpha 20..100',NaN,NaN,30,35,-100,100,20,100);
...
labels.amin = 'Alpha min abs (%)'; labels.amax = 'Alpha max abs (%)';
