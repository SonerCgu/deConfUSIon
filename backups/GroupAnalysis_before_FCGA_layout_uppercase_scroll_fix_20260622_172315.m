...
try
    ...
    try
        S.hFCZoom = mkBtn(pFCTop,'Large',[0.835 0.020 0.060 0.105],C.btnAction,@(~,~) showFCLargeView_GA_20260622(hFig));
        set(S.hFCZoom,'TooltipString','Open current FC-GA view in a large exportable window');
    catch
        try, S.hFCZoom = mkBtn(pFCTop,'Large',[0.835 0.020 0.060 0.105],C.btnAction,@(~,~) showFCLargeView_GA_20260622(hFig)); catch, end
    end
...
forceAllTicks=false; try, forceAllTicks=isappdata(ax,'FCGA_FORCE_ALL_TICKS'); catch, end; maxTicks=32; if strcmpi(strtrimSafe(labelMode),'Full'), maxTicks=18; end; if forceAllTicks, maxTicks=max(size(M)); end
ticksY = fcTickIdx_SAFE_20260617(nR,maxTicks);
ticksX = fcTickIdx_SAFE_20260617(nC,maxTicks);
...
hold(ax,'on');
fcDrawHeatmapGrid_GA_20260622(ax,nR,nC);
hold(ax,'off');
...
if ~isempty(strfind(s,'||'))
    ...
    left = strtrimSafe(parts{1});
    if numel(parts) >= 2, full = strtrimSafe(parts{2}); else, full = ''; end
else
    left = s;
end
left = regexprep(left,'\[[^\]]*\]','');
...
try
S=guidata(hFig); if isempty(S)||~isfield(S,'lastFC')||isempty(S.lastFC), return; end
f=figure('Name','FC-GA large interactive view','Color',[0.08 0.08 0.08],'NumberTitle','off','MenuBar','none','ToolBar','figure','Position',[40 35 1620 940]); guidata(f,S); setappdata(f,'mainFig',hFig);
ax=axes('Parent',f,'Units','normalized','Position',[0.055 0.105 0.760 0.820]); setappdata(ax,'FCGA_FORCE_ALL_TICKS',true);
viewItems=get(S.hFCView,'String'); if ischar(viewItems), viewItems=cellstr(viewItems); end; seedItems=get(S.hFCRegion1,'String'); if ischar(seedItems), seedItems=cellstr(seedItems); end; roiItems=get(S.hFCRegion2,'String'); if ischar(roiItems), roiItems=cellstr(roiItems); end; sliceItems=get(S.hFCSlice,'String'); if ischar(sliceItems), sliceItems=cellstr(sliceItems); end
labels={'View','Display','Hemisphere','Color map','Labels','ROI subset','Seed','ROI 2','Slice'}; y=0.895;
for k=1:numel(labels), uicontrol(f,'Style','text','String',labels{k},'Units','normalized','Position',[0.835 y 0.060 0.025],'BackgroundColor',[0.08 0.08 0.08],'ForegroundColor',[1 1 1],'FontWeight','bold','HorizontalAlignment','left'); y=y-0.055; end
y=0.890;
hView=uicontrol(f,'Style','popupmenu','String',viewItems,'Value',get(S.hFCView,'Value'),'Units','normalized','Position',[0.905 y 0.080 0.035]); y=y-0.055;
hDisp=uicontrol(f,'Style','popupmenu','String',{'Pearson r','Fisher z'},'Value',max(1,min(2,get(S.hFCDisplay,'Value'))),'Units','normalized','Position',[0.905 y 0.080 0.035]); y=y-0.055;
hHemi=uicontrol(f,'Style','popupmenu','String',{'All','Left only','Right only','Merged L/R','Left vs Right','Right vs Left'},'Value',max(1,min(6,get(S.hFCHemi,'Value'))),'Units','normalized','Position',[0.905 y 0.080 0.035]); y=y-0.055;
hColor=uicontrol(f,'Style','popupmenu','String',{'Blue-White-Red','Blue-White','Red-White-Blue','parula','hot','jet','gray'},'Units','normalized','Position',[0.905 y 0.080 0.035]); y=y-0.055;
hLab=uicontrol(f,'Style','popupmenu','String',{'Abbrev','Full','Label ID'},'Value',max(1,min(3,get(S.hFCLabelMode,'Value'))),'Units','normalized','Position',[0.905 y 0.080 0.035]); y=y-0.055;
hROISet=uicontrol(f,'Style','popupmenu','String',{'Main selection/top','All ROIs','Seed +10/-10','Manual Names selection'},'Units','normalized','Position',[0.905 y 0.080 0.035]); y=y-0.055;
hSeed=uicontrol(f,'Style','popupmenu','String',seedItems,'Value',get(S.hFCRegion1,'Value'),'Units','normalized','Position',[0.905 y 0.080 0.035]); y=y-0.055;
hROI2=uicontrol(f,'Style','popupmenu','String',roiItems,'Value',get(S.hFCRegion2,'Value'),'Units','normalized','Position',[0.905 y 0.080 0.035]); y=y-0.055;
hSlice=uicontrol(f,'Style','popupmenu','String',sliceItems,'Value',get(S.hFCSlice,'Value'),'Units','normalized','Position',[0.905 y 0.080 0.035]);
hAutoY=uicontrol(f,'Style','checkbox','String','Y auto','Value',1,'Units','normalized','Position',[0.835 0.335 0.070 0.030],'BackgroundColor',[0.08 0.08 0.08],'ForegroundColor',[1 1 1]); hY0=uicontrol(f,'Style','edit','String','0','Units','normalized','Position',[0.905 0.335 0.035 0.030]); hY1=uicontrol(f,'Style','edit','String','1.5','Units','normalized','Position',[0.950 0.335 0.035 0.030]);
uicontrol(f,'Style','pushbutton','String','Update','Units','normalized','Position',[0.835 0.255 0.070 0.045],'BackgroundColor',[0.10 0.45 0.95],'ForegroundColor','w','FontWeight','bold','Callback',@(src,evt)fcGALargeReplot_20260622(f));
uicontrol(f,'Style','pushbutton','String','Export PNG','Units','normalized','Position',[0.915 0.255 0.070 0.045],'BackgroundColor',[0.10 0.60 0.25],'ForegroundColor','w','FontWeight','bold','Callback',@(src,evt)fcExportLargeView_GA_20260622(f));
D=struct('mainFig',hFig,'ax',ax,'hView',hView,'hDisp',hDisp,'hHemi',hHemi,'hColor',hColor,'hLab',hLab,'hROISet',hROISet,'hSeed',hSeed,'hROI2',hROI2,'hSlice',hSlice,'hAutoY',hAutoY,'hY0',hY0,'hY1',hY1); setappdata(f,'D',D);
set([hView hDisp hHemi hColor hLab hROISet hSeed hROI2 hSlice hAutoY hY0 hY1],'Callback',@(src,evt)fcGALargeReplot_20260622(f)); set(f,'WindowScrollWheelFcn',@(src,evt)fcGALargeScrollSlice_20260622(f,evt)); fcGALargeReplot_20260622(f);
catch ME, try, errordlg(ME.message,'FC-GA large view'); catch, end, end
end

function fcGALargeReplot_20260622(f)
D=getappdata(f,'D'); S=guidata(D.mainFig); Rfull=S.lastFC;
vi=get(D.hView,'String'); viewMode=vi{get(D.hView,'Value')}; di=get(D.hDisp,'String'); dispMode=di{get(D.hDisp,'Value')}; hi=get(D.hHemi,'String'); hemiMode=hi{get(D.hHemi,'Value')}; ci=get(D.hColor,'String'); cmapName=ci{get(D.hColor,'Value')}; li=get(D.hLab,'String'); labelMode=li{get(D.hLab,'Value')}; ri=get(D.hROISet,'String'); roiMode=ri{get(D.hROISet,'Value')};
seedOrig=max(1,min(get(D.hSeed,'Value'),numel(Rfull.labels))); roi2Orig=max(1,min(get(D.hROI2,'Value'),numel(Rfull.labels))); sliceVal=get(D.hSlice,'Value');
try, set(S.hFCRegion1,'Value',seedOrig); set(S.hFCRegion2,'Value',roi2Orig); set(S.hFCSlice,'Value',sliceVal); guidata(D.mainFig,S); catch, end
if strcmpi(dispMode,'Fisher z'), M0full=Rfull.meanZ; stack0full=Rfull.Zstack; clim=[-2.5 2.5]; valTxt='Fisher z'; else, M0full=Rfull.meanR; stack0full=Rfull.Rstack; clim=[-1 1]; valTxt='Pearson r'; end
switch lower(roiMode), case 'all rois', selIdx=1:numel(Rfull.labels); selNote=sprintf('ALL ROIs: %d regions',numel(selIdx)); case 'manual names selection', if isfield(S,'fcSelectedROIIdx'), selIdx=fc_region_keep_indices_GA_20260622(S.fcSelectedROIIdx,numel(Rfull.labels)); else, selIdx=[]; end; if isempty(selIdx), selIdx=1:numel(Rfull.labels); end; selNote=sprintf('manual/all: %d ROI(s)',numel(selIdx)); otherwise, [selIdx,selNote]=fcGASelectedOrTopROIIdx_20260622(S,Rfull,M0full,seedOrig,roi2Orig,20); if isempty(selIdx), selIdx=1:numel(Rfull.labels); end; end
R=Rfull; M0=M0full(selIdx,selIdx); stack0=stack0full(selIdx,selIdx,:); R.names=Rfull.names(selIdx); R.labels=Rfull.labels(selIdx); R.meanR=Rfull.meanR(selIdx,selIdx); R.meanZ=Rfull.meanZ(selIdx,selIdx); seedIdx=find(selIdx==seedOrig,1,'first'); if isempty(seedIdx), seedIdx=1; end; roi2Idx=find(selIdx==roi2Orig,1,'first'); if isempty(roi2Idx), roi2Idx=min(2,numel(selIdx)); end
cla(D.ax);
switch lower(viewMode)
case 'heatmap', [M,~,~,rowIdx,colIdx,hemiTitle]=applyHemisphereMode_ADV_20260617(M0,R.names,R.labels,hemiMode); plotFCMatrix_CLEAN_20260617(D.ax,M,clim,sprintf('%s FC heatmap LARGE | %s | %s',Rfull.groupName,hemiTitle,selNote),R.names(colIdx),R.names(rowIdx),R.labels(colIdx),R.labels(rowIdx),S.C,cmapName,labelMode,hemiTitle);
case {'seed profile ± sd','seed profile +/- sd','seed profile'}, plotSeedProfile_ADV_20260617(D.ax,stack0,seedIdx,1:size(stack0,2),R,valTxt,S.C,S);
case {'animal pair values','roi trace'}, plotAnimalPairValues_ADV_20260617(D.ax,stack0,seedIdx,roi2Idx,R,valTxt,S.C,S);
case {'roi pair summary','roi pair'}, plotROIPairSummary_ADV_20260617(D.ax,stack0,seedIdx,roi2Idx,R,valTxt,S.C,S);
case 'subject matrix', [M,~,~,rowIdx,colIdx,hemiTitle]=applyHemisphereMode_ADV_20260617(M0,R.names,R.labels,hemiMode); plotFCMatrix_CLEAN_20260617(D.ax,M,clim,sprintf('%s subject matrix LARGE | %s | %s',Rfull.groupName,hemiTitle,selNote),R.names(colIdx),R.names(rowIdx),R.labels(colIdx),R.labels(rowIdx),S.C,cmapName,labelMode,hemiTitle);
otherwise, guidata(f,S); plotROIOverlay_ADV_20260617(D.ax,S,Rfull,cmapName); try, set(findobj(D.ax,'Type','Image'),'ButtonDownFcn',@(src,evt)fcGALargeOverlayClick_20260622(f,src,evt)); catch, end
end
try, if ~get(D.hAutoY,'Value'), y0=str2double(get(D.hY0,'String')); y1=str2double(get(D.hY1,'String')); if isfinite(y0)&&isfinite(y1)&&y1>y0, ylim(D.ax,[y0 y1]); end, end, catch, end
end

function fcGALargeOverlayClick_20260622(f,src,evt)
try, D=getappdata(f,'D'); ax=ancestor(src,'axes'); labelSlice=getappdata(ax,'fcGAOverlayLabelSlice'); labs=getappdata(ax,'fcGAOverlayLabels'); cp=get(ax,'CurrentPoint'); x=round(cp(1,1)); y=round(cp(1,2)); if y<1||x<1||y>size(labelSlice,1)||x>size(labelSlice,2), return; end; lab=double(labelSlice(y,x)); if lab==0||~isfinite(lab), return; end; idx=find(labs==lab,1,'first'); if isempty(idx), idx=find(abs(labs)==abs(lab),1,'first'); end; if isempty(idx), return; end; set(D.hSeed,'Value',idx); fcGALargeReplot_20260622(f); catch, end
end
function fcGALargeScrollSlice_20260622(f,evt)
try, D=getappdata(f,'D'); items=get(D.hSlice,'String'); if ischar(items), items=cellstr(items); end; if numel(items)<2, return; end; v=get(D.hSlice,'Value'); v=max(1,min(numel(items),v+sign(evt.VerticalScrollCount))); set(D.hSlice,'Value',v); fcGALargeReplot_20260622(f); catch, end
end
function fcExportLargeView_GA_20260622(f)
try, [fn,pn]=uiputfile({'*.png','PNG image (*.png)'},'Export FC-GA large view',fullfile(pwd,'FCGA_large_view.png')); if isequal(fn,0), return; end; out=fullfile(pn,fn); try, exportgraphics(f,out,'Resolution',300); catch, print(f,out,'-dpng','-r300'); end; catch ME, errordlg(ME.message,'Export large view'); end
end
