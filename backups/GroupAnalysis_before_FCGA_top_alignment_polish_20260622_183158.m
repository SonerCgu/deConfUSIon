...
try
    ...
    % Shift FC matrices axes right to avoid y-label/tick overlap with panel border.
    try
        set(S.axFCA,'Units','normalized','Position',[0.085 0.110 0.780 0.825]);
    catch
    end

    % Status line: restored below FC matrices.
    ...
    % Improve label dropdown wording.
    try
    ...
    % ==============================================================
    % Upper FC box: symmetric rows, moved slightly lower inside box.
    % ==============================================================
    fsLab = 10.5;
    fsCtl = 10.0;
    fsBig = 11.5;
    ...
    % Row 1
    try, if isfield(S,'hFCLoad') && ishghandle(S.hFCLoad), set(S.hFCLoad,'Parent',pTop,'Units','normalized','Position',[0.020 0.585 0.135 0.300],'FontSize',10.5); end, catch, end
    try, if isfield(S,'hFCScan') && ishghandle(S.hFCScan), set(S.hFCScan,'Parent',pTop,'Units','normalized','Position',[0.165 0.585 0.110 0.300],'FontSize',10.5); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Group:',[0.310 0.655 0.060 0.170],fsLab);
    try, if isfield(S,'hFCGroupA') && ishghandle(S.hFCGroupA), set(S.hFCGroupA,'Parent',pTop,'Units','normalized','Position',[0.370 0.620 0.125 0.240],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Display:',[0.525 0.655 0.075 0.170],fsLab);
    try, if isfield(S,'hFCDisplay') && ishghandle(S.hFCDisplay), set(S.hFCDisplay,'Parent',pTop,'Units','normalized','Position',[0.600 0.620 0.130 0.240],'FontSize',fsCtl); end, catch, end

    try, if isfield(S,'hFCCompute') && ishghandle(S.hFCCompute), set(S.hFCCompute,'Parent',pTop,'Units','normalized','Position',[0.755 0.560 0.115 0.355],'String','Recompute','FontSize',fsBig,'FontWeight','bold'); end, catch, end
    try, if isfield(S,'hFCExport') && ishghandle(S.hFCExport), set(S.hFCExport,'Parent',pTop,'Units','normalized','Position',[0.880 0.560 0.105 0.355],'String','Export GA','FontSize',fsBig,'FontWeight','bold'); end, catch, end

    % Row 2
    fcGASetLabelPos_20260624(pTop,'Subject:',[0.020 0.340 0.075 0.160],fsLab);
    try, if isfield(S,'hFCSubject') && ishghandle(S.hFCSubject), set(S.hFCSubject,'Parent',pTop,'Units','normalized','Position',[0.095 0.305 0.170 0.240],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'View:',[0.290 0.340 0.055 0.160],fsLab);
    try, if isfield(S,'hFCView') && ishghandle(S.hFCView), set(S.hFCView,'Parent',pTop,'Units','normalized','Position',[0.345 0.305 0.145 0.240],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Hemi:',[0.515 0.340 0.055 0.160],fsLab);
    try, if isfield(S,'hFCHemi') && ishghandle(S.hFCHemi), set(S.hFCHemi,'Parent',pTop,'Units','normalized','Position',[0.570 0.305 0.130 0.240],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Color:',[0.720 0.340 0.060 0.160],fsLab);
    try, if isfield(S,'hFCColorMap') && ishghandle(S.hFCColorMap), set(S.hFCColorMap,'Parent',pTop,'Units','normalized','Position',[0.780 0.305 0.110 0.240],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Labels:',[0.900 0.340 0.065 0.160],fsLab);
    try, if isfield(S,'hFCLabelMode') && ishghandle(S.hFCLabelMode), set(S.hFCLabelMode,'Parent',pTop,'Units','normalized','Position',[0.965 0.305 0.025 0.240],'FontSize',fsCtl); end, catch, end

    % Labels dropdown needs more space; if the panel is narrow, move it a little left.
    try
        if isfield(S,'hFCLabelMode') && ishghandle(S.hFCLabelMode)
            set(S.hFCLabelMode,'Position',[0.935 0.305 0.055 0.240]);
            fcGASetLabelPos_20260624(pTop,'Labels:',[0.875 0.340 0.060 0.160],fsLab);
        end
    catch
    end

    % Row 3: moved left and aligned.
    fcGASetLabelPos_20260624(pTop,'Seed:',[0.020 0.085 0.050 0.160],fsLab);
    try, if isfield(S,'hFCRegion1') && ishghandle(S.hFCRegion1), set(S.hFCRegion1,'Parent',pTop,'Units','normalized','Position',[0.070 0.055 0.245 0.245],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'ROI 2:',[0.335 0.085 0.060 0.160],fsLab);
    try, if isfield(S,'hFCRegion2') && ishghandle(S.hFCRegion2), set(S.hFCRegion2,'Parent',pTop,'Units','normalized','Position',[0.395 0.055 0.250 0.245],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Slice:',[0.665 0.085 0.055 0.160],fsLab);
    try, if isfield(S,'hFCSlice') && ishghandle(S.hFCSlice), set(S.hFCSlice,'Parent',pTop,'Units','normalized','Position',[0.720 0.055 0.115 0.245],'FontSize',fsCtl); end, catch, end
    ...
    try, fprintf('FC-GA final polish warning: %s\n',ME.message); catch, end
end
end
