...
% GA_CMD_FIX_20260616_START
function S = assignBundleToRow(S,r,fp)
fp = strtrimSafe(fp); if isempty(fp) || r < 1 || r > size(S.subj,1), return; end
if size(S.subj,2) < 9, S.subj(:,end+1:9) = {''}; end
S.subj{r,1} = true;
if isempty(strtrimSafe(S.subj{r,2})), S.subj{r,2} = guessSubjectID(fp); end
if isempty(strtrimSafe(S.subj{r,5})), S.subj{r,5} = S.subj{r,2}; end
S = ensureFCRowFilesSizeV13(S);
if localGA_isFCBundleFile_CMD(fp) && ~localGA_isSCMBundleFile_CMD(fp)
    S.fcRowFiles{r,1} = fp;
else
    S.subj{r,8} = fp;
end
...
fp = strtrimSafe(fp); if isempty(fp), return; end
...
animalName = guessSubjectID(fp); g = 'PACAP'; c = 'CondA';
...
if strcmpi(ext,'.txt')
    row{7} = fp;
elseif localGA_isFCBundleFile_CMD(fp) && ~localGA_isSCMBundleFile_CMD(fp)
    % keep FC path separate below
elseif localGA_isSCMBundleFile_CMD(fp)
    row{8} = fp;
else
    row{6} = fp;
end
S.subj(end+1,1:9) = row;
S = ensureFCRowFilesSizeV13(S); newRow = size(S.subj,1);
if localGA_isFCBundleFile_CMD(fp) && ~localGA_isSCMBundleFile_CMD(fp), S.fcRowFiles{newRow,1} = fp; end
end

function tf = localGA_isFCBundleFile_CMD(fp)
tf = false; fp = strtrimSafe(fp); if isempty(fp), return; end
[~,nm,ext] = fileparts(fp);
if strcmpi(ext,'.mat') && ~isempty(regexpi(nm,'^FC_GroupBundle_|FC.*GroupBundle|GroupBundle.*FC','once')), tf = true; return; end
if exist(fp,'file') == 2
    try, info = whos('-file',fp); tf = any(strcmp({info.name},'fcBundle')); catch, tf = false; end
end
end

function tf = localGA_isSCMBundleFile_CMD(fp)
tf = false; fp = strtrimSafe(fp); if isempty(fp), return; end
[~,nm,ext] = fileparts(fp);
if strcmpi(ext,'.mat') && ~isempty(regexpi(nm,'SCM|GroupExport|SCM_Group|MapBundle','once')), tf = true; return; end
if exist(fp,'file') == 2
    try, info = whos('-file',fp); tf = any(strcmp({info.name},'G')); catch, tf = false; end
end
end

function subj = guessSubjectID(txt)
subj = ''; txt = strtrimSafe(txt);
try
    p = strrep(txt,'\','/'); parts = regexp(p,'[/]+','split');
    for kk = numel(parts):-1:1
        nm = strtrim(parts{kk}); low = lower(nm);
        if isempty(nm) || contains(low,'groupanalysis') || contains(low,'functionalconnectivity') || contains(low,'bundle') || contains(low,'github') || contains(low,'deconfusion'), continue; end
        [~,stem,ext] = fileparts(nm); if ~isempty(ext), nm = stem; end
        subj = nm; return;
    end
catch
end
try, [~,subj,~] = fileparts(txt); catch, subj = ''; end
if isempty(subj), subj = ['S' datestr(now,'HHMMSS')]; end
end

function s = displayScanID(scanID)
s = strtrimSafe(scanID); if isempty(s) || strcmpi(s,'N/A'), s = ''; return; end
s = regexprep(s,'(?i)^FUS_?',''); s = regexprep(s,'(?i)^SCAN','scan');
end

function s = simplifyFileLabel(fp)
fp = strtrimSafe(fp); if isempty(fp), s = ''; return; end
try, [~,bn,ext] = fileparts(fp); s = [bn ext]; catch, s = fp; end
if numel(s) > 38, s = [s(1:35) '...']; end
end

function s = bundlePresenceLabel(fp)
fp = strtrimSafe(fp); if isempty(fp), s = ''; elseif exist(fp,'file') == 2, s = 'Exists'; else, s = 'Missing'; end
end

function s = deriveRowStatusWithFCV13(row,fcFile)
roi=''; bundle=''; st=''; use=true; fcFile=strtrimSafe(fcFile);
try, roi=strtrimSafe(row{7}); catch, end; try, bundle=strtrimSafe(row{8}); catch, end; try, st=lower(strtrimSafe(row{9})); catch, end; try, use=logicalCellValue(row{1}); catch, end
if contains(st,'excluded'), s='Excluded'; elseif ~use, s='Not used'; elseif ~isempty(roi)||~isempty(bundle)||~isempty(fcFile), s='OK'; else, s='Not set'; end
end

function rows = getTargetRows(S)
rows = []; try, rows = clampSelRows(S.selectedRows,size(S.subj,1)); catch, end
if isempty(rows), try, if isfield(S,'applyAllIfNoneSelected') && S.applyAllIfNoneSelected, rows = find(logicalCol(S.subj,1)); end, catch, end, end
end

function c = mapConditionFromGroup(S,g)
c=''; g=strtrimSafe(g); try, if isfield(S,'groupToCondMap') && isa(S.groupToCondMap,'containers.Map'), key=upper(g); if isKey(S.groupToCondMap,key), c=S.groupToCondMap(key); return; end, end, catch, end
u=upper(g); if contains(u,'PACAP')||contains(u,'GROUPA'), c='CondA'; elseif contains(u,'VEH')||contains(u,'CONTROL')||contains(u,'GROUPB'), c='CondB'; end
end

function idx = findActiveROIRowsLocal(subj)
idx=[]; for ii=1:size(subj,1), try, if logicalCellValue(subj{ii,1}) && ~isempty(strtrimSafe(subj{ii,7})), idx(end+1)=ii; end, catch, end, end
end

function [idx,missingIdx] = findActiveBundleRowsLocal(S)
idx=[]; missingIdx=[]; for r=1:size(S.subj,1), try, use=logicalCellValue(S.subj{r,1}); catch, use=true; end; if ~use, continue; end; try, bf=strtrimSafe(S.subj{r,8}); catch, bf=''; end; if ~isempty(bf), if exist(bf,'file')==2, idx(end+1)=r; else, missingIdx(end+1)=r; end, end, end
end

function keys = makeRowKeysLocal(tbl)
keys=cell(size(tbl,1),1); for ii=1:size(tbl,1), a='';g='';c='';p=''; try,a=strtrimSafe(tbl{ii,2});catch,end; try,g=strtrimSafe(tbl{ii,3});catch,end; try,c=strtrimSafe(tbl{ii,4});catch,end; try,p=strtrimSafe(tbl{ii,5});catch,end; keys{ii}=lower([a '|' g '|' c '|' p]); end
end

function S = readROISettingsFromUI(S)
try,S.tc_computePSC=logical(get(S.hTC_ComputePSC,'Value'));catch,end; try,S.tc_baseMin0=safeNum(get(S.hBase0,'String'),S.tc_baseMin0);catch,end; try,S.tc_baseMin1=safeNum(get(S.hBase1,'String'),S.tc_baseMin1);catch,end; try,S.tc_injMin0=safeNum(get(S.hInj0,'String'),S.tc_injMin0);catch,end; try,S.tc_injMin1=safeNum(get(S.hInj1,'String'),S.tc_injMin1);catch,end; try,items=get(S.hTC_Metric,'String'); S.tc_metric=items{get(S.hTC_Metric,'Value')};catch,end
end

function S = readPreviewAxisControlsLocal(S)
try,S.plotTop.auto=logical(get(S.hTopAuto,'Value'));catch,end; try,S.plotTop.forceZero=logical(get(S.hTopZero,'Value'));catch,end; try,S.plotTop.step=max(0,safeNum(get(S.hTopStep,'String'),S.plotTop.step));catch,end; try,S.plotTop.ymin=safeNum(get(S.hTopYmin,'String'),S.plotTop.ymin);catch,end; try,S.plotTop.ymax=safeNum(get(S.hTopYmax,'String'),S.plotTop.ymax);catch,end; try,S.plotBot.auto=logical(get(S.hBotAuto,'Value'));catch,end; try,S.plotBot.forceZero=logical(get(S.hBotZero,'Value'));catch,end; try,S.plotBot.step=max(0,safeNum(get(S.hBotStep,'String'),S.plotBot.step));catch,end; try,S.plotBot.ymin=safeNum(get(S.hBotYmin,'String'),S.plotBot.ymin);catch,end; try,S.plotBot.ymax=safeNum(get(S.hBotYmax,'String'),S.plotBot.ymax);catch,end
end

function S = readMapSettingsFromUI(S)
try,items=get(S.hMapSummary,'String');S.mapSummary=items{get(S.hMapSummary,'Value')};catch,end; try,items=get(S.hMapSource,'String');S.mapSource=items{get(S.hMapSource,'Value')};catch,end; try,items=get(S.hMapColormap,'String');S.mapColormap=items{get(S.hMapColormap,'Value')};catch,end; try,S.mapBlackBody=logical(get(S.hMapBlackBody,'Value'));catch,end; try,items=get(S.hMapFlipMode,'String');S.mapFlipMode=items{get(S.hMapFlipMode,'Value')};catch,end; try,S.mapModMin=safeNum(get(S.hMapModMin,'String'),S.mapModMin);catch,end; try,S.mapModMax=safeNum(get(S.hMapModMax,'String'),S.mapModMax);catch,end; try,S.mapSigma=safeNum(get(S.hMapSigma,'String'),S.mapSigma);catch,end; try,ca=str2num(get(S.hMapCaxis,'String')); if numel(ca)>=2, S.mapCaxis=ca(1:2); end, catch, end; try,items=get(S.hMapPolarity,'String');S.mapPolarity=items{get(S.hMapPolarity,'Value')};catch,end; try,S.mapUseGlobalWindows=logical(get(S.hMapUseGlobalWin,'Value'));catch,end
end

function Rm = makeMapRenderStructLocal(S)
Rm=struct(); Rm.threshold=0; Rm.caxis=S.mapCaxis; Rm.alphaModOn=S.mapAlphaModOn; Rm.modMin=S.mapModMin; Rm.modMax=S.mapModMax; Rm.blackBody=S.mapBlackBody; Rm.colormapName=S.mapColormap; Rm.flipUDPreview=true; try,Rm.polarity=S.mapPolarity;catch,end
end

function D = buildCurrentMapDisplayLocal(S)
D=struct(); if isfield(S,'lastMapDisplay') && isstruct(S.lastMapDisplay) && isfield(S.lastMapDisplay,'map') && ~isempty(S.lastMapDisplay.map), D=S.lastMapDisplay; else, if isfield(S,'lastMAP') && isfield(S.lastMAP,'groupMap'), D.map=S.lastMAP.groupMap; else, D.map=[]; end; if isfield(S,'lastMAP') && isfield(S.lastMAP,'commonUnderlay'), D.underlay=S.lastMAP.commonUnderlay; else, D.underlay=zeros(size(D.map)); end; D.title='Group map'; end; D.render=makeMapRenderStructLocal(S);
end

function nZ = inferMapSliceCountFromBundleLocal(G)
nZ=1; f={'pscAtlas4D','psc4D','underlayAtlas','underlay3D','scmMapAtlas'}; for ii=1:numel(f), try,A=G.(f{ii}); if ndims(A)>=4, nZ=size(A,3); return; elseif ndims(A)==3 && size(A,3)~=3, nZ=size(A,3); return; end, catch, end, end
end

function z = defaultMapSliceFromBundleLocal(G,nZ)
if nargin<2 || isempty(nZ) || ~isfinite(nZ), nZ=inferMapSliceCountFromBundleLocal(G); end; z=max(1,round(nZ/2));
end

function [U,overlayMask,infoTxt] = GA_loadUnderlayAndOverlayMask_20260504(fp)
overlayMask=[]; infoTxt=simplifyFileLabel(fp); fp=strtrimSafe(fp); [~,~,ext]=fileparts(fp); if strcmpi(ext,'.mat'), L=load(fp); fn=fieldnames(L); U=[]; for ii=1:numel(fn), X=L.(fn{ii}); if isempty(U) && (isnumeric(X)||islogical(X)), U=X; elseif isstruct(X), if isempty(U) && isfield(X,'underlay2D'), U=X.underlay2D; end; if isfield(X,'mask'), overlayMask=X.mask; end; if isfield(X,'overlayMask'), overlayMask=X.overlayMask; end; end, end; if isempty(U), error('No numeric underlay found.'); end; else, U=imread(fp); end
end

function GA_scm_print_error(ME,whereTxt)
try,fprintf(2,'\nGA error in %s:\n%s\n',whereTxt,getReport(ME,'extended','hyperlinks','on'));catch,disp(ME.message);end
end

function outFile = localGA_motorExportIntegratedV6(kind,hFig)
outFile=''; try, S=guidata(hFig); D=buildCurrentMapDisplayLocal(S); %#ok<NASGU> catch, end; if nargin>=1 && strcmpi(kind,'ppt'), errordlg('PPT exporter helper is not available in this repaired GroupAnalysis.m. Use Export PNG / Export Data SCM.','GroupAnalysis PPT export'); end
end

function [FC,cache] = localGA_loadFCFilesV13(fileList,cache)
if nargin<2 || isempty(cache), cache=struct(); end; fileList=fileList(:);
try, [FC,cache]=callFC('loadFCGroupBundlesFromFiles',fileList,cache); try,FC.loaded=true;catch,end; return; catch ME1, try,disp(['FC module loader fallback: ' ME1.message]);catch,end, end
FC=struct(); FC.files=fileList; FC.subjects=struct([]); FC.nSubjects=0; FC.loaded=false; idx=0;
for ii=1:numel(fileList)
    fp=strtrimSafe(fileList{ii}); if isempty(fp)||exist(fp,'file')~=2, continue; end
    try
        L=load(fp); Slist=struct([]);
        if isfield(L,'fcBundle') && isstruct(L.fcBundle) && isfield(L.fcBundle,'subjects'), Slist=L.fcBundle.subjects; elseif isfield(L,'FC') && isstruct(L.FC) && isfield(L.FC,'subjects'), Slist=L.FC.subjects; end
        for jj=1:numel(Slist)
            subj0=Slist(jj); [M,Z,labs,nms]=localGA_fcSubjectMatrix_CMD(subj0); if isempty(M)||isempty(labs), continue; end
            idx=idx+1; FC.subjects(idx).sourceFile=fp;
            if isfield(subj0,'name') && ~isempty(subj0.name), FC.subjects(idx).name=strtrimSafe(subj0.name); else, FC.subjects(idx).name=guessSubjectID(fp); end
            if isfield(subj0,'group') && ~isempty(subj0.group), FC.subjects(idx).group=strtrimSafe(subj0.group); else, FC.subjects(idx).group=localGA_fcInferGroupV32(FC.subjects(idx)); end
            FC.subjects(idx).labels=double(labs(:)); FC.subjects(idx).names=nms(:); FC.subjects(idx).R=double(M); FC.subjects(idx).Z=double(Z);
            FC.subjects(idx).displayMatrix=FC.subjects(idx).R; FC.subjects(idx).displayZ=FC.subjects(idx).Z; FC.subjects(idx).displayNames=FC.subjects(idx).names; FC.subjects(idx).displayLabels=FC.subjects(idx).labels;
        end
    catch ME2, try,disp(['Skipping FC file: ' fp ' | ' ME2.message]);catch,end
    end
end
FC.nSubjects=idx; FC.loaded=idx>0;
end

function [M,Z,labs,nms] = localGA_fcSubjectMatrix_CMD(subj0)
M=[]; Z=[]; labs=[]; nms={};
if isfield(subj0,'labels'), labs=double(subj0.labels(:)); elseif isfield(subj0,'displayLabels'), labs=double(subj0.displayLabels(:)); end
if isfield(subj0,'R'), M=double(subj0.R); elseif isfield(subj0,'displayMatrix'), M=double(subj0.displayMatrix); elseif isfield(subj0,'corrMatrix'), M=double(subj0.corrMatrix); elseif isfield(subj0,'matrix'), M=double(subj0.matrix); end
if isfield(subj0,'Z'), Z=double(subj0.Z); elseif isfield(subj0,'displayZ'), Z=double(subj0.displayZ); end
if isempty(M) && ~isempty(Z), M=tanh(Z); end; if isempty(Z) && ~isempty(M), Z=atanh(max(-0.999999,min(0.999999,M))); end
if isempty(labs) && ~isempty(M), labs=(1:size(M,1))'; end
if isfield(subj0,'names') && ~isempty(subj0.names), nms=subj0.names(:); elseif isfield(subj0,'displayNames') && ~isempty(subj0.displayNames), nms=subj0.displayNames(:); else, nms=arrayfun(@(x)sprintf('ROI_%g',x),labs(:),'UniformOutput',false); end
end

function S = localGA_buildFCFromTableRowsV13(S)
S=ensureFCRowFilesSizeV13(S); files=S.fcRowFiles(~cellfun(@isempty,S.fcRowFiles)); [FC,cacheOut]=localGA_loadFCFilesV13(files,S.cache); S.cache=cacheOut; S.FC=FC; S.FC.loaded=isfield(FC,'subjects') && ~isempty(FC.subjects);
end

function S = localGA_autoAttachFCFilesToRowsV13(S,FC)
S=ensureFCRowFilesSizeV13(S); if ~isfield(FC,'subjects')||isempty(FC.subjects), return; end
for ii=1:numel(FC.subjects), fp=''; name=''; grp=''; try,fp=strtrimSafe(FC.subjects(ii).sourceFile);catch,end; try,name=strtrimSafe(FC.subjects(ii).name);catch,end; if isempty(name), name=guessSubjectID(fp); end; try,grp=strtrimSafe(FC.subjects(ii).group);catch,end; if isempty(grp), grp=localGA_fcInferGroupV32(FC.subjects(ii)); end; hit=[]; for r=1:size(S.subj,1), if strcmpi(strtrimSafe(S.subj{r,2}),name), hit=r; break; end, end; if isempty(hit), cond=mapConditionFromGroup(S,grp); if isempty(cond), cond='CondA'; end; S.subj(end+1,1:9)={true,name,grp,cond,name,'','','',''}; S=ensureFCRowFilesSizeV13(S); hit=size(S.subj,1); end; S.fcRowFiles{hit,1}=fp; if ~isempty(grp), S.subj{hit,3}=grp; end; end
end

function S = localGA_autoFCAtlasFileV13(S,verboseFlag)
if nargin<2, verboseFlag=false; end %#ok<NASGU>
if ~isfield(S,'fcAtlasLabelFile'), S.fcAtlasLabelFile=''; end
end

function S = refreshFCRegionPopupsV15(S)
try, if isfield(S,'FC') && isfield(S.FC,'subjects') && ~isempty(S.FC.subjects), names=S.FC.subjects(1).names; labs=S.FC.subjects(1).labels; items=cell(numel(labs),1); for ii=1:numel(labs), items{ii}=sprintf('%g  %s',labs(ii),strtrimSafe(names{ii})); end; if isfield(S,'hFCRegion1') && isgraphics(S.hFCRegion1), set(S.hFCRegion1,'String',items,'Value',1); end; if isfield(S,'hFCRegion2') && isgraphics(S.hFCRegion2), set(S.hFCRegion2,'String',items,'Value',min(2,numel(items))); end; end, catch, end
end

function val = localGA_fcPopupV32(S,fieldName,fb)
val = localGA_fcPopupV31(S,fieldName,fb);
end
function C = localGA_fcColorsV32(), C = localGA_fcColorsV31(); end
function localGA_fcTextV32(ax,msg,C), localGA_fcTextV31(ax,msg,C); end
function localGA_fcSetInfoV32(S,msg), localGA_fcSetInfoV31(S,msg); end
function localGA_fcPlotViewV32(ax,FC,R,viewMode,activeGroup,hemiMode,dispMode,reg1,reg2,thr,C), localGA_fcPlotViewV31(ax,FC,R,viewMode,activeGroup,hemiMode,dispMode,reg1,reg2,thr,C); end
function s = localGA_fcStrV32(x), s = localGA_fcStrV31(x); end
function u = localGA_fcUniqueV32(c), u = localGA_fcUniqueV31(c); end
function M = localGA_fcMean3V32(X), M = localGA_fcMean3V31(X); end
function g = localGA_fcInferGroupV32(subj)
g=''; txt=''; try,txt=[txt ' ' strtrimSafe(subj.name)];catch,end; try,txt=[txt ' ' strtrimSafe(subj.sourceFile)];catch,end; u=upper(txt); if contains(u,'PACAP')||contains(u,'GROUPA')||contains(u,'CONDA'), g='PACAP'; elseif contains(u,'VEH')||contains(u,'CONTROL')||contains(u,'PBS')||contains(u,'ACSF')||contains(u,'GROUPB')||contains(u,'CONDB'), g='Vehicle'; else, g='Unassigned'; end
end
function v = localGA_fcGetNumericVectorV32(S,names)
v=[]; for ii=1:numel(names), try, if isfield(S,names{ii}) && ~isempty(S.(names{ii})), v=double(S.(names{ii})(:)); return; end, catch, end, end
end
function M = localGA_fcGetNumericMatrixV32(S,names)
M=[]; for ii=1:numel(names), try, if isfield(S,names{ii}) && isnumeric(S.(names{ii})) && ndims(S.(names{ii}))==2, M=double(S.(names{ii})); return; end, catch, end, end
end
% GA_CMD_FIX_20260616_END
...
% GA_CMD_FIX: corrected table row layout. Col1=Use, Col2=Animal, Col8=SCM bundle.
try
    fp = strtrimSafe(bundleFile);
    if isempty(fp), return; end
    if ~isfield(S,'subj') || isempty(S.subj) || ~iscell(S.subj), S.subj = cell(0,9); end
    if size(S.subj,2) < 9, S.subj(:,end+1:9) = {''}; elseif size(S.subj,2) > 9, S.subj = S.subj(:,1:9); end
    animalName = guessSubjectID(fp);
    g = 'PACAP'; c = 'CondA';
    try, if isfield(S,'defaultGroup') && ~isempty(S.defaultGroup), g = S.defaultGroup; end, catch, end
    try, if isfield(S,'defaultCond')  && ~isempty(S.defaultCond),  c = S.defaultCond;  end, catch, end
    row = {true,animalName,g,c,animalName,'','','',''};
    isFC = localGA_isFCBundleFile_CMD(fp);
    isSCM = localGA_isSCMBundleFile_CMD(fp);
    if isFC && ~isSCM
        % FC bundle: attach to fcRowFiles only, do not pollute SCM bundle column.
    else
        row{8} = fp;
    end
    S.subj(end+1,1:9) = row;
    ...
    try, [d,~,~] = fileparts(fp); if exist(d,'dir') == 7, S.lastBundleDir=d; S.lastFCBundleDir=d; S.lastBrowseDir=d; end, catch, end
catch ME
...
% GA_SESSION_PARSE_CMD_FIX
    tokS = regexp(joined,'(?i)(Session[_ -]?0*\d+|S\d+)','tokens','once');
    if ~isempty(tokS)
        meta.session = tokS{1};
        meta.sessionID = meta.session;
        meta.sessionId = meta.session;
    end
    if isempty(meta.session)
        meta.session = ''; meta.sessionID = ''; meta.sessionId = '';
    end
