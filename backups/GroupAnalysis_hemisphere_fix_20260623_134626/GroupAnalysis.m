...
% Correct FC-GA hemisphere handling.
% All: full matrix.
% Left: left x left.
% Right: right x right.
% Left vs Right: left rows x right columns.
% Merged: L/R homologues collapsed into one base ROI, e.g. L_ALV/R_ALV -> ALV.
try
    n = size(M,1);
    ...
    % Merged / bilateral must be checked before Left/Right.
    ...
    if strcmp(mode,'left') || strcmp(mode,'l') || ~isempty(strfind(mode,'left hemisphere'))
        if any(isL)
            rowIdx = find(isL);
            colIdx = find(isL);
            M2 = M(rowIdx,colIdx);
    ...
    if strcmp(mode,'right') || strcmp(mode,'r') || ~isempty(strfind(mode,'right hemisphere'))
        if any(isR)
            rowIdx = find(isR);
            colIdx = find(isR);
            M2 = M(rowIdx,colIdx);
...
try
    ...
    base = regexprep(base,'^(L|R|LEFT|RIGHT)_','');
