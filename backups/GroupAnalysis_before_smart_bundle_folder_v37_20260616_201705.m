...
% Restored helper for Add Bundles / file-browser start path.
% Returns a valid existing folder. Never errors.
...
try
    ...
    % Recently used / explicitly stored folders.
    ...
    for ii = 1:numel(fields)
        try
            if isfield(S,fields{ii}) && ~isempty(S.(fields{ii}))
                candidates{end+1,1} = getSmartBrowseDir_str(S.(fields{ii}));
            end
        catch
        end
    end

    % FC row files / loaded bundle files.
    try
        if isfield(S,'fcRowFiles') && ~isempty(S.fcRowFiles)
            for ii = 1:numel(S.fcRowFiles)
                p = getSmartBrowseDir_str(S.fcRowFiles{ii});
                if ~isempty(p)
                    [d,~,~] = fileparts(p);
                    candidates{end+1,1} = d;
                end
            end
        end
    catch
    ...
    try
        if isfield(S,'FC') && isstruct(S.FC) && isfield(S.FC,'files') && ~isempty(S.FC.files)
            for ii = 1:numel(S.FC.files)
                p = getSmartBrowseDir_str(S.FC.files{ii});
                if ~isempty(p)
                    [d,~,~] = fileparts(p);
                    candidates{end+1,1} = d;
                end
    ...
    % Main table: original GroupAnalysis uses column 8 as Bundle File.
    try
        if isfield(S,'subj') && iscell(S.subj) && ~isempty(S.subj)
            nRows = size(S.subj,1);
            nCols = size(S.subj,2);
            cols = [];
            if nCols >= 8, cols = 8; end
            cols = unique([cols 1:nCols]);

            for r = 1:nRows
                for cc = cols
                    p = getSmartBrowseDir_str(S.subj{r,cc});
                    if isempty(p), continue; end
                    if exist(p,'dir') == 7
                        candidates{end+1,1} = p;
                    else
                        [d,~,~] = fileparts(p);
    ...
    % Current folder and toolbox root fallbacks.
    try, candidates{end+1,1} = pwd; catch, end
    try, candidates{end+1,1} = fileparts(mfilename('fullpath')); catch, end

    for ii = 1:numel(candidates)
        p = getSmartBrowseDir_str(candidates{ii});
        if getSmartBrowseDir_isdir(p)
            startPath = p;
            return;
        end
    end
...
function s = getSmartBrowseDir_str(x)
s = '';
try
    if nargin < 1 || isempty(x)
        return;
    elseif ischar(x)
        s = strtrim(x);
    elseif iscell(x)
        if ~isempty(x), s = getSmartBrowseDir_str(x{1}); end
    elseif isnumeric(x) || islogical(x)
        if isscalar(x), s = strtrim(num2str(x)); else, s = strtrim(mat2str(x)); end
    else
        try, s = strtrim(char(x)); catch, s = ''; end
...
function tf = getSmartBrowseDir_isdir(p)
tf = false;
try
