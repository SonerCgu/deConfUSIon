...
pFCTop = uipanel(pFCBG,'Units','normalized','Position',[0.02 0.835 0.96 0.155], ...
    'Title','Functional Connectivity group analysis', ...
    'BackgroundColor',bg2,'ForegroundColor','w','FontWeight','bold');

S.hFCLoad = mkBtn(pFCTop,'Load FC Bundles',[0.010 0.58 0.130 0.30],C.btnAction,@onLoadFCGroupBundles);
S.hFCScan = mkBtn(pFCTop,'Scan Folder',[0.150 0.58 0.105 0.30],C.btnSecondary,@onScanFCGroupFolder);
S.hFCLoadAtlas = mkBtn(pFCTop,'Atlas TXT/Auto',[0.265 0.58 0.120 0.30],C.btnSecondary,@onLoadFCAtlasLabels);

uicontrol(pFCTop,'Style','text','String','Group A:', ...
    'Units','normalized','Position',[0.405 0.66 0.060 0.18], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCGroupA = uicontrol(pFCTop,'Style','popupmenu','String',S.groupList, ...
    'Units','normalized','Position',[0.470 0.62 0.105 0.25], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w');

uicontrol(pFCTop,'Style','text','String','Group B:', ...
    'Units','normalized','Position',[0.585 0.66 0.060 0.18], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCGroupB = uicontrol(pFCTop,'Style','popupmenu','String',S.groupList, ...
    'Units','normalized','Position',[0.650 0.62 0.105 0.25], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w');

uicontrol(pFCTop,'Style','text','String','Units:', ...
    'Units','normalized','Position',[0.765 0.66 0.050 0.18], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCDisplay = uicontrol(pFCTop,'Style','popupmenu','String',{'Pearson r','Fisher z'}, ...
    'Units','normalized','Position',[0.815 0.62 0.095 0.25], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'TooltipString','Display units: Pearson r or Fisher z.', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Hide |r|<:', ...
    'Units','normalized','Position',[0.010 0.18 0.075 0.18], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCThreshold = uicontrol(pFCTop,'Style','edit','String','0', ...
    'Units','normalized','Position',[0.090 0.14 0.055 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'TooltipString','Visual threshold only. Connections with absolute r below this value are hidden in display/export figures; statistics are not recomputed.', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','View:', ...
    'Units','normalized','Position',[0.160 0.18 0.045 0.18], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCView = uicontrol(pFCTop,'Style','popupmenu', ...
    ...
    'Units','normalized','Position',[0.205 0.14 0.140 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Seed/region:', ...
    'Units','normalized','Position',[0.360 0.18 0.085 0.18], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCSeedRegion = uicontrol(pFCTop,'Style','edit','String','', ...
    'Units','normalized','Position',[0.450 0.14 0.160 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'TooltipString','Enter atlas label number or part of region name. Empty = first ROI.', ...
    'Callback',@updateFCTabPreview);

S.hFCCompute = mkBtn(pFCTop,'Compute Group FC',[0.635 0.10 0.150 0.30],C.btnPrimary,@onComputeGroupFC);
S.hFCExport  = mkBtn(pFCTop,'Export All FC',[0.800 0.10 0.135 0.30],C.btnAction,@onExportGroupFC);
...
pFCAx = uipanel(pFCBG,'Units','normalized','Position',[0.02 0.02 0.96 0.760], ...
    'Title','FC matrices', ...
    'BackgroundColor',bg2,'ForegroundColor','w','FontWeight','bold');
...
S0.activeTab = 'MAP';
    ...
    function onComputeGroupFC(~,~)
        ...
        try
            gAll = cell(numel(S0.FC.subjects),1);
            for ii=1:numel(S0.FC.subjects), gAll{ii}=strtrimSafe(S0.FC.subjects(ii).group); end
            gAll = uniqueStable(gAll);
            if isempty(groupA) || ~any(strcmpi(gAll,groupA)), groupA = gAll{1}; end
            if isempty(groupB) || strcmpi(groupA,groupB) || ~any(strcmpi(gAll,groupB)), groupB = ''; end
        catch
        end
        setStatus(false);
    ...
    function updateFCTabPreview(~,~)
        ...
        try
            callFC('fcPlotFCAdvancedViewV13',viewMode,S0.axFCA,S0.axFCB,S0.axFCD,S0.axFCP,S0.FC,R,seedText,dispMode,thr,S0.C);
        catch ME
            try, GA_printErrorLocal(ME,'caught error in GroupAnalysis.m'); catch, end
