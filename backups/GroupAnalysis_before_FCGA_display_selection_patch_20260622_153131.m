...
try
    ...
    S.hFCColorMap = uicontrol(pFCTop,'Style','popupmenu', ...
        'String',{'Blue-White','Blue-White-Red','Red-White-Blue','parula','hot','jet','gray','blackbody'}, ...
...
cmapName = popupString_SINGLE_20260616(S,'hFCColorMap','Blue-White');
thr = 0;
try, thr = safeNum(get(S.hFCThreshold,'String'),0); catch, end
if strcmpi(dispMode,'Fisher z')
...
switch lower(viewMode)
    case 'heatmap'
        plotFCMatrix_CLEAN_20260617(S.axFCA,M,clim,sprintf('%s FC heatmap | %s | %s | n=%d',R.groupName,hemiTitle,valTxt,R.n),names,names,labels,labels,S.C,cmapName,labelMode,hemiTitle);
    case {'seed profile ± sd','seed profile +/- sd','seed profile'}
        plotSeedProfile_ADV_20260617(S.axFCA,stack0,seedIdx0,rowIdx,R,valTxt,S.C,S);
    ...
    case 'subject matrix'
        if subjIdx <= 1
            plotFCMatrix_CLEAN_20260617(S.axFCA,M,clim,sprintf('%s mean subject matrix | %s | n=%d',R.groupName,hemiTitle,R.n),names,names,labels,labels,S.C,cmapName,labelMode,hemiTitle);
        else
            si = max(1,min(subjIdx-1,size(stack0,3)));
            Ms0 = stack0(:,:,si);
            if thr > 0, Ms0(abs(Ms0) < thr) = 0; end
            [Ms,nms,lbs,~,~,ht] = applyHemisphereMode_ADV_20260617(Ms0,R.names,R.labels,hemiMode);
            plotFCMatrix_CLEAN_20260617(S.axFCA,Ms,clim,sprintf('Subject matrix: %s | %s',strtrimSafe(R.subjectNames{si}),ht),nms,nms,lbs,lbs,S.C,cmapName,labelMode,ht);
        end
    otherwise
...
title(ax,sprintf('Seed profile mean ± SD: %s | n=%d',fcNiceName_SAFE_20260617(R.names{seedIdx},R.labels(seedIdx),'Full',false),size(stack,3)),'Color',C.txt,'FontWeight','bold','Interpreter','none');
...
if isempty(map2)
    ...
    text(ax,0.5,0.45,'Re-export FC bundles after this patch so roiAtlas/labelMap is saved.', 'Color',S.C.muted,'HorizontalAlignment','center');
...
try, colormap(ax,cmapFC_ADV_20260617(cmapName,256)); catch, colormap(ax,jet(256)); end
cb = colorbar(ax); try, set(cb,'Color',S.C.txt); catch, end
...
if strcmpi(labelMode,'Full') && ~isempty(full)
    base = full;
elseif strcmpi(labelMode,'Full')
    base = fcFullName_SAFE_20260617(acr);
else
    base = acr;
end
...
maxN = 12; if strcmpi(labelMode,'Full'), maxN = 38; end
