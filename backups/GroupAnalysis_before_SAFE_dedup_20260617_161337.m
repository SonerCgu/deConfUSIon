...
% Safe readable FC heatmap plotter.
...
imagesc(ax,M);
try, caxis(ax,clim); catch, end
try, colormap(ax,cmapFC_SAFE_20260617(cmapName,256)); catch, colormap(ax,jet(256)); end
cb = colorbar(ax);
...
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted); grid(ax,'on');
title(ax,sprintf('Seed profile mean ± SD: %s | n=%d',fcNiceName_SAFE_20260617(R.names{seedIdx},R.labels(seedIdx),'Abbrev',false),size(stack,3)),'Color',C.txt,'FontWeight','bold','Interpreter','none');
...
v = vals(isfinite(vals)); if ~isempty(v), plot(ax,[min(x) max(x)],[mean(v) mean(v)],'-','LineWidth',2.2); end
hold(ax,'off');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted); grid(ax,'on');
title(ax,sprintf('Animal FC values: %s ↔ %s',fcNiceName_SAFE_20260617(R.names{seedIdx},R.labels(seedIdx),'Abbrev',false),fcNiceName_SAFE_20260617(R.names{roi2Idx},R.labels(roi2Idx),'Abbrev',false)),'Color',C.txt,'FontWeight','bold','Interpreter','none');
...
% Dot plot with mean line and SD whisker, like simple publication summary plots.
try, delete(findall(ancestor(ax,'figure'),'Type','ColorBar')); catch, end
cla(ax);
...
plot(ax,xj,vals,'o','MarkerSize',8,'LineWidth',1.6);
if isfinite(mu), plot(ax,[0.82 1.18],[mu mu],'-','LineWidth',3.0); end
if n > 1 && isfinite(sd)
...
hold(ax,'off');
xlim(ax,[0.55 1.45]);
set(ax,'XTick',1,'XTickLabel',{'FC value'},'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',11); grid(ax,'on');
title(ax,sprintf('ROI pair summary: %s ↔ %s | n=%d',fcNiceName_SAFE_20260617(R.names{seedIdx},R.labels(seedIdx),'Abbrev',false),fcNiceName_SAFE_20260617(R.names{roi2Idx},R.labels(roi2Idx),'Abbrev',false),n),'Color',C.txt,'FontWeight','bold','Interpreter','none');
ylabel(ax,valTxt,'Color',C.txt);
fcApplyY_SAFE_20260617(ax,S);
end
...
try
    ...
    items = cell(numel(labels),1);
    for ii = 1:numel(labels)
        items{ii} = fcNiceName_SAFE_20260617(names{ii},labels(ii),'Abbrev',false);
    end
    if isempty(items), items = {'No ROI labels'}; end
    if isfield(S,'hFCRegion1') && ishghandle(S.hFCRegion1), set(S.hFCRegion1,'String',items,'Value',min(get(S.hFCRegion1,'Value'),numel(items))); end
    if isfield(S,'hFCRegion2') && ishghandle(S.hFCRegion2), set(S.hFCRegion2,'String',items,'Value',min(max(get(S.hFCRegion2,'Value'),2),numel(items))); end
...
try
    ...
    if ok
        ...
        S.lastFC = struct();
        guidata(hFig,S);
        try, S = guidata(hFig); updateFCTabPreview([],[]); catch, end
    end
catch ME
...
try
    ...
    items = cell(numel(labels),1);
    for ii = 1:numel(labels)
        items{ii} = sprintf('%g | %s',labels(ii),fcNiceName_SAFE_20260617(names{ii},labels(ii),'Full',false));
    end
    init = 1:numel(items);
...
try
    ...
    for ii = 1:numel(labels)
        lines{ii} = sprintf('%g = %s',labels(ii),fcNiceName_SAFE_20260617(names{ii},labels(ii),'Full',false));
    end
...
acr = fcAcr_SAFE_20260617(raw);
if strcmpi(labelMode,'Full'), base = fcFullName_SAFE_20260617(acr); else, base = acr; end
side = fcSide_SAFE_20260617(raw,label);
if ~stripSide && ~isempty(side), base = [side '_' base]; end
base = regexprep(base,'\s+',' ');
base = strrep(base,sprintf('\n'),' ');
maxN = 10; if strcmpi(labelMode,'Full'), maxN = 22; end
