...
% Enrich FC group-analysis bundle with spatial maps and slice-specific matrices when available.
try
...
try
    ...
    for vv = 1:numel(vars)
        ...
        [map2,note] = deConfUSIon_FCGA_findMapRecursive_SAFE_20260617(X,vars(vv).name,0);
    ...
    if isstruct(X)
        ...
        for i = 1:numel(fn)
            ...
            if any(strcmp(fn{i},keys)) && (isnumeric(val) || islogical(val))
                map2 = squeeze(double(val)); note = [prefix '.' fn{i}];
                if ndims(map2) > 2, map2 = map2(:,:,round(size(map2,3)/2)); end
                return;
            end
        end
        for i = 1:numel(fn)
            val = X.(fn{i});
            if isstruct(val)
                [map2,note] = deConfUSIon_FCGA_findMapRecursive_SAFE_20260617(val,[prefix '.' fn{i}],depth+1);
                if ~isempty(map2), return; end
            end
        end
    end
...
try
    ...
    for vv = 1:numel(vars)
        if vars(vv).bytes > 5e8, continue; end
        X = evalin('caller',vars(vv).name);
        sr = deConfUSIon_FCGA_findSliceRecursive_SAFE_20260617(X,0);
        if ~isempty(sr), sliceResults = sr; return; end
    end
catch
end
end
...
sr = []; if depth > 3, return; end
try
    if isstruct(X)
        if isfield(X,'sliceResults') && isstruct(X.sliceResults) && ~isempty(X.sliceResults)
            sr = X.sliceResults; return;
        end
        if numel(X) > 1 && (isfield(X,'R') || isfield(X,'Z') || isfield(X,'displayMatrix'))
            sr = X; return;
        end
        if numel(X) > 1, X = X(1); end
        fn = fieldnames(X);
        for i = 1:numel(fn)
            val = X.(fn{i});
            if isstruct(val)
                sr = deConfUSIon_FCGA_findSliceRecursive_SAFE_20260617(val,depth+1);
