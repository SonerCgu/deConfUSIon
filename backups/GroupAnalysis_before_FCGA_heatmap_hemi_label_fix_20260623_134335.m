...
% Robust hemisphere modes for FC-GA after ROI sorting/subsetting.
% Supports: All, Left, Right, Left vs Right, Merged/Bilateral.
try
    n = size(M,1);
    ...
    [isL,isR,baseNames] = fcGAHemiMask_20260624(names,labels);

    % Merged/bilateral: collapse homologous L/R ROIs by base region name.
    if ~isempty(strfind(mode,'merged')) || ~isempty(strfind(mode,'bilateral'))
        [M2,names2,labels2] = fcGAMergeHemisphereMatrix_20260624(M,names,labels,baseNames);
        rowIdx = 1:size(M2,1);
        colIdx = 1:size(M2,2);
    ...
    % Left-vs-right rectangular matrix.
    if ~isempty(strfind(mode,'left vs right')) || ~isempty(strfind(mode,'left-right')) || ~isempty(strfind(mode,'l vs r'))
        if any(isL) && any(isR)
            rowIdx = find(isL);
            colIdx = find(isR);
            M2 = M(rowIdx,colIdx);
    ...
    % Left only.
    if strcmpi(mode,'left') || strcmpi(mode,'l') || ~isempty(strfind(mode,'left hemisphere'))
        if any(isL)
            rowIdx = find(isL);
            colIdx = find(isL);
            M2 = M(rowIdx,colIdx);
    ...
    % Right only.
    if strcmpi(mode,'right') || strcmpi(mode,'r') || ~isempty(strfind(mode,'right hemisphere'))
        if any(isR)
            rowIdx = find(isR);
            colIdx = find(isR);
            M2 = M(rowIdx,colIdx);
    ...
    try, fprintf('FC-GA hemisphere mode warning: %s\n',ME.message); catch, end
    M2 = M;
    names2 = names;
    labels2 = labels;
    rowIdx = 1:size(M,1);
...
% Infer left/right robustly from R_/L_, right/left words, or signed labels.
n = numel(labels);
isL = false(n,1);
isR = false(n,1);
baseNames = cell(n,1);
...
% If only one side was found by names, infer the complement as the other side.
if any(isL) && ~any(isR)
    isR = ~isL;
end
if any(isR) && ~any(isL)
...
try
    ...
    if ~isempty(regexp(su,'^L_','once')) || ~isempty(regexp(su,'^LEFT_','once'))
        side = 'L';
    elseif ~isempty(regexp(su,'^R_','once')) || ~isempty(regexp(su,'^RIGHT_','once'))
        side = 'R';
    elseif ~isempty(regexp(su,'_L$','once')) || ~isempty(regexp(su,'_LEFT$','once'))
        side = 'L';
    elseif ~isempty(regexp(su,'_R$','once')) || ~isempty(regexp(su,'_RIGHT$','once'))
        side = 'R';
    elseif ~isempty(regexp(su,'(^|_)L($|_)','once')) || ~isempty(regexp(su,'(^|_)LEFT($|_)','once'))
        side = 'L';
    elseif ~isempty(regexp(su,'(^|_)R($|_)','once')) || ~isempty(regexp(su,'(^|_)RIGHT($|_)','once'))
        side = 'R';
    end

    base = su;
    base = regexprep(base,'^L_','');
    base = regexprep(base,'^R_','');
    base = regexprep(base,'^LEFT_','');
    base = regexprep(base,'^RIGHT_','');
    base = regexprep(base,'_L$','');
    base = regexprep(base,'_R$','');
    base = regexprep(base,'_LEFT$','');
    base = regexprep(base,'_RIGHT$','');
    base = regexprep(base,'_+','_');
    base = strtrim(base);
    if isempty(base), base = su; end
catch
...
% Average homologous left/right ROIs into base-region matrix.
n = size(M,1);
keys = baseNames(:);
for ii = 1:numel(keys)
    if isempty(keys{ii}), keys{ii} = sprintf('ROI_%03d',ii); end
end
[keysSorted,ord] = sort(lower(keys));
baseSorted = keys(ord);
unq = {};
for ii = 1:numel(baseSorted)
    if isempty(unq) || ~strcmpi(unq{end},baseSorted{ii})
        unq{end+1,1} = baseSorted{ii}; %#ok<AGROW>
    end
end
k = numel(unq);
Mmerge = NaN(k,k);
