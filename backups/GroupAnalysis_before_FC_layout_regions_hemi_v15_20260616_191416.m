...
pFCTop = uipanel(pFCBG,'Units','normalized','Position',[0.02 0.790 0.96 0.200], ...
    'Title','Functional Connectivity group analysis', ...
    'BackgroundColor',bg2,'ForegroundColor','w','FontWeight','bold');

S.hFCLoad = mkBtn(pFCTop,'Load FC Bundles',[0.010 0.62 0.120 0.28],C.btnAction,@onLoadFCGroupBundles);
S.hFCScan = mkBtn(pFCTop,'Scan Folder',[0.140 0.62 0.100 0.28],C.btnSecondary,@onScanFCGroupFolder);
S.hFCLoadAtlas = mkBtn(pFCTop,'Atlas TXT/Auto',[0.250 0.62 0.120 0.28],C.btnSecondary,@onLoadFCAtlasLabels);

uicontrol(pFCTop,'Style','text','String','Group A:', ...
    'Units','normalized','Position',[0.390 0.70 0.060 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCGroupA = uicontrol(pFCTop,'Style','popupmenu','String',S.groupList, ...
    'Units','normalized','Position',[0.450 0.64 0.085 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w');

uicontrol(pFCTop,'Style','text','String','Group B:', ...
    'Units','normalized','Position',[0.545 0.70 0.060 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCGroupB = uicontrol(pFCTop,'Style','popupmenu','String',S.groupList, ...
    'Units','normalized','Position',[0.605 0.64 0.085 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w');

S.hFCCompute = mkBtn(pFCTop,'Compute FC',[0.705 0.62 0.115 0.28],C.btnPrimary,@onComputeGroupFC);
S.hFCExport  = mkBtn(pFCTop,'Export All FC',[0.835 0.62 0.120 0.28],C.btnAction,@onExportGroupFC);

uicontrol(pFCTop,'Style','text','String','View:', ...
    'Units','normalized','Position',[0.010 0.235 0.045 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCView = uicontrol(pFCTop,'Style','popupmenu', ...
    'String',{'Matrix summary','Seed profile','Max connections','ROI time course','Subject heatmap'}, ...
    'Units','normalized','Position',[0.055 0.185 0.140 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Show:', ...
    'Units','normalized','Position',[0.205 0.235 0.050 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCActiveGroup = uicontrol(pFCTop,'Style','popupmenu','String',{'Group A'}, ...
    'Units','normalized','Position',[0.255 0.185 0.120 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'TooltipString','Choose which group or difference to display as one large plot.', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Units:', ...
    'Units','normalized','Position',[0.385 0.235 0.050 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCDisplay = uicontrol(pFCTop,'Style','popupmenu','String',{'Pearson r','Fisher z'}, ...
    'Units','normalized','Position',[0.435 0.185 0.090 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'TooltipString','Display units only. Pearson r is easier to interpret; Fisher z is used for statistics.', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Hide |r|<:', ...
    'Units','normalized','Position',[0.535 0.235 0.075 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCThreshold = uicontrol(pFCTop,'Style','edit','String','0', ...
    'Units','normalized','Position',[0.610 0.185 0.055 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'TooltipString','Visual threshold only. Connections with |r| below this are hidden in plots/export figures.', ...
    'Callback',@updateFCTabPreview);

uicontrol(pFCTop,'Style','text','String','Seed/region:', ...
    'Units','normalized','Position',[0.675 0.235 0.090 0.16], ...
    'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left','FontWeight','bold');
S.hFCSeedRegion = uicontrol(pFCTop,'Style','edit','String','', ...
    'Units','normalized','Position',[0.765 0.185 0.095 0.24], ...
    'BackgroundColor',C.editBg,'ForegroundColor','w', ...
    'TooltipString','Enter atlas label number, e.g. 105, or part of a region name. Use Pick Region to choose from available ROIs.', ...
    'Callback',@updateFCTabPreview);

S.hFCPickRegion = mkBtn(pFCTop,'Pick Region',[0.870 0.165 0.085 0.28],C.btnSecondary,@onPickFCRegionV14);
...
pFCAx = uipanel(pFCBG,'Units','normalized','Position',[0.02 0.02 0.96 0.725], ...
    'Title','FC matrices', ...
    'BackgroundColor',bg2,'ForegroundColor','w','FontWeight','bold');

S.axFCA = axes('Parent',pFCAx,'Units','normalized','Position',[0.065 0.105 0.875 0.835]);
...
S0.activeTab = 'MAP';
    ...
    function refreshFCGroupPopups()
        ...
        try, setPopupToString(S0.hFCGroupA,'PACAP'); catch, end
        try, setPopupToString(S0.hFCGroupB,'Vehicle'); catch, end
        try
    ...
    function updateFCTabPreview(~,~)
        ...
        R = S0.lastFC;
        try, set(S0.axFCA,'Visible','on','Position',[0.065 0.105 0.875 0.835]); catch, end
        ...
        try
            if isfield(S0,'hFCSeedRegion') && ishghandle(S0.hFCSeedRegion)
                seedText = strtrim(char(get(S0.hFCSeedRegion,'String')));
                S0.fcSeedRegion = seedText;
                guidata(hFig,S0);
            end
        catch
        end
        try
            callFC('fcPlotFCAdvancedOneViewV14',S0.axFCA,S0.FC,R,seedText,viewMode,activeGroup,dispMode,thr,S0.C);
        catch ME
            try, GA_printErrorLocal(ME,'caught error in GroupAnalysis.m'); catch, end
