...
try, fcGAFinalBottomStripLayout_20260624(S); catch, end
try, fcGAFinalBottomStripLayout_20260624(S); catch, end
...
function fcGAFinalBottomStripLayout_20260624(S)
% Final FC-GA layout:
% - Status text stays under FC matrices plot.
% - Extra controls move to a separate strip BELOW the FC matrices box.
try
    ...
    mainParent = get(pMat,'Parent');
    if isempty(mainParent) || ~ishghandle(mainParent), return; end
    fig = ancestor(pMat,'figure');

    try, set(pMat,'Units','normalized'); catch, end
    try, set(mainParent,'Units','normalized'); catch, end

    % Keep original FC-matrices-panel position so repeated refreshes do not shrink it again.
    if isappdata(pMat,'FCGA_ORIGINAL_MATRIX_PANEL_POS_20260624')
        basePos = getappdata(pMat,'FCGA_ORIGINAL_MATRIX_PANEL_POS_20260624');
    else
        basePos = get(pMat,'Position');
        setappdata(pMat,'FCGA_ORIGINAL_MATRIX_PANEL_POS_20260624',basePos);
    end

    % Create space BELOW the FC matrices box for the detached control strip.
    stripH = 0.075;
    gap = 0.010;
    oldBottom = basePos(2);
    oldTop = basePos(2) + basePos(4);
    newBottom = oldBottom + stripH + gap;
    newHeight = max(0.20, oldTop - newBottom);
    set(pMat,'Position',[basePos(1) newBottom basePos(3) newHeight]);

    % The FC axes get a small bottom margin for the status line.
    try
        set(S.axFCA,'Units','normalized','Position',[0.055 0.145 0.820 0.780]);
    catch
    end

    % Restore status/info text to its old logical place: inside FC matrices, bottom-left.
    if isfield(S,'hFCInfo') && ~isempty(S.hFCInfo) && ishghandle(S.hFCInfo)
        try
            set(S.hFCInfo,'Parent',pMat);
        catch
        end
        set(S.hFCInfo,'Units','normalized', ...
            'Position',[0.008 0.018 0.965 0.050], ...
            'HorizontalAlignment','left', ...
            'FontSize',7.5, ...
            'ForegroundColor',[0.60 0.85 1.00]);
    end

    % Create or reuse bottom strip below the FC matrices box.
    pStrip = findall(mainParent,'Type','uipanel','Tag','FCGA_BOTTOM_CONTROL_STRIP_20260624');
    if isempty(pStrip) || ~ishghandle(pStrip(1))
        pStrip = uipanel('Parent',mainParent, ...
            'Units','normalized', ...
            'Position',[basePos(1) oldBottom basePos(3) stripH], ...
            'BorderType','line', ...
            'HighlightColor',[0.70 0.70 0.70], ...
            'ShadowColor',[0.25 0.25 0.25], ...
            'BackgroundColor',[0.07 0.07 0.075], ...
            'Tag','FCGA_BOTTOM_CONTROL_STRIP_20260624');
    else
        pStrip = pStrip(1);
    ...
    % Find labels/buttons anywhere in the FC tab figure/panel.
    hYLab = fcGAFindControlText_20260624(fig,'Y:','text');
    hPlotLab = fcGAFindControlText_20260624(fig,'Plot','text');
    hAnimals = fcGAFindControlText_20260624(fig,'Animals','pushbutton');
    hNames = fcGAFindControlText_20260624(fig,'Names','pushbutton');
    hPNG = fcGAFindControlText_20260624(fig,'PNG','pushbutton');
    hLarge = fcGAFindControlText_20260624(fig,'Large','pushbutton');
    if isempty(hLarge) || ~ishghandle(hLarge)
        hLarge = fcGAFindControlText_20260624(fig,'⛶','pushbutton');
    end

    % Move controls into detached bottom strip.
    fcGASetCtrlPos_20260624(hYLab,pStrip,[0.010 0.170 0.028 0.650]);

    if isfield(S,'hFCYAuto') && ~isempty(S.hFCYAuto) && ishghandle(S.hFCYAuto)
        fcGASetCtrlPos_20260624(S.hFCYAuto,pStrip,[0.045 0.145 0.070 0.700]);
    else
        hAuto = fcGAFindControlText_20260624(fig,'Auto','checkbox');
        fcGASetCtrlPos_20260624(hAuto,pStrip,[0.045 0.145 0.070 0.700]);
    end

    if isfield(S,'hFCYMin') && ~isempty(S.hFCYMin) && ishghandle(S.hFCYMin), fcGASetCtrlPos_20260624(S.hFCYMin,pStrip,[0.120 0.165 0.045 0.650]); end
    if isfield(S,'hFCYMax') && ~isempty(S.hFCYMax) && ishghandle(S.hFCYMax), fcGASetCtrlPos_20260624(S.hFCYMax,pStrip,[0.170 0.165 0.045 0.650]); end
    if isfield(S,'hFCYStep') && ~isempty(S.hFCYStep) && ishghandle(S.hFCYStep), fcGASetCtrlPos_20260624(S.hFCYStep,pStrip,[0.220 0.165 0.045 0.650]); end

    fcGASetCtrlPos_20260624(hPlotLab,pStrip,[0.285 0.170 0.045 0.650]);
    if isfield(S,'hFCPlotColor') && ~isempty(S.hFCPlotColor) && ishghandle(S.hFCPlotColor)
        fcGASetCtrlPos_20260624(S.hFCPlotColor,pStrip,[0.335 0.165 0.090 0.650]);
    end

    fcGASetCtrlPos_20260624(hAnimals,pStrip,[0.590 0.120 0.090 0.760]);
    fcGASetCtrlPos_20260624(hNames,pStrip,[0.685 0.120 0.075 0.760]);
    fcGASetCtrlPos_20260624(hPNG,pStrip,[0.765 0.120 0.075 0.760]);

    if ~isempty(hLarge) && ishghandle(hLarge)
        fcGASetCtrlPos_20260624(hLarge,pStrip,[0.845 0.120 0.085 0.760]);
        try
    ...
    % Make the upper FC group-analysis box cleaner by removing the now-unused bottom strip space.
    % We do not force every selector here, because their positions vary across your current patched file.
    drawnow limitrate;
catch ME
    try, fprintf('FC-GA final bottom-strip layout warning: %s\n',ME.message); catch, end
end
end
