...
try, fcGAStatusStripLayout_20260623(S); catch, end
...
function fcGAStatusStripLayout_20260623(S)
% Move secondary FC-GA controls below the status/info line.
try
    p = [];
    if isfield(S,'hFCInfo') && ~isempty(S.hFCInfo) && ishghandle(S.hFCInfo)
        p = get(S.hFCInfo,'Parent');
    end
    if isempty(p) || ~ishghandle(p)
        flds = {'hFCRegion1','hFCView','hFCDisplay','hFCSubject'};
        for ii = 1:numel(flds)
            if isfield(S,flds{ii}) && ~isempty(S.(flds{ii})) && ishghandle(S.(flds{ii}))
    ...
    % Status/info text: second-bottom row.
    if isfield(S,'hFCInfo') && ~isempty(S.hFCInfo) && ishghandle(S.hFCInfo)
        set(S.hFCInfo,'Units','normalized', ...
            'Position',[0.010 0.115 0.965 0.075], ...
            'HorizontalAlignment','left','FontSize',8.0);
    end

    % Bottom strip: Y controls and Plot color.
    y0 = 0.010;
    hYLab = fcGAFindControlText_20260623(p,'Y:','text');
    fcGASetCtrlPos_20260623(hYLab,[0.010 y0 0.030 0.085]);

    if isfield(S,'hFCYAuto') && ~isempty(S.hFCYAuto) && ishghandle(S.hFCYAuto)
        fcGASetCtrlPos_20260623(S.hFCYAuto,[0.045 y0 0.060 0.085]);
    else
        hAuto = fcGAFindControlText_20260623(p,'Auto','checkbox');
        fcGASetCtrlPos_20260623(hAuto,[0.045 y0 0.060 0.085]);
    end

    if isfield(S,'hFCYMin') && ~isempty(S.hFCYMin) && ishghandle(S.hFCYMin), fcGASetCtrlPos_20260623(S.hFCYMin,[0.110 y0 0.040 0.085]); end
    if isfield(S,'hFCYMax') && ~isempty(S.hFCYMax) && ishghandle(S.hFCYMax), fcGASetCtrlPos_20260623(S.hFCYMax,[0.155 y0 0.040 0.085]); end
    if isfield(S,'hFCYStep') && ~isempty(S.hFCYStep) && ishghandle(S.hFCYStep), fcGASetCtrlPos_20260623(S.hFCYStep,[0.200 y0 0.040 0.085]); end

    hPlotLab = fcGAFindControlText_20260623(p,'Plot','text');
    fcGASetCtrlPos_20260623(hPlotLab,[0.255 y0 0.040 0.085]);
    if isfield(S,'hFCPlotColor') && ~isempty(S.hFCPlotColor) && ishghandle(S.hFCPlotColor)
        fcGASetCtrlPos_20260623(S.hFCPlotColor,[0.300 y0 0.080 0.085]);
    end

    % Bottom strip: action buttons on the right.
    hAnimals = fcGAFindControlText_20260623(p,'Animals','pushbutton');
    fcGASetCtrlPos_20260623(hAnimals,[0.565 y0 0.080 0.085]);

    hNames = fcGAFindControlText_20260623(p,'Names','pushbutton');
    fcGASetCtrlPos_20260623(hNames,[0.650 y0 0.070 0.085]);

    hPNG = fcGAFindControlText_20260623(p,'PNG','pushbutton');
    fcGASetCtrlPos_20260623(hPNG,[0.725 y0 0.060 0.085]);

    hLarge = fcGAFindControlText_20260623(p,'Large','pushbutton');
    if isempty(hLarge) || ~ishghandle(hLarge)
        hLarge = fcGAFindControlText_20260623(p,'⛶','pushbutton');
    end
    if ~isempty(hLarge) && ishghandle(hLarge)
        set(hLarge,'Units','normalized', ...
            'Position',[0.790 y0 0.075 0.085], ...
            'String','Large', ...
            'Callback',@(src,evt)showFCLargeView_GA_20260622(ancestor(src,'figure')));
    end

    % Give the main FC selectors slightly more vertical breathing room.
    topControls = {'hFCView','hFCDisplay','hFCHemi','hFCColorMap','hFCLabelMode','hFCThreshold','hFCSlice','hFCRegion1','hFCRegion2','hFCSubject'};
    for ii = 1:numel(topControls)
        try
            h = S.(topControls{ii});
            if ~isempty(h) && ishghandle(h)
                pos = get(h,'Position');
                if numel(pos) == 4 && pos(2) < 0.20
                    pos(2) = 0.205;
                    set(h,'Position',pos);
                end
            end
    ...
    try, fprintf('FC-GA status-strip layout warning: %s\n',ME.message); catch, end
end
end
...
function fcGASetCtrlPos_20260623(h,pos)
try
    if ~isempty(h) && ishghandle(h)
