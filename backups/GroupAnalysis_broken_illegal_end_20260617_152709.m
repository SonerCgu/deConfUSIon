...
try
    ...
    S.hFCAnimals = mkBtn(pFCTop,'Animals...',[0.735 0.155 0.080 0.205],C.btnSecondary,@(~,~) selectFCAnimals_FIX_20260617(hFig));
    S.hFCNames   = mkBtn(pFCTop,'Names',[0.825 0.155 0.065 0.205],C.btnSecondary,@(~,~) showFCRegionNames_FIX_20260617(hFig));
...
%%% GA_FC_DISPLAY_FIX_UI_20260617_START
try
    % Put the utility buttons on the third row and add y-axis controls.
    try, set(S.hFCAnimals,'Position',[0.725 0.155 0.075 0.205],'String','Animals'); catch, end
    S.hFCRegions = mkBtn(pFCTop,'Regions',[0.805 0.155 0.075 0.205],C.btnSecondary,@(~,~) selectFCRegions_FIX_20260617(hFig));
    try, set(S.hFCNames,'Position',[0.885 0.155 0.050 0.205],'String','Names'); catch, end
    try, set(S.hFCExportPNG,'Position',[0.940 0.155 0.045 0.205],'String','PNG'); catch, end

    uicontrol(pFCTop,'Style','checkbox','String','Auto Y', ...
        'Units','normalized','Position',[0.725 0.015 0.075 0.120], ...
        'Value',1,'BackgroundColor',bg2,'ForegroundColor','w', ...
        'Callback',@updateFCTabPreview);
    S.hFCYAuto = findall(pFCTop,'Style','checkbox','String','Auto Y');

    uicontrol(pFCTop,'Style','text','String','Y min:', ...
        'Units','normalized','Position',[0.805 0.025 0.050 0.090], ...
        'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left');
    S.hFCYMin = uicontrol(pFCTop,'Style','edit','String','-1', ...
        'Units','normalized','Position',[0.855 0.020 0.050 0.115], ...
        'BackgroundColor',C.editBg,'ForegroundColor','w', ...
        'Callback',@updateFCTabPreview);

    uicontrol(pFCTop,'Style','text','String','Y max:', ...
        'Units','normalized','Position',[0.915 0.025 0.050 0.090], ...
        'BackgroundColor',bg2,'ForegroundColor','w','HorizontalAlignment','left');
    S.hFCYMax = uicontrol(pFCTop,'Style','edit','String','1', ...
        'Units','normalized','Position',[0.965 0.020 0.030 0.115], ...
        'BackgroundColor',C.editBg,'ForegroundColor','w', ...
        'Callback',@updateFCTabPreview);
catch ME_fcfix_ui
    try, disp(['FC display-fix UI warning: ' ME_fcfix_ui.message]); catch, end
end
%%% GA_FC_DISPLAY_FIX_UI_20260617_END
...
drawnow;
        try
            ...
            S0.FC = FC;
            S0.FC.loaded = true;
            S0 = attachFCGABundlesToTable_NOROW_SINGLE_20260616(S0,fileList,FC);
            S0 = sanitizeTableStruct(S0);
            S0 = ensureRowPacapSideSize(S0);
            S0 = ensureFCRowFilesSizeGA_TARGETED(S0);
            ...
            try, refreshFCRegionPopups_FIX_20260617(hFig); catch, end
            try, refreshFCSubjectPopup_FIX_20260617(hFig); catch, end
                try, refreshFCSlicePopup_FIX_20260617(hFig); catch, end
    ...
    function updateFCTabPreview(~,~)
        ...
        try
            ...
            if needCompute
                ...
                S0.lastFC = computeSingleGroupFC_FIX_20260617(S0);
                guidata(hFig,S0);
                try, refreshFCRegionPopups_FIX_20260617(hFig); catch, end
                try, refreshFCSubjectPopup_FIX_20260617(hFig); catch, end
                try, refreshFCSlicePopup_FIX_20260617(hFig); catch, end
            ...
            updateFCTabPreview_FIX_20260617(S0);
        catch ME
...
switch lower(viewMode)
    case 'heatmap'
        plotFCMatrix_ADV_20260617(S.axFCA,M,clim,sprintf('%s FC heatmap | %s | %s | n=%d',R.groupName,hemiTitle,valTxt,R.n),names,S.C,cmapName,labelMode);
    case {'seed profile ± sd','seed profile +/- sd','seed profile'}
        plotSeedProfile_ADV_20260617(S.axFCA,stack0,seedIdx0,R,valTxt,S.C);
    case {'animal pair values','roi trace'}
        plotAnimalPairValues_ADV_20260617(S.axFCA,stack0,seedIdx0,roi2Idx0,R,valTxt,S.C);
    case {'roi pair summary','roi pair'}
        plotROIPairSummary_ADV_20260617(S.axFCA,stack0,seedIdx0,roi2Idx0,R,valTxt,S.C);
    case 'subject matrix'
        if subjIdx <= 1
            plotFCMatrix_ADV_20260617(S.axFCA,M,clim,sprintf('%s mean subject matrix | %s | n=%d',R.groupName,hemiTitle,R.n),names,S.C,cmapName,labelMode);
        else
            ...
            plotFCMatrix_ADV_20260617(S.axFCA,Ms,clim,sprintf('Subject matrix: %s | %s',strtrimSafe(R.subjectNames{si}),ht),nms,S.C,cmapName,labelMode);
        end
    otherwise
...
function plotFCMatrix_ADV_20260617(ax,M,clim,titleStr,names,C,cmapName,labelMode)
cla(ax);
if isempty(M), fcNoDataLocal(ax,titleStr,C); return; end
imagesc(ax,M);
try, caxis(ax,clim); catch, end
...
function plotSeedProfile_ADV_20260617(ax,stack,seedIdx,R,valTxt,C)
cla(ax);
...
function plotAnimalPairValues_ADV_20260617(ax,stack,seedIdx,roi2Idx,R,valTxt,C)
...
function plotROIPairSummary_ADV_20260617(ax,stack,seedIdx,roi2Idx,R,valTxt,C)
...
function refreshFCRegionPopups_FIX_20260617(hFig)
try
...
function refreshFCSubjectPopup_FIX_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
    items=cell(numel(S.FC.subjects)+1,1); items{1}='Group mean';
    for ii=1:numel(S.FC.subjects), nm=sprintf('Subject %d',ii); try,nm=strtrimSafe(S.FC.subjects(ii).name);catch,end; if isempty(nm),nm=sprintf('Subject %d',ii);end; items{ii+1}=nm; end
    if isfield(S,'hFCSubject')&&ishghandle(S.hFCSubject), set(S.hFCSubject,'String',items,'Value',min(get(S.hFCSubject,'Value'),numel(items))); end
catch
end
end

function refreshFCSlicePopup_FIX_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
    nZ=1;
    for ii=1:numel(S.FC.subjects)
...
function selectFCAnimals_ADV_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
    ...
    if ok, S.fcSelectedSubjectIdx=sel(:); S.lastFC=struct(); guidata(hFig,S); S = guidata(hFig); try, S.lastFC = computeSingleGroupFC_FIX_20260617(S); guidata(hFig,S); updateFCTabPreview_FIX_20260617(S); catch MEa, try, set(S.hFCInfo,'String',['FC animal selection: ' MEa.message]); catch, end, end end
catch ME, try,errordlg(ME.message,'FC animals');catch,end
end
end

function showFCRegionNames_ADV_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
    subj=S.FC.subjects(1); labels=getFCLabels_SINGLE_DIRECT_20260617(subj); names=getFCNames_SINGLE_DIRECT_20260617(subj,labels);
...
% GA_FC_DISPLAY_FIX_HELPERS_20260617_START
function R = computeSingleGroupFC_FIX_20260617(S)
G = alignFCSubjectsWithOptions_FIX_20260617(S);
groupSel = popupString_SINGLE_20260616(S,'hFCGroupA','All loaded');
idx = true(numel(G.groups),1);
if ~isempty(groupSel) && ~strcmpi(groupSel,'All loaded') && ~strcmpi(groupSel,'All')
    idx = strcmpi(G.groups,groupSel);
...
try
    if isfield(S,'fcSelectedSubjectIdx') && ~isempty(S.fcSelectedSubjectIdx)
        keep = false(numel(idx),1);
        kk = S.fcSelectedSubjectIdx(:);
        kk = kk(kk>=1 & kk<=numel(idx));
        keep(kk) = true;
        idx = idx & keep;
        if ~any(idx), idx = keep; end
    end
...
pMat = pOneSampleApprox_SINGLE_20260616(Z);
R = struct();
R.mode = 'Functional Connectivity Single Group Fixed';
R.groupName = groupSel;
R.groupA = groupSel;
R.groupB = '';
R.n = sum(idx);
...
R.sourceFiles = G.sourceFiles(idx);
R.sliceMode = G.sliceMode;
R.sliceNote = G.sliceNote;
R.note = 'Single-group FC-GA: mean is computed in Fisher z space; Pearson r = tanh(mean z).';
end

function updateFCTabPreview_FIX_20260617(S)
if ~isfield(S,'lastFC') || isempty(fieldnames(S.lastFC))
    if isfield(S,'FC') && isfield(S.FC,'subjects') && ~isempty(S.FC.subjects)
        S.lastFC = computeSingleGroupFC_FIX_20260617(S);
    else
        return;
    end
end
...
if thr > 0, M0(abs(M0) < thr) = 0; end
roiKeep = selectedROIIdx_FIX_20260617(S,R);
Msel = M0(roiKeep,roiKeep);
namesSel = R.names(roiKeep); labelsSel = R.labels(roiKeep);
[M,namesX,namesY,labelsX,labelsY,hemiTitle] = applyHemisphereMatrix_FIX_20260617(Msel,namesSel,labelsSel,hemiMode);
seedIdx0 = popupIndex_SINGLE_20260616(S,'hFCRegion1',1);
roi2Idx0 = popupIndex_SINGLE_20260616(S,'hFCRegion2',min(2,size(M0,1)));
seedIdx0 = max(1,min(seedIdx0,size(M0,1)));
roi2Idx0 = max(1,min(roi2Idx0,size(M0,1)));
...
switch lower(viewMode)
    case 'heatmap'
        plotFCMatrix_FIX_20260617(S.axFCA,M,clim,sprintf('%s FC heatmap | %s | %s | n=%d',R.groupName,hemiTitle,valTxt,R.n),namesX,namesY,labelsX,labelsY,S.C,cmapName,labelMode,hemiTitle);
    case {'seed profile ± sd','seed profile +/- sd','seed profile'}
        plotSeedProfile_FIX_20260617(S.axFCA,stack0,seedIdx0,roiKeep,R,valTxt,S.C,S);
    case {'animal pair values','roi trace'}
        plotAnimalPairValues_FIX_20260617(S.axFCA,stack0,seedIdx0,roi2Idx0,R,valTxt,S.C,S);
    case {'roi pair summary','roi pair'}
        plotROIPairSummary_FIX_20260617(S.axFCA,stack0,seedIdx0,roi2Idx0,R,valTxt,S.C,S);
    case 'subject matrix'
        if subjIdx <= 1
            plotFCMatrix_FIX_20260617(S.axFCA,M,clim,sprintf('%s mean subject matrix | %s | n=%d',R.groupName,hemiTitle,R.n),namesX,namesY,labelsX,labelsY,S.C,cmapName,labelMode,hemiTitle);
        else
            si = max(1,min(subjIdx-1,size(stack0,3)));
            Ms0 = stack0(:,:,si);
            if thr > 0, Ms0(abs(Ms0) < thr) = 0; end
            Ms0 = Ms0(roiKeep,roiKeep);
            [Ms,nx,ny,lx,ly,ht] = applyHemisphereMatrix_FIX_20260617(Ms0,namesSel,labelsSel,hemiMode);
            plotFCMatrix_FIX_20260617(S.axFCA,Ms,clim,sprintf('Subject matrix: %s | %s',strtrimSafe(R.subjectNames{si}),ht),nx,ny,lx,ly,S.C,cmapName,labelMode,ht);
        end
    otherwise
        plotROIOverlay_FIX_20260617(S.axFCA,S,R,cmapName);
end
try
    msg = sprintf('Loaded %d FC subject(s). Showing %s, n=%d | %s | %s | Slice=%s%s | ROIs=%d | Seed=%s | ROI2=%s', ...
        numel(S.FC.subjects),R.groupName,R.n,viewMode,hemiMode,R.sliceMode,R.sliceNote,numel(roiKeep),roiName_SINGLE_20260616(R,seedIdx0),roiName_SINGLE_20260616(R,roi2Idx0));
    set(S.hFCInfo,'String',msg);
catch
end
end

function G = alignFCSubjectsWithOptions_FIX_20260617(S)
FC = S.FC;
if ~isfield(FC,'subjects') || isempty(FC.subjects), error('No FC subjects loaded.'); end
sliceMode = popupString_SINGLE_20260616(S,'hFCSlice','All slices');
nSub = numel(FC.subjects);
...
if isempty(commonLabels), error('No common ROI labels found across FC subjects.'); end
nR = numel(commonLabels);
Zstack = nan(nR,nR,nSub); Rstack = nan(nR,nR,nSub); names = cell(nR,1);
for ii = 1:nSub
    subj = FC.subjects(ii);
    labs = getFCLabels_SINGLE_DIRECT_20260617(subj);
    [Rmat,Zmat,usedSlice] = getFCMatricesForSlice_FIX_20260617(subj,sliceMode);
    ...
    if any(~isfinite(idx)), error('Internal FC ROI alignment error.'); end
    idx = double(idx(:));
    Rstack(:,:,ii) = Rmat(idx,idx); Zstack(:,:,ii) = Zmat(idx,idx);
    if ii == 1
        nms = getFCNames_SINGLE_DIRECT_20260617(subj,labs);
        for kk = 1:nR
            srcIdx = idx(kk);
...
G = struct(); G.labels = commonLabels; G.names = names; G.Zstack = Zstack; G.Rstack = Rstack; G.nSubjects = nSub;
G.subjectNames = cell(nSub,1); G.groups = cell(nSub,1); G.sourceFiles = cell(nSub,1); G.sliceMode = sliceMode;
if usedSliceSpecific, G.sliceNote = ''; else, G.sliceNote = ' (subject-level matrix; no slice-specific matrices in bundle)'; end
for ii = 1:nSub
    try, G.subjectNames{ii}=strtrimSafe(FC.subjects(ii).name); catch, G.subjectNames{ii}=sprintf('Subject_%02d',ii); end
    try, G.groups{ii}=strtrimSafe(FC.subjects(ii).group); catch, G.groups{ii}='Unassigned'; end
    if isempty(G.groups{ii}), G.groups{ii}='Unassigned'; end
...
function [Rmat,Zmat,usedSlice] = getFCMatricesForSlice_FIX_20260617(subj,sliceMode)
usedSlice = false;
if nargin < 2 || isempty(sliceMode), sliceMode = 'All slices'; end
useSlice = NaN;
tok = regexp(sliceMode,'(\d+)','tokens','once');
...
if hasSlices
    ...
    if isfinite(useSlice)
        z = max(1,min(round(useSlice),numel(SR)));
        [Rmat,Zmat] = getMatrixFromStruct_FIX_20260617(SR(z));
        if ~isempty(Rmat), usedSlice = true; return; end
    else
        Zs = {}; usedCount = 0;
        for zz = 1:numel(SR)
            [Rz,Zz] = getMatrixFromStruct_FIX_20260617(SR(zz));
            if ~isempty(Rz) && ~isempty(Zz)
                Zs{end+1} = Zz; %#ok<AGROW>
        ...
        if ~isempty(Zs)
            Zcat = cat(3,Zs{:}); Zmat = mean3nan_SINGLE_20260616(Zcat); Rmat = tanh(Zmat); usedSlice = usedCount > 0; return;
        end
    end
end
[Rmat,Zmat] = getFCMatrices_SINGLE_DIRECT_20260617(subj);
...
function [Rmat,Zmat] = getMatrixFromStruct_FIX_20260617(X)
Rmat=[]; Zmat=[];
try, if isfield(X,'R') && ~isempty(X.R), Rmat=double(X.R); end, catch, end
try, if isempty(Rmat) && isfield(X,'displayMatrix') && ~isempty(X.displayMatrix), Rmat=double(X.displayMatrix); end, catch, end
try, if isempty(Rmat) && isfield(X,'corrMatrix') && ~isempty(X.corrMatrix), Rmat=double(X.corrMatrix); end, catch, end
...
try, if isempty(Zmat) && isfield(X,'displayZ') && ~isempty(X.displayZ), Zmat=double(X.displayZ); end, catch, end
if isempty(Rmat) && ~isempty(Zmat), Rmat=tanh(Zmat); end
if isempty(Zmat) && ~isempty(Rmat), Rc=max(-0.999999,min(0.999999,Rmat)); Zmat=atanh(Rc); try, Zmat(1:size(Zmat,1)+1:end)=0; catch, end, end
end

function roiKeep = selectedROIIdx_FIX_20260617(S,R)
roiKeep = 1:numel(R.labels);
try
    if isfield(S,'fcSelectedROIIdx') && ~isempty(S.fcSelectedROIIdx)
        kk = unique(round(double(S.fcSelectedROIIdx(:)')),'stable');
        kk = kk(kk >= 1 & kk <= numel(R.labels));
        if ~isempty(kk), roiKeep = kk; end
    end
catch
end
end

function [M2,namesX,namesY,labelsX,labelsY,titleTxt] = applyHemisphereMatrix_FIX_20260617(M,names,labels,mode)
mode = lower(strtrimSafe(mode)); labels = double(labels(:));
L = labels < 0; R = labels > 0;
if ~any(L), L = contains(lower(names),'l_') | contains(lower(names),'left') | startsWith(lower(names),'l-'); end
if ~any(R), R = contains(lower(names),'r_') | contains(lower(names),'right') | startsWith(lower(names),'r-'); end
if contains(mode,'left vs right')
    rowIdx=find(L); colIdx=find(R); if isempty(rowIdx)||isempty(colIdx), rowIdx=1:size(M,1); colIdx=1:size(M,2); end
    M2=M(rowIdx,colIdx); namesY=names(rowIdx); namesX=names(colIdx); labelsY=labels(rowIdx); labelsX=labels(colIdx); titleTxt='Left rows × Right columns';
elseif contains(mode,'left') && ~contains(mode,'right')
...
elseif contains(mode,'merged')
    [M2,namesX,labelsX] = mergeLRMatrix_FIX_20260617(M,names,labels); namesY=namesX; labelsY=labelsX; titleTxt='Merged L+R homologs';
else
    M2=M; namesX=names; namesY=names; labelsX=labels; labelsY=labels; titleTxt='All ROIs';
end
end

function [Mm,namesM,labelsM] = mergeLRMatrix_FIX_20260617(M,names,labels)
labels = double(labels(:));
if any(labels < 0) && any(labels > 0), ids = unique(abs(labels)); ids = ids(ids>0); else, ids = (1:numel(labels))'; end
n = numel(ids); Mm = nan(n,n); namesM = cell(n,1); labelsM = ids(:);
for i=1:n
    ii=find(abs(labels)==ids(i)); if isempty(ii), ii=i; end
    namesM{i}=stripSideFromName_FIX_20260617(names{ii(1)});
    for j=1:n
        jj=find(abs(labels)==ids(j)); if isempty(jj), jj=j; end
        block=M(ii,jj); v=block(isfinite(block)); if ~isempty(v), Mm(i,j)=mean(v); end
    end
...
function plotFCMatrix_FIX_20260617(ax,M,clim,titleStr,namesX,namesY,labelsX,labelsY,C,cmapName,labelMode,hemiTitle)
cla(ax); if isempty(M), fcNoDataLocal(ax,titleStr,C); return; end
imagesc(ax,M); try, caxis(ax,clim); catch, end
colormap(ax,cmapFC_FIX_20260617(cmapName,256)); cb=colorbar(ax); try, set(cb,'Color',[1 1 1]); ylabel(cb,'FC','Color',[1 1 1]); catch, end
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontName','Arial','FontSize',9,'TickLength',[0 0]);
title(ax,titleStr,'Color',C.txt,'FontWeight','bold','Interpreter','none');
nR=size(M,1); nC=size(M,2); ticksY=tickIdx_SINGLE_20260616(nR); ticksX=tickIdx_SINGLE_20260616(nC);
labelN = 12; if strcmpi(labelMode,'Full'), labelN = 22; end
xLabs = makeRegionTickLabels_FIX_20260617(namesX,labelsX,labelMode,hemiTitle,labelN);
yLabs = makeRegionTickLabels_FIX_20260617(namesY,labelsY,labelMode,hemiTitle,labelN);
...
if contains(lower(hemiTitle),'left rows')
    xlabel(ax,'Right hemisphere regions','Color',C.txt); ylabel(ax,'Left hemisphere regions','Color',C.txt);
elseif contains(lower(hemiTitle),'left only')
    xlabel(ax,'Left hemisphere regions','Color',C.txt); ylabel(ax,'Left hemisphere regions','Color',C.txt);
elseif contains(lower(hemiTitle),'right only')
    xlabel(ax,'Right hemisphere regions','Color',C.txt); ylabel(ax,'Right hemisphere regions','Color',C.txt);
else
    xlabel(ax,'Regions','Color',C.txt); ylabel(ax,'Regions','Color',C.txt);
end
end

function labs = makeRegionTickLabels_FIX_20260617(names,labels,labelMode,hemiTitle,n)
labs = cell(numel(names),1);
for i=1:numel(names)
    raw = strtrimSafe(names{i});
    side = sideFromNameOrLabel_FIX_20260617(raw,labels(i));
    acr = acronymFromName_FIX_20260617(raw);
    if strcmpi(labelMode,'Full')
        base = fullRegionName_FIX_20260617(acr);
    else
        base = acr;
    end
    if contains(lower(hemiTitle),'all rois')
        if ~isempty(side), base = [side '-' base]; end
    end
    labs{i} = truncateLabel_FIX_20260617(base,n);
end
end

function plotSeedProfile_FIX_20260617(ax,stack,seedIdx,roiKeep,R,valTxt,C,S)
cla(ax); roiKeep = roiKeep(:)'; Y=squeeze(stack(seedIdx,roiKeep,:)); if isvector(Y), Y=reshape(Y,numel(roiKeep),[]); end
mu=nanmean_fix_20260617(Y,2); sd=nanstd_fix_20260617(Y,0,2); x=1:numel(mu);
patch(ax,[x fliplr(x)],[mu(:)'+sd(:)' fliplr(mu(:)'-sd(:)')],[0.6 0.6 0.6],'FaceAlpha',0.25,'EdgeColor','none'); hold(ax,'on'); plot(ax,x,mu,'LineWidth',2.0); hold(ax,'off');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted); grid(ax,'on');
title(ax,sprintf('Seed profile mean ± SD: %s | n=%d',roiName_SINGLE_20260616(R,seedIdx),size(stack,3)),'Color',C.txt,'FontWeight','bold','Interpreter','none');
xlabel(ax,'Selected regions in FC atlas label order','Color',C.txt); ylabel(ax,valTxt,'Color',C.txt);
if numel(roiKeep) <= 35
    labs = makeRegionTickLabels_FIX_20260617(R.names(roiKeep),R.labels(roiKeep),'Abbrev','All ROIs',12);
    set(ax,'XTick',x,'XTickLabel',labs); try, xtickangle(ax,45); catch, end
end
applyYLimFC_FIX_20260617(ax,S);
end

function plotAnimalPairValues_FIX_20260617(ax,stack,seedIdx,roi2Idx,R,valTxt,C,S)
cla(ax); vals=squeeze(stack(seedIdx,roi2Idx,:)); vals=vals(:); x=1:numel(vals);
plot(ax,x,vals,'-o','LineWidth',1.8,'MarkerSize',7); hold(ax,'on'); plot(ax,x,repmat(nanmean_fix_20260617(vals,1),size(x)),'--','LineWidth',1.4); hold(ax,'off');
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted); grid(ax,'on');
title(ax,sprintf('Animal-wise FC values: %s ↔ %s',roiName_SINGLE_20260616(R,seedIdx),roiName_SINGLE_20260616(R,roi2Idx)),'Color',C.txt,'FontWeight','bold','Interpreter','none');
xlabel(ax,'Animal index','Color',C.txt); ylabel(ax,valTxt,'Color',C.txt); applyYLimFC_FIX_20260617(ax,S);
end

function plotROIPairSummary_FIX_20260617(ax,stack,seedIdx,roi2Idx,R,valTxt,C,S)
cla(ax); vals=squeeze(stack(seedIdx,roi2Idx,:)); vals=vals(:); mu=nanmean_fix_20260617(vals,1); sd=nanstd_fix_20260617(vals,0,1);
bar(ax,1,mu,0.38); hold(ax,'on'); errorbar(ax,1,mu,sd,'k','LineStyle','none','LineWidth',1.8);
if numel(vals)>1, plot(ax,1+linspace(-0.10,0.10,numel(vals)),vals,'o','MarkerSize',8,'LineWidth',1.5); end
hold(ax,'off'); xlim(ax,[0.45 1.55]);
set(ax,'XTick',1,'XTickLabel',{'Mean ± SD + animals'},'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted,'FontSize',11); grid(ax,'on');
title(ax,sprintf('ROI pair summary: %s ↔ %s | n=%d',roiName_SINGLE_20260616(R,seedIdx),roiName_SINGLE_20260616(R,roi2Idx),numel(vals)),'Color',C.txt,'FontWeight','bold','Interpreter','none');
ylabel(ax,valTxt,'Color',C.txt); applyYLimFC_FIX_20260617(ax,S);
end

function plotROIOverlay_FIX_20260617(ax,S,R,cmapName)
cla(ax); [map2,note] = findROIOverlayMap_FIX_20260617(S);
if isempty(map2)
    text(ax,0.5,0.56,'ROI overlay map not found in loaded FC bundle', 'Color',S.C.txt,'HorizontalAlignment','center','FontWeight','bold');
    text(ax,0.5,0.46,'Run the FunctionalConnectivity export enrichment patch below and re-export the FC GA bundle.', 'Color',S.C.muted,'HorizontalAlignment','center');
    set(ax,'Color',S.C.axisBg,'XTick',[],'YTick',[]); title(ax,'ROI overlay map', 'Color',S.C.txt); return;
end
imagesc(ax,map2); axis(ax,'image'); axis(ax,'off'); colormap(ax,cmapFC_FIX_20260617(cmapName,256)); colorbar(ax);
title(ax,['ROI overlay map | ' note],'Color',S.C.txt,'FontWeight','bold','Interpreter','none');
end

function [map2,note] = findROIOverlayMap_FIX_20260617(S)
map2=[]; note='';
try
    if ~isfield(S,'FC') || ~isfield(S.FC,'subjects') || isempty(S.FC.subjects), return; end
    fns={'roiMap','labelMap','parcelMap','labelMask','roiLabelMask','atlasLabels2D','maskLabels','segmentationMap','segMap'};
    subj=S.FC.subjects(1);
    for ii=1:numel(fns), if isfield(subj,fns{ii}) && isnumeric(subj.(fns{ii})), map2=squeeze(subj.(fns{ii})); note=fns{ii}; if ndims(map2)>2,map2=map2(:,:,round(size(map2,3)/2));end; return; end, end
    if isfield(subj,'sourceFile') && exist(subj.sourceFile,'file')==2
        L=load(subj.sourceFile); [map2,note]=findMapRecursive_FIX_20260617(L,0); if ~isempty(map2), return; end
    end
catch
end
end

function [map2,note] = findMapRecursive_FIX_20260617(X,depth)
map2=[]; note=''; if depth>4, return; end
keys={'roiMap','labelMap','parcelMap','labelMask','roiLabelMask','atlasLabels2D','maskLabels','segmentationMap','segMap'};
try
    if isstruct(X)
        fn=fieldnames(X);
        for i=1:numel(fn)
            val=X.(fn{i});
            if any(strcmp(fn{i},keys)) && isnumeric(val)
                map2=squeeze(val); note=fn{i}; if ndims(map2)>2,map2=map2(:,:,round(size(map2,3)/2));end; return;
            end
        end
        for i=1:numel(fn)
            val=X.(fn{i});
            if isstruct(val)
                ...
                [map2,note]=findMapRecursive_FIX_20260617(val,depth+1); if ~isempty(map2), return; end
            end
        end
    end
catch
...
function selectFCRegions_FIX_20260617(hFig)
try
    S=guidata(hFig);
    if isempty(S) || ~isfield(S,'FC') || ~isfield(S.FC,'subjects') || isempty(S.FC.subjects), return; end
    subj=S.FC.subjects(1); labels=getFCLabels_SINGLE_DIRECT_20260617(subj); names=getFCNames_SINGLE_DIRECT_20260617(subj,labels);
    ...
    for i=1:numel(labels)
        acr=acronymFromName_FIX_20260617(names{i}); full=fullRegionName_FIX_20260617(acr); side=sideFromNameOrLabel_FIX_20260617(names{i},labels(i));
        if isempty(side), items{i}=sprintf('%g | %s = %s',labels(i),acr,full); else, items{i}=sprintf('%g | %s-%s = %s',labels(i),side,acr,full); end
    end
    init=1:numel(items); try, if isfield(S,'fcSelectedROIIdx') && ~isempty(S.fcSelectedROIIdx), init=S.fcSelectedROIIdx; end, catch, end
    [sel,ok]=listdlg('PromptString','Select regions shown in FC-GA plots:','SelectionMode','multiple','ListString',items,'InitialValue',init,'ListSize',[520 420]);
    if ok
        S.fcSelectedROIIdx=sel(:);
        guidata(hFig,S);
        S=guidata(hFig);
        if ~isfield(S,'lastFC') || isempty(fieldnames(S.lastFC)), S.lastFC=computeSingleGroupFC_FIX_20260617(S); guidata(hFig,S); end
        S=guidata(hFig); updateFCTabPreview_FIX_20260617(S);
    end
catch ME, try,errordlg(ME.message,'FC regions');catch,end
end
end

function selectFCAnimals_FIX_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
    names=cell(numel(S.FC.subjects),1); for ii=1:numel(S.FC.subjects), try,names{ii}=strtrimSafe(S.FC.subjects(ii).name);catch,names{ii}=sprintf('Subject %d',ii);end; if isempty(names{ii}),names{ii}=sprintf('Subject %d',ii);end; end
    init=1:numel(names); try, if isfield(S,'fcSelectedSubjectIdx') && ~isempty(S.fcSelectedSubjectIdx), init=S.fcSelectedSubjectIdx; end, catch, end
    [sel,ok]=listdlg('PromptString','Select animals included in FC-GA:','SelectionMode','multiple','ListString',names,'InitialValue',init,'ListSize',[420 320]);
    if ok
        S.fcSelectedSubjectIdx=sel(:); S.lastFC=struct(); guidata(hFig,S);
        S=guidata(hFig); S.lastFC=computeSingleGroupFC_FIX_20260617(S); guidata(hFig,S); S=guidata(hFig); updateFCTabPreview_FIX_20260617(S);
    end
catch ME, try,errordlg(ME.message,'FC animals');catch,end
end
end

function showFCRegionNames_FIX_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
    subj=S.FC.subjects(1); labels=getFCLabels_SINGLE_DIRECT_20260617(subj); names=getFCNames_SINGLE_DIRECT_20260617(subj,labels);
    lines=cell(numel(labels),1);
    for ii=1:numel(labels)
        acr=acronymFromName_FIX_20260617(names{ii}); full=fullRegionName_FIX_20260617(acr); side=sideFromNameOrLabel_FIX_20260617(names{ii},labels(ii));
        if isempty(side), lines{ii}=sprintf('%g = %s = %s',labels(ii),acr,full); else, lines{ii}=sprintf('%g = %s-%s = %s',labels(ii),side,acr,full); end
    end
    f=figure('Name','FC ROI labels','Color',[0.08 0.08 0.08],'MenuBar','none','ToolBar','none','NumberTitle','off','Position',[200 120 720 760]);
    uicontrol(f,'Style','listbox','String',lines,'Units','normalized','Position',[0.03 0.08 0.94 0.89],'BackgroundColor',[0 0 0],'ForegroundColor',[1 1 1],'FontName','Consolas','FontSize',11);
    uicontrol(f,'Style','pushbutton','String','Copy to clipboard','Units','normalized','Position',[0.33 0.015 0.34 0.045],'Callback',@(s,e) clipboard('copy',strjoin(lines,newline)));
catch ME, try,errordlg(ME.message,'FC names');catch,end
end
...
try
    ...
    items=cell(numel(labels),1);
    for ii=1:numel(labels)
        acr=acronymFromName_FIX_20260617(names{ii}); side=sideFromNameOrLabel_FIX_20260617(names{ii},labels(ii));
        if isempty(side), items{ii}=sprintf('%g | %s',labels(ii),acr); else, items{ii}=sprintf('%g | %s-%s',labels(ii),side,acr); end
    end
    if isempty(items), items={'No ROI labels'}; end
    if isfield(S,'hFCRegion1')&&ishghandle(S.hFCRegion1), set(S.hFCRegion1,'String',items,'Value',min(get(S.hFCRegion1,'Value'),numel(items))); end
    if isfield(S,'hFCRegion2')&&ishghandle(S.hFCRegion2), set(S.hFCRegion2,'String',items,'Value',min(max(get(S.hFCRegion2,'Value'),2),numel(items))); end
...
function refreshFCSubjectPopup_FIX_20260617(hFig)
refreshFCSubjectPopup_SINGLE_20260616(hFig);
end
function refreshFCSlicePopup_FIX_20260617(hFig)
refreshFCSlicePopup_ADV_20260617(hFig);
end

function s=sideFromNameOrLabel_FIX_20260617(raw,label)
s=''; rawL=lower(strtrimSafe(raw));
if startsWith(rawL,'l_')||startsWith(rawL,'l-')||contains(rawL,'left'), s='L'; return; end
if startsWith(rawL,'r_')||startsWith(rawL,'r-')||contains(rawL,'right'), s='R'; return; end
try, if label < 0, s='L'; elseif label > 0, s='R'; end, catch, end
end
function acr=acronymFromName_FIX_20260617(raw)
acr=strtrimSafe(raw); acr=regexprep(acr,'\[[^\]]*\]',''); acr=regexprep(acr,'(?i)^(L|R)[_\-\s]+',''); acr=regexprep(acr,'(?i)\b(left|right)\b',''); acr=strtrim(regexprep(acr,'\s+',' '));
if isempty(acr), acr=strtrimSafe(raw); end
end
function s=stripSideFromName_FIX_20260617(raw)
s=acronymFromName_FIX_20260617(raw);
end
function out=truncateLabel_FIX_20260617(s,n)
out=strtrimSafe(s); out=strrep(out,'_','-'); if numel(out)>n, out=[out(1:max(1,n-3)) '...']; end
end
function full=fullRegionName_FIX_20260617(acr)
a=lower(strtrimSafe(acr)); a=regexprep(a,'[^a-z0-9]','');
switch a
    case 'cpu', full='caudate putamen';
    case 'alv', full='alveus';
    case 'cc', full='corpus callosum';
    case 'aca', full='anterior commissure anterior part';
    case 'acp', full='anterior commissure posterior part';
    ...
    case 'm1', full='primary motor cortex';
    case 'm2', full='secondary motor cortex';
    case 's1', full='primary somatosensory cortex';
    case 's2', full='secondary somatosensory cortex';
    case 'v1', full='primary visual cortex';
    case 'v2', full='secondary visual cortex';
    case 'aud', full='auditory cortex';
...
function cm=cmapFC_FIX_20260617(name,n)
cm=cmapFC_ADV_20260617(name,n);
end
function applyYLimFC_FIX_20260617(ax,S)
try
    autoY = true; try, autoY = logical(get(S.hFCYAuto,'Value')); catch, end
    if ~autoY
        ymin = safeNum(get(S.hFCYMin,'String'),NaN); ymax = safeNum(get(S.hFCYMax,'String'),NaN);
        if isfinite(ymin)&&isfinite(ymax)&&ymax>ymin, ylim(ax,[ymin ymax]); end
    end
catch
end
end
function m=nanmean_fix_20260617(X,dim)
if nargin<2,dim=1;end; X=double(X); ok=isfinite(X); X(~ok)=0; n=sum(ok,dim); m=sum(X,dim)./max(n,1); m(n==0)=NaN;
end
function s=nanstd_fix_20260617(X,flag,dim)
if nargin<2,flag=0;end; if nargin<3,dim=1;end; mu=nanmean_fix_20260617(X,dim); sz=ones(1,ndims(X)); sz(dim)=size(X,dim); muRep=repmat(mu,sz); D=(X-muRep).^2; D(~isfinite(D))=NaN; v=nanmean_fix_20260617(D,dim); s=sqrt(v);
end
% GA_FC_DISPLAY_FIX_HELPERS_20260617_END
