...
function key = makeCacheKey(x)
% Safe cache-key helper for FC bundle loading.
% Converts a file path/string into a valid MATLAB struct field name.
try
    if isstring(x)
        x = char(x);
    end
...
% Small stable hash without external dependencies.
h = uint32(2166136261);
for ii = 1:numel(x)
    h = bitxor(h,uint32(double(x(ii))));
    h = uint32(mod(double(h) * 16777619, 4294967295));
end
...
key = sprintf('k_%s_%08X',base,double(h));
