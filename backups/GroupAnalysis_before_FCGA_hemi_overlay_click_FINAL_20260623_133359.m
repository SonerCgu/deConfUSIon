...
% Robust hemisphere filtering after ROI selection/sorting.
try
    n = size(M,1);
    if nargin < 2 || isempty(names), names = cell(n,1); for ii=1:n, names{ii}=sprintf('ROI_%03d',ii); end, end
    if nargin < 3 || isempty(labels), labels = (1:n)'; end
    if nargin < 4 || isempty(hemiMode), hemiMode = 'All'; end
    labels = labels(:);
    mode = lower(strtrim(hemiMode));
    [isL,isR] = fcGAHemiMask_20260624(names,labels);
    allIdx = 1:n;
    rowIdx = allIdx;
    colIdx = allIdx;
    hemiTitle = 'All hemispheres';

    if ~isempty(strfind(mode,'left vs right')) || ~isempty(strfind(mode,'l/r'))
        if any(isL) && any(isR)
            rowIdx = find(isL);
            colIdx = find(isR);
            hemiTitle = 'Left vs Right';
        end
    elseif ~isempty(strfind(mode,'left')) || strcmpi(mode,'L')
        if any(isL)
            rowIdx = find(isL);
            colIdx = find(isL);
            hemiTitle = 'Left hemisphere';
        end
    elseif ~isempty(strfind(mode,'right')) || strcmpi(mode,'R')
        if any(isR)
            rowIdx = find(isR);
            colIdx = find(isR);
            hemiTitle = 'Right hemisphere';
        end
    elseif ~isempty(strfind(mode,'merged')) || ~isempty(strfind(mode,'bilateral'))
        rowIdx = allIdx;
        colIdx = allIdx;
        hemiTitle = 'Merged/Bilateral view';
    end

    if isempty(rowIdx), rowIdx = allIdx; end
    if isempty(colIdx), colIdx = allIdx; end
    M2 = M(rowIdx,colIdx);
    names2 = names(rowIdx);
    labels2 = labels(rowIdx);
catch ME
    try, fprintf('FC-GA hemisphere filter warning: %s\n',ME.message); catch, end
    M2 = M;
    rowIdx = 1:size(M,1);
    colIdx = 1:size(M,2);
    names2 = names;
...
function [isL,isR] = fcGAHemiMask_20260624(names,labels)
% Infer hemisphere from region name first, then signed label if available.
n = numel(labels);
isL = false(n,1);
isR = false(n,1);
for ii = 1:n
    s = '';
    try, s = char(names{ii}); catch, end
    sl = lower(strtrim(s));
    sl = strrep(sl,'-','_');
    sl = strrep(sl,' ','_');
    if ~isempty(regexp(sl,'(^|_)l(_|$)','once')) || ~isempty(regexp(sl,'(^|_)left(_|$)','once'))
        isL(ii) = true;
    end
    if ~isempty(regexp(sl,'(^|_)r(_|$)','once')) || ~isempty(regexp(sl,'(^|_)right(_|$)','once'))
        isR(ii) = true;
    end
end
if ~any(isL) && ~any(isR)
...
% Scientific FC heatmap: selected ROIs labeled; title/colorbar remain readable.
try, cla(ax,'reset'); catch, cla(ax); end
imagesc(ax,M);
set(ax,'YDir','reverse');
axis(ax,'tight');
try, colormap(ax,fcGAColormap_LOCAL_20260624(cmapName)); catch, colormap(ax,parula(256)); end
try, caxis(ax,clim); catch, end
cb = [];
try
    cb = colorbar(ax);
...
labsX = fcGAHeatLabels_20260624(namesX,labelsX,labelMode);
labsY = fcGAHeatLabels_20260624(namesY,labelsY,labelMode);
nx = numel(labsX); ny = numel(labsY);
nMax = max(nx,ny);
fig = ancestor(ax,'figure');
isLarge = false;
try, isLarge = ~isempty(strfind(lower(get(fig,'Name')),'large')); catch, end
if isLarge
    if nMax > 120, fs = 5.0; elseif nMax > 90, fs = 5.5; elseif nMax > 60, fs = 6.0; elseif nMax > 35, fs = 7.0; else, fs = 9.0; end
else
    if nMax > 120, fs = 4.5; elseif nMax > 90, fs = 5.0; elseif nMax > 60, fs = 5.5; elseif nMax > 35, fs = 6.5; else, fs = 8.5; end
end
...
ht = title(ax,ttl,'Color',C.txt,'FontWeight','bold','Interpreter','none');
hx = xlabel(ax,'Regions', 'Color',C.txt,'Interpreter','none');
hy = ylabel(ax,'Regions', 'Color',C.txt,'Interpreter','none');
try, set(ht,'FontSize',12); catch, end
try, set(hx,'FontSize',10); catch, end
try, set(hy,'FontSize',10); catch, end
try, if ~isempty(cb), ylabel(cb,'FC value','Color',C.txt,'FontSize',9); end, catch, end
box(ax,'on');
end

function labs = fcGAHeatLabels_20260624(names,labels,labelMode)
n = numel(labels);
labs = cell(n,1);
for ii = 1:n
    try
        labs{ii} = fcNiceName_SAFE_20260617(names{ii},labels(ii),labelMode,false);
    catch
        labs{ii} = sprintf('%g',labels(ii));
    end
end
end
...
% Robust click-to-seed installer for ROI overlay in normal and large mode.
try
    if isempty(ax) || ~ishghandle(ax), return; end
    fig = ancestor(ax,'figure');
    if isempty(fig) || ~ishghandle(fig), return; end
...
try
    ...
    if isstruct(D) && isfield(D,'S') && isfield(D,'ax')
        ...
        try, updateFCTabPreview_ADV_20260617(S); catch, end
    end
catch ME
...
function L = fcGAGetOverlayLabelMap_20260624(S,R)
L = [];
try, if isfield(R,'roiMap') && ~isempty(R.roiMap), L = R.roiMap; return; end, catch, end
try, if isfield(R,'labelMap') && ~isempty(R.labelMap), L = R.labelMap; return; end, catch, end
try, if isfield(R,'roiAtlas') && ~isempty(R.roiAtlas), L = R.roiAtlas; return; end, catch, end
try, if isfield(R,'atlasLabels') && ~isempty(R.atlasLabels), L = R.atlasLabels; return; end, catch, end
try, if isfield(S,'lastFC') && isfield(S.lastFC,'roiMap') && ~isempty(S.lastFC.roiMap), L = S.lastFC.roiMap; return; end, catch, end
try, if isfield(S,'lastFC') && isfield(S.lastFC,'labelMap') && ~isempty(S.lastFC.labelMap), L = S.lastFC.labelMap; return; end, catch, end
try, if isfield(S,'lastFC') && isfield(S.lastFC,'roiAtlas') && ~isempty(S.lastFC.roiAtlas), L = S.lastFC.roiAtlas; return; end, catch, end
end
...
try
    ...
    patch = double(L2(yy,xx));
    vals = patch(isfinite(patch) & patch ~= 0);
    if ~isempty(vals), lab = vals(1); end
catch
    lab = [];
end
end
