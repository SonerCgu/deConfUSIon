...
% Polished FC-GA top layout: aligned labels/dropdowns/buttons.
try
    ...
    % Keep top box compact but give enough vertical room for three aligned rows.
    set(pTop,'Position',[0.02 0.790 0.96 0.200]);
    set(pMat,'Position',[0.02 0.130 0.96 0.645]);

    % Matrix axes: move right to avoid y-axis label/ticks touching border.
    try
        set(S.axFCA,'Units','normalized','Position',[0.095 0.110 0.765 0.825]);
    catch
    end

    % Status line below matrix, unchanged style.
    ...
    % Bottom strip stays below status.
    ...
    % ==============================================================
    % Row 1: Load / Scan / Group / Display / Recompute / Export GA
    % ==============================================================
    try, if isfield(S,'hFCLoad') && ishghandle(S.hFCLoad), set(S.hFCLoad,'Parent',pTop,'Units','normalized','Position',[0.020 row1 0.135 0.305],'FontSize',11.0,'FontWeight','bold'); end, catch, end
    try, if isfield(S,'hFCScan') && ishghandle(S.hFCScan), set(S.hFCScan,'Parent',pTop,'Units','normalized','Position',[0.165 row1 0.110 0.305],'FontSize',11.0,'FontWeight','bold'); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Group:',[0.305 row1 0.060 hLab],fsLab);
    try, if isfield(S,'hFCGroupA') && ishghandle(S.hFCGroupA), set(S.hFCGroupA,'Parent',pTop,'Units','normalized','Position',[0.370 row1 0.125 hCtl],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Display:',[0.525 row1 0.075 hLab],fsLab);
    try, if isfield(S,'hFCDisplay') && ishghandle(S.hFCDisplay), set(S.hFCDisplay,'Parent',pTop,'Units','normalized','Position',[0.605 row1 0.135 hCtl],'FontSize',fsCtl); end, catch, end

    try, if isfield(S,'hFCCompute') && ishghandle(S.hFCCompute), set(S.hFCCompute,'Parent',pTop,'Units','normalized','Position',[0.765 0.535 0.110 0.395],'String','Recompute','FontSize',fsBtn,'FontWeight','bold'); end, catch, end
    try, if isfield(S,'hFCExport') && ishghandle(S.hFCExport), set(S.hFCExport,'Parent',pTop,'Units','normalized','Position',[0.885 0.535 0.100 0.395],'String','Export GA','FontSize',fsBtn,'FontWeight','bold'); end, catch, end

    % ==============================================================
    % Row 2: Subject / View / Hemi / Color / Labels
    % ==============================================================
    fcGASetLabelPos_20260624(pTop,'Subject:',[0.020 row2 0.070 hLab],fsLab);
    try, if isfield(S,'hFCSubject') && ishghandle(S.hFCSubject), set(S.hFCSubject,'Parent',pTop,'Units','normalized','Position',[0.095 row2 0.175 hCtl],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'View:',[0.290 row2 0.050 hLab],fsLab);
    try, if isfield(S,'hFCView') && ishghandle(S.hFCView), set(S.hFCView,'Parent',pTop,'Units','normalized','Position',[0.345 row2 0.155 hCtl],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Hemi:',[0.520 row2 0.055 hLab],fsLab);
    try, if isfield(S,'hFCHemi') && ishghandle(S.hFCHemi), set(S.hFCHemi,'Parent',pTop,'Units','normalized','Position',[0.580 row2 0.125 hCtl],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Color:',[0.715 row2 0.060 hLab],fsLab);
    try, if isfield(S,'hFCColorMap') && ishghandle(S.hFCColorMap), set(S.hFCColorMap,'Parent',pTop,'Units','normalized','Position',[0.780 row2 0.110 hCtl],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Labels:',[0.900 row2 0.065 hLab],fsLab);
    try, if isfield(S,'hFCLabelMode') && ishghandle(S.hFCLabelMode), set(S.hFCLabelMode,'Parent',pTop,'Units','normalized','Position',[0.965 row2 0.030 hCtl],'FontSize',fsCtl); end, catch, end
    % If label popup is too narrow, use a wider slot and slightly overlap available empty area.
    try
        if isfield(S,'hFCLabelMode') && ishghandle(S.hFCLabelMode)
            fcGASetLabelPos_20260624(pTop,'Labels:',[0.865 row2 0.065 hLab],fsLab);
            set(S.hFCLabelMode,'Position',[0.930 row2 0.065 hCtl]);
        end
    catch
    end

    % ==============================================================
    % Row 3: Seed / ROI 2 / Slice -- lower and aligned, less empty bottom gap
    % ==============================================================
    fcGASetLabelPos_20260624(pTop,'Seed:',[0.020 row3 0.050 hLab],fsLab);
    try, if isfield(S,'hFCRegion1') && ishghandle(S.hFCRegion1), set(S.hFCRegion1,'Parent',pTop,'Units','normalized','Position',[0.075 row3 0.270 hCtl],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'ROI 2:',[0.365 row3 0.065 hLab],fsLab);
    try, if isfield(S,'hFCRegion2') && ishghandle(S.hFCRegion2), set(S.hFCRegion2,'Parent',pTop,'Units','normalized','Position',[0.435 row3 0.275 hCtl],'FontSize',fsCtl); end, catch, end

    fcGASetLabelPos_20260624(pTop,'Slice:',[0.735 row3 0.055 hLab],fsLab);
    try, if isfield(S,'hFCSlice') && ishghandle(S.hFCSlice), set(S.hFCSlice,'Parent',pTop,'Units','normalized','Position',[0.795 row3 0.125 hCtl],'FontSize',fsCtl); end, catch, end

    % ==============================================================
    % Bottom strip: Y / Plot / Animals / Names / PNG / Large
    % ==============================================================
    ...
    try, fprintf('FC-GA top alignment polish warning: %s\n',ME.message); catch, end
end
end
