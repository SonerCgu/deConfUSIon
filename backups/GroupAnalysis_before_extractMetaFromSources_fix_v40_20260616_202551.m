...
% Restored helper used by onAddBundles.
% Adds selected bundle MAT as a new table row and records it in fcRowFiles.
try
    ...
    nCols = max(8,size(S.subj,2));
    if size(S.subj,2) < nCols
        S.subj(:,end+1:nCols) = {''};
    end
    ...
    row{1} = animalName;
    if nCols >= 2, row{2} = ''; end
    if nCols >= 3, row{3} = ''; end
    if nCols >= 8, row{8} = fp; end

    S.subj(end+1,:) = row;
    newRow = size(S.subj,1);

    if ~isfield(S,'fcRowFiles') || isempty(S.fcRowFiles)
        S.fcRowFiles = cell(newRow,1);
    elseif numel(S.fcRowFiles) < newRow
