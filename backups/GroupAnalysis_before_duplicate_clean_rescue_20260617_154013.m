...
try
    ...
    S.hFCAnimals = mkBtn(pFCTop,'Animals...',[0.735 0.155 0.080 0.205],C.btnSecondary,@(~,~) selectFCAnimals_CLEAN_20260617(hFig));
    S.hFCNames   = mkBtn(pFCTop,'Names',[0.825 0.155 0.065 0.205],C.btnSecondary,@(~,~) showFCRegionNames_CLEAN_20260617(hFig));
...
drawnow;
        try
            ...
            try, refreshFCRegionPopups_CLEAN_20260617(hFig); catch, end
...
switch lower(viewMode)
    case 'heatmap'
        plotFCMatrix_CLEAN_20260617(S.axFCA,M,clim,sprintf('%s FC heatmap | %s | %s | n=%d',R.groupName,hemiTitle,valTxt,R.n),names,S.C,cmapName,labelMode);
    ...
    case 'subject matrix'
        if subjIdx <= 1
            plotFCMatrix_CLEAN_20260617(S.axFCA,M,clim,sprintf('%s mean subject matrix | %s | n=%d',R.groupName,hemiTitle,R.n),names,S.C,cmapName,labelMode);
        else
            ...
            plotFCMatrix_CLEAN_20260617(S.axFCA,Ms,clim,sprintf('Subject matrix: %s | %s',strtrimSafe(R.subjectNames{si}),ht),nms,S.C,cmapName,labelMode);
        end
    otherwise
...
function plotFCMatrix_CLEAN_20260617(ax,M,clim,titleStr,names,C,cmapName,labelMode)
cla(ax);
if isempty(M), fcNoDataLocal(ax,titleStr,C); return; end
imagesc(ax,M);
try, caxis(ax,clim); catch, end
...
function refreshFCRegionPopups_CLEAN_20260617(hFig)
try
...
function selectFCAnimals_CLEAN_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
...
function showFCRegionNames_CLEAN_20260617(hFig)
try
    S=guidata(hFig); if isempty(S)||~isfield(S,'FC')||~isfield(S.FC,'subjects')||isempty(S.FC.subjects), return; end
    subj=S.FC.subjects(1); labels=getFCLabels_SINGLE_DIRECT_20260617(subj); names=getFCNames_SINGLE_DIRECT_20260617(subj,labels);
...
% Clean FC heatmap plotting: sparse readable ticks, no repeated L_/R_ in hemi-specific modes.
cla(ax);
if isempty(M)
    fcNoDataLocal(ax,titleStr,C);
    return;
end
imagesc(ax,M);
try, caxis(ax,clim); catch, end
try, colormap(ax,cmapFC_CLEAN_20260617(cmapName,256)); catch, colormap(ax,jet(256)); end
cb = colorbar(ax);
try, set(cb,'Color',[1 1 1]); ylabel(cb,'FC','Color',[1 1 1]); catch, end
set(ax,'Color',C.axisBg,'XColor',C.muted,'YColor',C.muted, ...
    'FontName','Arial','FontSize',9,'TickLength',[0 0]);
title(ax,titleStr,'Color',C.txt,'FontWeight','bold','Interpreter','none');

nR = size(M,1);
nC = size(M,2);
fullMode = strcmpi(strtrimSafe(labelMode),'Full');
maxTicks = 28;
if fullMode, maxTicks = 16; end
ticksY = cleanTickIdx_CLEAN_20260617(nR,maxTicks);
ticksX = cleanTickIdx_CLEAN_20260617(nC,maxTicks);

xLabs = makeCleanHeatmapLabels_CLEAN_20260617(ax,namesX,labelsX,labelMode,hemiTitle);
yLabs = makeCleanHeatmapLabels_CLEAN_20260617(ax,namesY,labelsY,labelMode,hemiTitle);
...
[xlab,ylab] = hemiAxisLabels_CLEAN_20260617(hemiTitle);
xlabel(ax,xlab,'Color',C.txt,'Interpreter','none');
ylabel(ax,ylab,'Color',C.txt,'Interpreter','none');
end

function idx = cleanTickIdx_CLEAN_20260617(n,maxTicks)
if nargin < 2 || isempty(maxTicks), maxTicks = 28; end
if n <= maxTicks
    idx = 1:n;
else
...
function labs = makeCleanHeatmapLabels_CLEAN_20260617(ax,names,labels,labelMode,hemiTitle)
if nargin < 5, hemiTitle = ''; end
n = numel(names);
labs = cell(n,1);
fullMode = strcmpi(strtrimSafe(labelMode),'Full');
stripSide = shouldStripSide_CLEAN_20260617(hemiTitle);
for i = 1:n
    raw = strtrimSafe(names{i});
    lab = NaN;
    try, lab = double(labels(i)); catch, end
    side = sideFromNameOrLabel_CLEAN_20260617(raw,lab);
    acr  = acronymFromName_CLEAN_20260617(raw);
    if fullMode
        txt = fullRegionName_CLEAN_20260617(ax,acr,lab);
    else
        txt = acr;
    end
    ...
    if fullMode
        labs{i} = truncateOneLine_CLEAN_20260617(txt,22);
    else
        labs{i} = truncateOneLine_CLEAN_20260617(txt,10);
    end
end
end

function tf = shouldStripSide_CLEAN_20260617(hemiTitle)
h = lower(strtrimSafe(hemiTitle));
tf = contains(h,'left only') || contains(h,'right only') || contains(h,'merged');
end

function [xlab,ylab] = hemiAxisLabels_CLEAN_20260617(hemiTitle)
h = lower(strtrimSafe(hemiTitle));
if contains(h,'left rows') || contains(h,'left rows') || contains(h,'left') && contains(h,'right columns')
    xlab = 'Right hemisphere regions';
    ylab = 'Left hemisphere regions';
elseif contains(h,'left only')
    xlab = 'Left hemisphere regions';
    ylab = 'Left hemisphere regions';
elseif contains(h,'right only')
    xlab = 'Right hemisphere regions';
    ylab = 'Right hemisphere regions';
elseif contains(h,'merged')
    xlab = 'Merged bilateral regions';
    ylab = 'Merged bilateral regions';
else
    xlab = 'Regions';
    ylab = 'Regions';
end
...
function s = sideFromNameOrLabel_CLEAN_20260617(raw,label)
s = '';
r = lower(strtrimSafe(raw));
if startsWith(r,'l_') || startsWith(r,'l-') || startsWith(r,'l ') || contains(r,'left')
    s = 'L';
    return;
end
if startsWith(r,'r_') || startsWith(r,'r-') || startsWith(r,'r ') || contains(r,'right')
    s = 'R';
    return;
end
try
...
function acr = acronymFromName_CLEAN_20260617(raw)
acr = strtrimSafe(raw);
acr = regexprep(acr,'\[[^\]]*\]','');
...
function out = truncateOneLine_CLEAN_20260617(s,n)
out = strtrimSafe(s);
out = strrep(out,'_','-');
...
function full = fullRegionName_CLEAN_20260617(ax,acr,label)
full = '';
try
    fig = ancestor(ax,'figure');
    S = guidata(fig);
    if isfield(S,'fcFullNameMap')
        key1 = sprintf('%g',abs(double(label)));
        key2 = lower(regexprep(acr,'[^A-Za-z0-9]',''));
        if isa(S.fcFullNameMap,'containers.Map')
            if isKey(S.fcFullNameMap,key1)
                full = S.fcFullNameMap(key1);
            elseif isKey(S.fcFullNameMap,key2)
                full = S.fcFullNameMap(key2);
            end
        end
    end
catch
...
if isempty(full)
    full = fullRegionNameFallback_CLEAN_20260617(acr);
end
end

function full = fullRegionNameFallback_CLEAN_20260617(acr)
a = lower(strtrimSafe(acr));
a = regexprep(a,'[^a-z0-9]','');
switch a
...
function cm = cmapFC_CLEAN_20260617(name,n)
if nargin < 2, n = 256; end
name = lower(strtrimSafe(name));
switch name
    case {'blue-white-red','bwr'}
        cm = bwr_SINGLE_20260616(n);
    case 'parula'
...
try
    S = guidata(hFig);
    if isempty(S) || ~isfield(S,'FC') || ~isfield(S.FC,'subjects') || isempty(S.FC.subjects), return; end
    % Give user chance to load real atlas .label full names.
    loadNames = false;
    if ~isfield(S,'fcFullNameMap') || isempty(S.fcFullNameMap)
        try
            q = questdlg('Load atlas .label file for full names?','FC region names','Yes','No','No');
            loadNames = strcmpi(q,'Yes');
        catch
            loadNames = false;
        end
    end
    if loadNames
        [f,p] = uigetfile({'*.label;*.txt;*.csv','Atlas label/name files (*.label, *.txt, *.csv)'},'Select atlas label file');
        if isequal(f,0)
        else
            S.fcFullNameMap = readAtlasNameMap_CLEAN_20260617(fullfile(p,f));
            S.fcAtlasLabelFile = fullfile(p,f);
            guidata(hFig,S);
        end
    end
    S = guidata(hFig);
    subj = S.FC.subjects(1);
    labels = getFCLabels_SINGLE_DIRECT_20260617(subj);
    names  = getFCNames_SINGLE_DIRECT_20260617(subj,labels);
    lines = cell(numel(labels),1);
    for ii = 1:numel(labels)
        side = sideFromNameOrLabel_CLEAN_20260617(names{ii},labels(ii));
        acr = acronymFromName_CLEAN_20260617(names{ii});
        full = fullRegionNameFromState_CLEAN_20260617(S,acr,labels(ii));
        if isempty(side)
            lines{ii} = sprintf('%g = %s = %s',labels(ii),acr,full);
        else
            lines{ii} = sprintf('%g = %s-%s = %s',labels(ii),side,acr,full);
        end
    end
    f2 = figure('Name','FC ROI labels','Color',[0.08 0.08 0.08],'MenuBar','none','ToolBar','none','NumberTitle','off','Position',[200 120 760 760]);
    uicontrol(f2,'Style','listbox','String',lines,'Units','normalized','Position',[0.03 0.08 0.94 0.89],'BackgroundColor',[0 0 0],'ForegroundColor',[1 1 1],'FontName','Consolas','FontSize',11);
    uicontrol(f2,'Style','pushbutton','String','Copy to clipboard','Units','normalized','Position',[0.33 0.015 0.34 0.045],'Callback',@(s,e) clipboard('copy',strjoin(lines,newline)));
catch ME
    try, errordlg(ME.message,'FC names'); catch, end
end
end

function full = fullRegionNameFromState_CLEAN_20260617(S,acr,label)
full = '';
try
    if isfield(S,'fcFullNameMap') && isa(S.fcFullNameMap,'containers.Map')
        key1 = sprintf('%g',abs(double(label)));
        key2 = lower(regexprep(acr,'[^A-Za-z0-9]',''));
        if isKey(S.fcFullNameMap,key1)
            full = S.fcFullNameMap(key1);
...
if isempty(full), full = fullRegionNameFallback_CLEAN_20260617(acr); end
end

function M = readAtlasNameMap_CLEAN_20260617(fp)
M = containers.Map('KeyType','char','ValueType','char');
fid = fopen(fp,'r');
if fid < 0, error('Could not open label file: %s',fp); end
...
while true
    ...
    toks = regexp(line,'^\s*(-?\d+)\s+(.+?)\s*$','tokens','once');
    if isempty(toks)
        toks = regexp(line,'^\s*(-?\d+)[,\t; ]+(.+?)\s*$','tokens','once');
    end
    if isempty(toks), continue; end
    lab = str2double(toks{1});
    nm = strtrim(toks{2});
    nm = regexprep(nm,'["'']','');
    if isfinite(lab) && ~isempty(nm)
        M(sprintf('%g',abs(lab))) = nm;
        acr = lower(regexprep(acronymFromName_CLEAN_20260617(nm),'[^A-Za-z0-9]',''));
        if ~isempty(acr), M(acr) = nm; end
    end
end
end

function selectFCRegions_CLEAN_20260617(hFig)
try
    S = guidata(hFig);
    if isempty(S) || ~isfield(S,'FC') || ~isfield(S.FC,'subjects') || isempty(S.FC.subjects), return; end
    subj = S.FC.subjects(1);
    ...
    for i = 1:numel(labels)
        side = sideFromNameOrLabel_CLEAN_20260617(names{i},labels(i));
        acr = acronymFromName_CLEAN_20260617(names{i});
        full = fullRegionNameFromState_CLEAN_20260617(S,acr,labels(i));
        if isempty(side)
            items{i} = sprintf('%g | %s = %s',labels(i),acr,full);
        else
            items{i} = sprintf('%g | %s-%s = %s',labels(i),side,acr,full);
        end
    end
    init = 1:numel(items);
    ...
    if ok
        ...
        guidata(hFig,S);
        fcAutoRefresh_CLEAN_20260617(hFig);
    end
catch ME
...
try
    ...
    for ii = 1:numel(S.FC.subjects)
        try, names{ii}=strtrimSafe(S.FC.subjects(ii).name); catch, names{ii}=sprintf('Subject %d',ii); end
        if isempty(names{ii}), names{ii}=sprintf('Subject %d',ii); end
    end
    init = 1:numel(names);
...
try
    ...
    for ii = 1:numel(labels)
        ...
        acr = acronymFromName_CLEAN_20260617(names{ii});
        if isempty(side)
            items{ii} = sprintf('%g | %s',labels(ii),acr);
        else
            items{ii} = sprintf('%g | %s-%s',labels(ii),side,acr);
        end
    end
    if isempty(items), items = {'No ROI labels'}; end
...
try
    ...
    for ii = 1:numel(S.FC.subjects)
        ...
        try
            if isfield(subj,'sliceResults') && ~isempty(subj.sliceResults)
                nZ = max(nZ,numel(subj.sliceResults));
                for zz = 1:numel(subj.sliceResults)
                    SR = subj.sliceResults(zz);
                    if (isfield(SR,'R') && ~isempty(SR.R)) || (isfield(SR,'Z') && ~isempty(SR.Z)) || (isfield(SR,'displayMatrix') && ~isempty(SR.displayMatrix))
                        hasTrueSlice = true;
                    end
                end
            end
    ...
    if hasTrueSlice
        items = cell(nZ+1,1);
        items{1} = 'All slices';
        for z=1:nZ, items{z+1}=sprintf('Slice %d',z); end
    else
        items = {'All slices (no slice FC exported)'};
    end
    if isfield(S,'hFCSlice') && ishghandle(S.hFCSlice)
        set(S.hFCSlice,'String',items,'Value',min(get(S.hFCSlice,'Value'),numel(items)));
    end
catch
end
end

function fcAutoRefresh_CLEAN_20260617(hFig)
try
    S = guidata(hFig);
    if isempty(S) || ~isfield(S,'FC') || ~isfield(S.FC,'subjects') || isempty(S.FC.subjects), return; end
    try, S = syncFCGroupsFromTable_TARGETED(S); catch, end
    if exist('computeSingleGroupFC_FIX_20260617','file') == 2
    end
    try
        S.lastFC = computeSingleGroupFC_FIX_20260617(S);
    catch
        S.lastFC = computeSingleGroupFC_SINGLE_20260616(S);
    end
    guidata(hFig,S);
    S = guidata(hFig);
    try
        updateFCTabPreview_FIX_20260617(S);
    catch
        try, updateFCTabPreview_ADV_20260617(S); catch, updateFCTabPreview_SINGLE_20260616(S); end
    end
catch ME
    try
        S = guidata(hFig);
        set(S.hFCInfo,'String',['FC refresh failed: ' ME.message]);
    catch
    end
end
end
% GA_FC_CLEAN_LABELS_20260617_END
