...
function createRows()function createRows()
    S = getState();
    n = numel(S.steps);
...
function startDrag(r)
    ...
    S.dragStartRow = 0;
    S.dragIsActive = false;
    try, set(S.fig,'WindowButtonMotionFcn',''); catch, end
    try, set(S.fig,'WindowButtonUpFcn',''); catch, end
    putState(S);
    refreshRows();
    refreshSelectedPanel();
    setStatus(['Selected row #' num2str(r) '. Use checkbox to run/skip; use Move Up/Down to reorder.'],[0.80 0.95 0.85]);
end
function dragMotion(~,~)
    S = getState();
    r = rowUnderMouse(S);
