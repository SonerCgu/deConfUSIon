...
% Row-aware folder choice for Add Bundles.
% Priority: selected animal row path -> loaded FC/bundle files -> stored dirs -> pwd.
...
try
    try
        ...
        localGA_installTableSelectionMemoryV37(fig);
    catch
        fig = [];
    end

    rows = localGA_selectedRowsFromStateV37(S,fig);
    startPath = localGA_dirFromSelectedRowsV37(S,rows);
    ...
    % Loaded FC/bundle files.
    try
        if isfield(S,'fcRowFiles') && ~isempty(S.fcRowFiles)
            for ii = 1:numel(S.fcRowFiles)
                p = localGA_strV37(S.fcRowFiles{ii});
                d = localGA_bestExistingDirV37(p);
                if ~isempty(d), candidates{end+1,1} = d; end
            end
        end
    catch
    end
    ...
    try
        if isfield(S,'FC') && isstruct(S.FC) && isfield(S.FC,'files') && ~isempty(S.FC.files)
            for ii = 1:numel(S.FC.files)
                p = localGA_strV37(S.FC.files{ii});
                d = localGA_bestExistingDirV37(p);
                if ~isempty(d), candidates{end+1,1} = d; end
            end
        end
    ...
    % Stored folder fields.
    fields = {'lastBrowseDir','lastBundleDir','lastFCBundleDir','lastDir','rootDir','baseDir','analysisDir','analysisRoot','projectRoot'};
    for ii = 1:numel(fields)
        try
            if isfield(S,fields{ii}) && ~isempty(S.(fields{ii}))
                p = localGA_strV37(S.(fields{ii}));
                d = localGA_bestExistingDirV37(p);
                if ~isempty(d), candidates{end+1,1} = d; end
            end
        catch
    ...
    % Table fallback: use any row with path-like entries.
    try
        if isfield(S,'subj') && iscell(S.subj)
            for r = 1:size(S.subj,1)
                d = localGA_bestDirFromRowV37(S.subj,r);
                if ~isempty(d), candidates{end+1,1} = d; end
            end
        end
    catch
    ...
    try, candidates{end+1,1} = pwd; catch, end
    try, candidates{end+1,1} = fileparts(mfilename('fullpath')); catch, end

    for ii = 1:numel(candidates)
        p = localGA_strV37(candidates{ii});
...
function localGA_installTableSelectionMemoryV37(fig)
% Install a lightweight row-selection memory on all uitables.
try
    ...
    for ii = 1:numel(tabs)
        h = tabs(ii);
        try
            if isappdata(h,'deconfusion_selection_memory_v37')
                continue;
            end
            oldCb = get(h,'CellSelectionCallback');
            setappdata(h,'deconfusion_old_cellselection_v37',oldCb);
            setappdata(h,'deconfusion_selection_memory_v37',true);
            set(h,'CellSelectionCallback',@localGA_tableSelectionMemoryCallbackV37);
        catch
        end
    end
catch
...
function localGA_tableSelectionMemoryCallbackV37(src,ev)
% Store selected table rows, then call original CellSelectionCallback.
try
    fig = ancestor(src,'figure');
    rows = [];
    try
        if isfield(ev,'Indices') && ~isempty(ev.Indices)
            rows = unique(ev.Indices(:,1));
        end
    catch
    end
    ...
    if ~isempty(rows)
        rows = rows(:);
        setappdata(src,'deconfusion_selected_rows_v37',rows);
        if ~isempty(fig) && ishandle(fig)
            setappdata(fig,'deconfusion_selected_rows_v37',rows);
            try
                S = guidata(fig);
                if isstruct(S)
                    S.selectedRows = rows;
                    S.lastSelectedRows = rows;
                    S.activeRow = rows(1);
                    guidata(fig,S);
                end
            catch
    ...
    localGA_callOriginalCellSelectionV37(src,ev);
catch
    try, localGA_callOriginalCellSelectionV37(src,ev); catch, end
end
end

function localGA_callOriginalCellSelectionV37(src,ev)
try
    oldCb = [];
    if isappdata(src,'deconfusion_old_cellselection_v37')
        oldCb = getappdata(src,'deconfusion_old_cellselection_v37');
    end
    if isempty(oldCb)
        return;
    end
    if isa(oldCb,'function_handle')
        oldCb(src,ev);
    elseif iscell(oldCb) && ~isempty(oldCb) && isa(oldCb{1},'function_handle')
        feval(oldCb{1},src,ev,oldCb{2:end});
    elseif ischar(oldCb)
        try, eval(oldCb); catch, end
    end
catch
end
end

function rows = localGA_selectedRowsFromStateV37(S,fig)
rows = [];

fields = {'selectedRows','lastSelectedRows','selectedRow','lastSelectedRow','activeRow','currentRow','rowIndex','selectedTableRows'};
for ii = 1:numel(fields)
    try
...
try
    if ~isempty(fig) && ishandle(fig) && isappdata(fig,'deconfusion_selected_rows_v37')
        x = getappdata(fig,'deconfusion_selected_rows_v37');
        rows = [rows; double(x(:))];
    end
catch
end
...
try
    if ~isempty(fig) && ishandle(fig)
        tabs = findall(fig,'Type','uitable');
        for ii = 1:numel(tabs)
            if isappdata(tabs(ii),'deconfusion_selected_rows_v37')
                x = getappdata(tabs(ii),'deconfusion_selected_rows_v37');
                rows = [rows; double(x(:))];
            end
        end
    end
...
function d = localGA_dirFromSelectedRowsV37(S,rows)
d = '';
try
    if isempty(rows) || ~isstruct(S) || ~isfield(S,'subj') || ~iscell(S.subj)
        return;
    end
    nRows = size(S.subj,1);
    rows = rows(rows >= 1 & rows <= nRows);
    for ii = 1:numel(rows)
        d = localGA_bestDirFromRowV37(S.subj,rows(ii));
        if ~isempty(d)
            return;
        end
    end
...
function d = localGA_bestDirFromRowV37(tbl,r)
d = '';
try
    if ~iscell(tbl) || r < 1 || r > size(tbl,1)
        return;
    end
    nCols = size(tbl,2);

    % Priority columns: original Bundle File col 8, then nearby path columns, then all.
    cols = [];
    if nCols >= 8, cols = [cols 8]; end
    cols = [cols 1:nCols];
    cols = unique(cols,'stable');

    for kk = 1:numel(cols)
        cc = cols(kk);
        p = localGA_strV37(tbl{r,cc});
        if isempty(p) || ~localGA_isLikelyPathV37(p)
            continue;
        end
        d = localGA_bestExistingDirV37(p);
        if ~isempty(d)
...
function d = localGA_bestExistingDirV37(p)
d = '';
try
    p = localGA_strV37(p);
    ...
    % For not-yet-existing bundle paths, walk upward until an existing parent is found.
    q = p;
    for ii = 1:12
        [q2,~,~] = fileparts(q);
...
function tf = localGA_isLikelyPathV37(p)
tf = false;
try
    p = localGA_strV37(p);
    if isempty(p), return; end
...
try
    ...
    elseif ischar(x)
        s = strtrim(x);
    elseif iscell(x)
        if ~isempty(x), s = localGA_strV37(x{1}); end
    elseif isnumeric(x) || islogical(x)
        if isscalar(x), s = strtrim(num2str(x)); else, s = strtrim(mat2str(x)); end
    else
        try, s = strtrim(char(x)); catch, s = ''; end
