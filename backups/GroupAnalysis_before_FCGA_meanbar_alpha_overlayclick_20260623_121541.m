...
% ROI subset selector used by both normal and large heatmap/seed-profile plotting.
...
if ~isempty(strfind(low,'all'))
    idx = 1:n;
    note = sprintf('All ROIs: %d regions',n);
    return;
end
...
if fcGAIsAlphaChoice_20260624(choice)
    ...
    if page >= nPages, page = 0; try, setappdata(fig,'FCGA_ALPHA_PAGE',0); catch, end, end
    st = page*k + 1;
    en = min(n,st+k-1);
    idx = alphaOrder(st:en);
    % Always include seed and ROI2 so seed profile / pair summaries remain valid.
...
hold(ax,'on');
for ii = 1:n
    if ii == selectedSubjectIdx, ms = 14; lw = 2.8; else, ms = 12; lw = 2.0; end
...
if isfinite(mu)
    ...
    errorbar(ax,xMean,mu,sem,'d','Color',meanCol,'MarkerFaceColor',meanCol, ...
        'MarkerEdgeColor',meanCol,'MarkerSize',13,'LineWidth',2.7,'CapSize',16, ...
...
if isIndividual || numel(vals) == 1
    plot(ax,1,vals(1),'o','MarkerSize',14,'LineWidth',2.8,'Color',lineCol,'MarkerFaceColor',lineCol,'DisplayName',subjectNote);
    xlim(ax,[0.5 1.5]);
    ...
    for ii = 1:n
        plot(ax,x(ii),vals(ii),'o','Color',cols(ii,:),'MarkerFaceColor',cols(ii,:), ...
            'MarkerSize',12,'LineWidth',2.0,'DisplayName',subjLabs{ii});
    end
    [mu,sem,nEff] = fcGAMeanSem_20260624(vals);
    if isfinite(mu)
        if n > 1, xMean = mean(x); else, xMean = 1; end
...
try
    ...
    allv = vals;
    if isfinite(mu) && isfinite(sem), allv = [allv; mu-sem; mu+sem]; end
...
try
    ...
    for ii=1:numel(vals), labs{ii} = sprintf(fmt,vals(ii)); end
catch
    for ii=1:numel(vals), labs{ii} = sprintf('%.3f',vals(ii)); end
end
end
...
% Make ROI overlay clickable also inside the large FC-GA window.
try
    if isempty(ax) || ~ishghandle(ax), return; end
    fig = ancestor(ax,'figure');
    set(ax,'ButtonDownFcn',@(src,evt)fcGAOverlayClickAnyMode_20260624(src,evt,S,R));
    kids = findall(ax);
    for ii = 1:numel(kids)
        try, set(kids(ii),'HitTest','on','ButtonDownFcn',@(src,evt)fcGAOverlayClickAnyMode_20260624(src,evt,S,R)); catch, end
    end
    try, set(fig,'WindowButtonDownFcn',[]); catch, end
catch
end
end
...
try
    ...
    L = fcGAGetOverlayLabelMap_20260624(R);
    if isempty(L), return; end
    z = fcGACurrentSliceIndex_20260624(S,L);
    if ndims(L) == 3, L2 = L(:,:,z); else, L2 = L; end
    if y < 1 || x < 1 || y > size(L2,1) || x > size(L2,2), return; end
    lab = double(L2(y,x));
    if lab == 0 || isnan(lab), return; end
...
function L = fcGAGetOverlayLabelMap_20260624(R)
L = [];
