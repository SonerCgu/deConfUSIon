...
switch mode
    case 'ppt', out = GA_exportGroupMeanSCMPPT_20260511b(fig); % GA_PPT_VISUAL_FIX_20260511B
...
%%% GA_PPT_STYLEFIX_20260511_START
function ga_write_ppt_ml(pptFile,summaryPng,slideSpecs,cbPng,blackPng)
% Picture-only mlreportgen PPT writer. No TextBox constructor is used.
% Creates slide backgrounds with title/labels/footer/colorbar, then inserts each brain PNG separately.
import mlreportgen.ppt.*

if nargin < 5, blackPng = ''; end %#ok<NASGU>
...
if exist(pptFile,'file') == 2
    try
        ...
        error('Could not overwrite existing PPT file. Close it first: %s', pptFile);
    end
end

tmpDir = fullfile(pptDir, ['_GA_PPT_stylefix_tmp_' datestr(now,'yyyymmdd_HHMMSS')]);
if exist(tmpDir,'dir') ~= 7, mkdir(tmpDir); end
cleanupObj = onCleanup(@()ga_sf_cleanup(tmpDir)); %#ok<NASGU>
...
try
    ...
    % ---------------------------------------------------------
    % First slide: existing summary/table PNG
    % ---------------------------------------------------------
    if ~isempty(summaryPng) && exist(summaryPng,'file') == 2
        ...
        ga_sf_add_full_picture(slide, summaryPng, slideW, slideH);
    end

    % ---------------------------------------------------------
    % Time-window slides
    % ---------------------------------------------------------
    nSpecs = ga_sf_num_specs(slideSpecs);
    for si = 1:nSpecs
        spec = ga_sf_get_spec(slideSpecs, si);

        % If a spec is only a PNG path, add it full-slide.
        if ischar(spec)
            if exist(spec,'file') == 2
                try, slide = add(ppt,'Blank'); catch, slide = add(ppt); end
                ga_sf_add_full_picture(slide, spec, slideW, slideH);
            end
            continue;
        end

        [tileFiles,tileLabels] = ga_sf_tiles_and_labels(spec);
        slideTitle = ga_sf_getchar(spec,{'title','titleStr','slideTitle','name'},sprintf('Group mean SCM | slide %d',si));
        footerStr  = ga_sf_get_footer(spec);

        if isempty(tileFiles)
            slidePng = ga_sf_getchar(spec,{'slidePng','slidePNG','backgroundPng','montagePng'},'');
            if ~isempty(slidePng) && exist(slidePng,'file') == 2
                try, slide = add(ppt,'Blank'); catch, slide = add(ppt); end
                ga_sf_add_full_picture(slide, slidePng, slideW, slideH);
            end
            continue;
        end

        bgFile = fullfile(tmpDir,sprintf('GA_slide_bg_%03d.png',si));
        ga_sf_render_background(bgFile,slideTitle,footerStr,tileLabels,cbPng,slideW,slideH);
        ...
        ga_sf_add_full_picture(slide,bgFile,slideW,slideH);

        pos = ga_sf_tile_positions(min(6,numel(tileFiles)));
        for k = 1:min(6,numel(tileFiles))
            if exist(tileFiles{k},'file') ~= 2
                continue;
            end
            ga_sf_add_picture_at(slide,tileFiles{k},pos(k).x,pos(k).y,pos(k).w,pos(k).h);
        end
    end
...
pause(0.3);
if exist(pptFile,'file') ~= 2
    error('PowerPoint file was not created: %s', pptFile);
end
end

function ga_sf_add_full_picture(slide,imgFile,slideW,slideH)
import mlreportgen.ppt.*
pic = Picture(imgFile);
pic.X = '0in';
pic.Y = '0in';
pic.Width  = sprintf('%.3fin',slideW);
pic.Height = sprintf('%.3fin',slideH);
add(slide,pic);
end

function ga_sf_add_picture_at(slide,imgFile,x,y,w,h)
import mlreportgen.ppt.*
pic = Picture(imgFile);
pic.X = sprintf('%.3fin',x);
pic.Y = sprintf('%.3fin',y);
pic.Width  = sprintf('%.3fin',w);
pic.Height = sprintf('%.3fin',h);
add(slide,pic);
end

function n = ga_sf_num_specs(S)
if isempty(S)
    n = 0;
elseif iscell(S)
    n = numel(S);
...
function spec = ga_sf_get_spec(S,i)
if iscell(S)
    spec = S{i};
elseif isstruct(S)
    spec = S(i);
...
function [files,labels] = ga_sf_tiles_and_labels(spec)
files = {};
labels = {};
if isempty(spec) || ~isstruct(spec), return; end

% Struct-array tiles: tiles(k).file / tiles(k).label
if isfield(spec,'tiles') && isstruct(spec.tiles)
    T = spec.tiles;
    for k = 1:numel(T)
        f = ga_sf_first_field_char(T(k),{'file','png','path','filename','img'});
        if ~isempty(f)
            files{end+1} = f; %#ok<AGROW>
            lab = ga_sf_first_field_char(T(k),{'label','lbl','title','text'});
...
if isempty(files)
    files = ga_sf_field_to_cell(spec,{'tilePngs','tilePNGs','tileFiles','pngList','pngs','brainPngs','brainPNGs','files','imageFiles'});
end
if isempty(labels)
    labels = ga_sf_field_to_cell(spec,{'tileLabels','labels','lblList','labelList','windowLabels'});
end

% Fallback labels from filenames
for k = 1:numel(files)
    if k > numel(labels) || isempty(labels{k})
        labels{k} = ga_sf_label_from_file(files{k},k);
    end
end
end

function c = ga_sf_field_to_cell(S,names)
c = {};
for ii = 1:numel(names)
    ...
    if isfield(S,nm) && ~isempty(S.(nm))
        ...
        if iscell(v)
            c = v(:)'; return;
        elseif ischar(v)
            c = cellstr(v); c = c(:)'; return;
        elseif isstruct(v)
            tmp = {};
            for kk = 1:numel(v)
                f = ga_sf_first_field_char(v(kk),{'file','png','path','filename','label','lbl','title'});
                if ~isempty(f), tmp{end+1} = f; end %#ok<AGROW>
            end
            c = tmp; return;
        end
    end
...
function s = ga_sf_first_field_char(S,names)
s = '';
...
function s = ga_sf_getchar(S,names,fb)
s = fb;
if isempty(S) || ~isstruct(S), return; end
for ii = 1:numel(names)
    nm = names{ii};
...
function footer = ga_sf_get_footer(S)
footer = ga_sf_getchar(S,{'footer','footerStr','info','infoStr','settingsText','metaText'},'');
if ~isempty(footer), return; end

parts = {};
try
    if isfield(S,'TR'), parts{end+1} = sprintf('TR=%gs',double(S.TR(1))); end
catch, end
try
    if isfield(S,'winLen'), parts{end+1} = sprintf('window=%gs',double(S.winLen(1))); end
catch, end
try
    if isfield(S,'threshold'), parts{end+1} = sprintf('threshold=%g',double(S.threshold(1))); end
catch, end
try
    if isfield(S,'caxis') && numel(S.caxis) >= 2
        parts{end+1} = sprintf('caxis=[%g %g]',double(S.caxis(1)),double(S.caxis(2)));
    end
catch, end
try
    if isfield(S,'modMin') && isfield(S,'modMax')
        parts{end+1} = sprintf('alpha mod=[%g %g]',double(S.modMin(1)),double(S.modMax(1)));
    end
catch, end
try
    if isfield(S,'sigma'), parts{end+1} = sprintf('spatial smoothing sigma=%g',double(S.sigma(1))); end
catch, end

if isempty(parts)
    footer = '';
else
    footer = strjoin(parts,' | ');
end
end

function label = ga_sf_label_from_file(f,k)
label = sprintf('Window %d',k);
try
    [~,nm,~] = fileparts(f);
    tok = regexp(nm,'(\d+)[_\-]*(\d+)s','tokens','once');
    if ~isempty(tok)
        label = sprintf('%s-%ss',tok{1},tok{2});
        return;
    end
    tok = regexp(nm,'(\d+)-(\d+)s','tokens','once');
    if ~isempty(tok)
        label = sprintf('%s-%ss',tok{1},tok{2});
        return;
    end
    tok = regexp(nm,'w0*(\d+)','tokens','once');
    if ~isempty(tok)
        label = sprintf('Window %s',tok{1});
        return;
    end
catch
end
...
function pos = ga_sf_tile_positions(n)
if nargin < 1, n = 6; end
pos = struct('x',{},'y',{},'w',{},'h',{},'labelY',{});

% Inches on 13.333 x 7.5 slide.
% Left space is reserved for a wider, readable colorbar.
x0 = 1.15;
xRight = 12.95;
colGap = 0.25;
w = (xRight - x0 - 2*colGap) / 3;
h = 2.38;
yTop = 1.22;
yBot = 4.20;
...
function ga_sf_render_background(outFile,titleStr,footerStr,labels,cbPng,slideW,slideH)
figS = figure('Visible','off','Color',[0 0 0],'InvertHardcopy','off', ...
    'MenuBar','none','ToolBar','none','NumberTitle','off');
set(figS,'Units','pixels','Position',[80 80 1920 1080]);
set(figS,'PaperPositionMode','auto');

% Title
ga_sf_anno(figS,[0.02 0.925 0.96 0.055],titleStr,16,'bold','center');

% Footer / settings info
if ~isempty(footerStr)
    footerStr = ga_sf_short(footerStr,220);
    ga_sf_anno(figS,[0.18 0.015 0.80 0.060],footerStr,10,'bold','right');
end

% Wider colorbar area on left
try
    if ~isempty(cbPng) && exist(cbPng,'file') == 2
        Icb = imread(cbPng);
    else
        Icb = repmat(uint8(linspace(255,0,512)'),1,45,3);
    end
    axCB = axes('Parent',figS,'Position',[0.025 0.175 0.045 0.690]);
    image(axCB,Icb);
    set(axCB,'XTick',[],'YTick',[],'Box','off');
    axis(axCB,'off');
    ga_sf_anno(figS,[0.010 0.865 0.080 0.035],'PSC (%)',10,'bold','center');
catch
end

% Labels above brain images
pos = ga_sf_tile_positions(min(6,numel(labels)));
for k = 1:min(6,numel(labels))
    lab = ga_sf_short(labels{k},65);
    if isempty(lab), lab = sprintf('Window %d',k); end
    box = ga_sf_normbox(pos(k).x,pos(k).labelY,pos(k).w,0.25,slideW,slideH);
    ga_sf_anno(figS,box,lab,12,'bold','center');
end

print(figS,outFile,'-dpng','-r180','-opengl');
close(figS);
end

function box = ga_sf_normbox(x,y,w,h,slideW,slideH)
% x/y/w/h are inches with y measured from top, as in PPT.
box = [x/slideW, 1 - (y+h)/slideH, w/slideW, h/slideH];
end

function ga_sf_anno(figS,box,str,fs,weight,halign)
if nargin < 6, halign = 'left'; end
if nargin < 5, weight = 'normal'; end
try
    annotation(figS,'textbox',box, ...
        'String',str, ...
        'Color',[1 1 1], ...
        'EdgeColor','none', ...
...
function ga_sf_cleanup(tmpDir)
try
    if ~isempty(tmpDir) && exist(tmpDir,'dir') == 7
        rmdir(tmpDir,'s');
    end
catch
end
end
%%% GA_PPT_STYLEFIX_20260511_END
