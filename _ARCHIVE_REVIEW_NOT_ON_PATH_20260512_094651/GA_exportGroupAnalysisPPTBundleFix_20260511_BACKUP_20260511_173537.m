...
function out = localExportSCM_V3_FULLTIME(fig)
% Fixed full-time SCM export for GroupAnalysis.
% Preserves the full PSC time series and repairs bad TR/time-unit bugs.
out = []; 
ga_export_status_V3(fig,'Export Data SCM: collecting full PSC/time-series bundles ...');

[paths,rows] = ga_collect_group_bundle_paths_V3(fig);
if isempty(paths)
    error(['No usable SCM/PSC bundle paths were found in GroupAnalysis. ' ...
           'Please make sure the GroupAnalysis table contains the original SCM bundle paths or MAT files with full PSC.']);
end

B = {}; names = {}; trs = []; baselines = {}; pars = {}; bgs = {};
for i = 1:numel(paths)
    try
        b = ga_load_one_scm_bundle_V3(paths{i});
        if isempty(b) || ~isfield(b,'PSC') || isempty(b.PSC)
            continue;
        end
        B{end+1} = b.PSC; %#ok<AGROW>
        names{end+1} = b.name; %#ok<AGROW>
        trs(end+1) = b.TRsec; %#ok<AGROW>
        baselines{end+1} = b.baseline; %#ok<AGROW>
        pars{end+1} = b.par; %#ok<AGROW>
        bgs{end+1} = b.bg; %#ok<AGROW>
    catch ME
...
if isempty(B)
    error('No full 3D/4D PSC time-series data found. Static 2D maps are ignored.');
end

% Use first bundle shape as reference.
refSize = size(B{1});
keep = true(1,numel(B));
for i = 1:numel(B)
    if ~isequal(size(B{i}), refSize)
        keep(i) = false;
        fprintf('[GroupAnalysis export] Skipped animal with mismatching PSC size: %s\n', names{i});
    end
end
B = B(keep); names = names(keep); trs = trs(keep); baselines = baselines(keep); pars = pars(keep); bgs = bgs(keep);

if isempty(B)
    error('All PSC arrays had mismatching sizes; cannot make group SCM bundle.');
end

% Average animals while preserving full time axis.
catDim = ndims(B{1}) + 1;
stack = cat(catDim,B{:});
PSC = ga_nanmean_V3(stack,catDim);

% Robust TR selection and repair.
nT = ga_time_length_V3(PSC);
TR = ga_repair_TR_seconds_V3(trs,nT);
totalMin = (max(1,nT)-1) * TR / 60;
...
if isempty(bg)
    bg = ga_make_bg_from_psc_V3(PSC);
end

% Metadata.
par = struct();
if ~isempty(pars) && isstruct(pars{1})
    par = pars{1};
end
par.isGroupAnalysisExport = true;
par.groupAnalysisExportType = 'Full group mean PSC bundle';
par.sourceAnimalNames = names;
par.nAnimals = numel(names);
par.TRsec = TR;
par.totalMinutes = totalMin;
par.fullTimeSeriesExport = true;
...
exportInfo = struct();
exportInfo.created = datestr(now);
exportInfo.sourceFiles = paths(:);
exportInfo.usedFiles = names(:);
exportInfo.nAnimals = numel(names);
exportInfo.nTimePoints = nT;
exportInfo.TRsec = TR;
exportInfo.totalMinutes = totalMin;
exportInfo.note = 'Full group mean PSC time series. TR repaired/preserved for SCM_gui.';

outDir = ga_export_dir_V3(fig);
if ~exist(outDir,'dir'), mkdir(outDir); end
outFile = fullfile(outDir, ['GroupAnalysis_FULL_SCM_GroupExport_' datestr(now,'yyyymmdd_HHMMSS') '.mat']);

save(outFile,'PSC','bg','TR','par','baseline','nVolsOrig','exportInfo','-v7.3');

ga_export_status_V3(fig, sprintf('Export Data SCM complete: %d animals, %d time points, TR %.4g sec, %.2f min', numel(names), nT, TR, totalMin));
fprintf('\n[GroupAnalysis export] Saved full SCM bundle:\n%s\n', outFile);
fprintf('[GroupAnalysis export] Time points: %d | TR: %.6g sec | Total: %.3f min\n', nT, TR, totalMin);
out = outFile;
end

function [paths,rows] = ga_collect_group_bundle_paths_V3(fig)
paths = {}; rows = [];

% Search uitables first.
tabs = findall(fig,'Type','uitable');
for t = 1:numel(tabs)
    try
        D = get(tabs(t),'Data');
        [p,r] = ga_paths_from_anything_V3(D);
...
% Search appdata/UserData too.
try
    S = getappdata(fig);
    [p,r] = ga_paths_from_anything_V3(S);
    paths = [paths p]; rows = [rows r];
catch
end
try
    U = get(fig,'UserData');
    [p,r] = ga_paths_from_anything_V3(U);
    paths = [paths p]; rows = [rows r];
catch
end

% Keep only existing MAT files, prefer SCM/group bundles but do not require that string.
clean = {}; cleanRows = [];
for i = 1:numel(paths)
    p = paths{i};
    if ischar(p) && exist(p,'file') && ~isempty(regexpi(p,'\.mat$'))
        clean{end+1} = p; %#ok<AGROW>
        if numel(rows) >= i, cleanRows(end+1) = rows(i); else, cleanRows(end+1) = i; end %#ok<AGROW>
    end
end
...
function [paths,rows] = ga_paths_from_anything_V3(x)
paths = {}; rows = [];
...
if iscell(x)
    for i = 1:numel(x)
        [p,~] = ga_paths_from_anything_V3(x{i});
        if ~isempty(p)
            paths = [paths p]; %#ok<AGROW>
            rows = [rows repmat(i,1,numel(p))]; %#ok<AGROW>
        end
    end
    return;
end
if isstruct(x)
    ...
    for k = 1:numel(x)
        for f = 1:numel(fn)
            try
                [p,~] = ga_paths_from_anything_V3(x(k).(fn{f}));
                if ~isempty(p)
                    paths = [paths p]; %#ok<AGROW>
                    rows = [rows repmat(k,1,numel(p))]; %#ok<AGROW>
                end
            catch
            end
        end
...
function b = ga_load_one_scm_bundle_V3(pathFile)
S = load(pathFile);
b = struct();
b.name = ga_clean_name_V3(pathFile);
b.PSC = ga_find_psc_array_V3(S);
if isempty(b.PSC)
    b = [];
    return;
end
nT = ga_time_length_V3(b.PSC);
b.TRsec = ga_find_TR_seconds_V3(S,nT);
b.bg = ga_find_bg_V3(S,b.PSC);
b.par = ga_find_struct_field_V3(S,{'par','params','opts','meta'});
b.baseline = ga_find_any_field_V3(S,{'baseline','baseWindow','baselineSec','baselineWindow'});
end

function A = ga_find_psc_array_V3(S)
A = [];
cands = ga_collect_numeric_arrays_V3(S,'');
if isempty(cands), return; end
bestScore = -inf; bestIdx = 0;
for i = 1:numel(cands)
    X = cands(i).data;
    sz = size(X);
    if ndims(X) < 3, continue; end
    nT = ga_time_length_V3(X);
    if nT < 10, continue; end
    score = nT;
    nm = lower(cands(i).name);
    if ~isempty(strfind(nm,'psc')), score = score + 100000; end
    if ~isempty(strfind(nm,'signal_change')), score = score + 50000; end
    if ~isempty(strfind(nm,'group')), score = score + 1000; end
    if numel(sz) >= 3, score = score + 10*numel(X)/max(1,nT); end
    if score > bestScore
        bestScore = score;
        bestIdx = i;
    end
...
if bestIdx > 0
    A = cands(bestIdx).data;
    A = double(A);
end
end

function cands = ga_collect_numeric_arrays_V3(x,prefix)
cands = struct('name',{},'data',{});
...
if isstruct(x)
    ...
    for k = 1:numel(x)
        for f = 1:numel(fn)
            try
                nm = fn{f};
                if isempty(prefix), p2 = nm; else, p2 = [prefix '.' nm]; end
                cc = ga_collect_numeric_arrays_V3(x(k).(nm),p2);
                cands = [cands cc]; %#ok<AGROW>
            catch
            end
        end
    end
...
function nT = ga_time_length_V3(A)
sz = size(A);
if ndims(A) >= 4 && sz(4) > 10
    nT = sz(4);
elseif ndims(A) >= 3 && sz(3) > 10
    nT = sz(3);
else
    nT = max(sz);
end
...
function TRsec = ga_find_TR_seconds_V3(S,nT)
TRsec = [];

% Prefer explicit TR fields, not plotted axes.
keys = {'TR','tr','TRsec','TR_sec','TRSeconds','framePeriod','dt','samplingInterval'};
vals = ga_find_numeric_scalars_by_name_V3(S,keys);
if ~isempty(vals)
    TRsec = ga_repair_TR_seconds_V3(vals,nT);
    return;
end

% Time vectors as fallback.
tvec = ga_find_time_vector_V3(S,nT);
if ~isempty(tvec) && numel(tvec) > 2
    d = median(diff(double(tvec(:))));
    if max(tvec) <= 120 && d < 5
        TRsec = d * 60;   % minutes vector
    else
        TRsec = d;        % seconds vector
    end
    TRsec = ga_repair_TR_seconds_V3(TRsec,nT);
    return;
end

% Conservative HUMoR step-motor fallback.
TRsec = ga_repair_TR_seconds_V3([],nT);
end

function TRsec = ga_repair_TR_seconds_V3(vals,nT)
vals = vals(isfinite(vals) & vals > 0);
if isempty(vals)
    TRsec = 18;  % HUMoR/step-motor safe fallback: 150 vols ~= 45 min
else
    TRsec = median(vals);
end

% If TR is probably stored in minutes, convert to seconds.
if TRsec < 1
    totalMinIfSec = ((max(1,nT)-1) * TRsec) / 60;
    totalMinIfMin = (max(1,nT)-1) * TRsec;
    if totalMinIfSec < 5 && totalMinIfMin > 5
        TRsec = TRsec * 60;
    end
end

% Repair absurd tiny exports such as 0.0475 min total for 150 volumes.
totalMin = ((max(1,nT)-1) * TRsec) / 60;
if nT >= 100 && nT <= 400 && totalMin < 10
    fprintf('[GroupAnalysis export] WARNING: detected impossible short SCM duration %.4f min for %d time points. Repairing TR to 18 sec.\n', totalMin, nT);
    TRsec = 18;
end
end

function vals = ga_find_numeric_scalars_by_name_V3(x,keys)
vals = [];
if isstruct(x)
    fn = fieldnames(x);
    for k = 1:numel(x)
        for f = 1:numel(fn)
            ...
            try
                v = x(k).(nm);
                if any(strcmpi(nm,keys)) && isnumeric(v) && isscalar(v)
                    vals(end+1) = double(v); %#ok<AGROW>
                elseif isstruct(v)
                    vals = [vals ga_find_numeric_scalars_by_name_V3(v,keys)]; %#ok<AGROW>
                end
            catch
            end
        end
...
function tvec = ga_find_time_vector_V3(x,nT)
tvec = [];
if isstruct(x)
    fn = fieldnames(x);
    for k = 1:numel(x)
        for f = 1:numel(fn)
            ...
            try
                v = x(k).(fn{f});
                if isnumeric(v) && isvector(v) && numel(v) == nT
                    if ~isempty(strfind(nm,'time')) || strcmp(nm,'t') || ~isempty(strfind(nm,'minutes'))
                        tvec = double(v(:)); return;
                    end
                elseif isstruct(v)
                    tvec = ga_find_time_vector_V3(v,nT);
...
function bg = ga_find_bg_V3(S,PSC)
bg = ga_find_any_field_V3(S,{'bg','underlay','background','meanImage','anat','anatomy'});
if isnumeric(bg) && ndims(bg) >= 2
    bg = double(bg);
else
    bg = ga_make_bg_from_psc_V3(PSC);
end
...
function bg = ga_make_bg_from_psc_V3(PSC)
try
    td = ga_time_dim_index_V3(PSC);
    bg = mean(PSC,td);
    bg = squeeze(bg);
catch
    sz = size(PSC);
...
function td = ga_time_dim_index_V3(A)
sz = size(A);
if ndims(A) >= 4 && sz(4) > 10
    td = 4;
elseif ndims(A) >= 3 && sz(3) > 10
...
function v = ga_find_any_field_V3(S,names)
v = [];
...
for k = 1:numel(S)
    for f = 1:numel(fn)
        nm = fn{f};
        try
            if any(strcmpi(nm,names))
                v = S(k).(nm); return;
            elseif isstruct(S(k).(nm))
                v = ga_find_any_field_V3(S(k).(nm),names);
                if ~isempty(v), return; end
            end
        catch
        end
...
function v = ga_find_struct_field_V3(S,names)
v = ga_find_any_field_V3(S,names);
if ~isstruct(v), v = struct(); end
end

function M = ga_nanmean_V3(X,dim)
mask = isfinite(X);
X(~mask) = 0;
n = sum(mask,dim);
n(n==0) = NaN;
...
function outDir = ga_export_dir_V3(fig)
outDir = pwd;
try
    S = getappdata(fig);
    cand = ga_find_any_field_V3(S,{'saveRoot','exportPath','outDir','outputDir'});
    if ischar(cand) && exist(cand,'dir'), outDir = cand; end
catch
end
try
    outDir2 = fullfile(outDir,'GroupAnalysis_Exports');
...
function name = ga_clean_name_V3(p)
[~,name,~] = fileparts(p);
name = regexprep(name,'[^A-Za-z0-9_\\-]','_');
end

function ga_export_status_V3(fig,msg)
fprintf('[GroupAnalysis export] %s\n',msg);
try
    setappdata(fig,'GA_lastExportStatus',msg);
catch
end
try
    h = findall(fig,'Style','text');
    for i = 1:numel(h)
        s = get(h(i),'String');
        if ischar(s) && ~isempty(strfind(lower(s),'group map analysis'))
            set(h(i),'String',msg);
            drawnow;
            return;
        end
