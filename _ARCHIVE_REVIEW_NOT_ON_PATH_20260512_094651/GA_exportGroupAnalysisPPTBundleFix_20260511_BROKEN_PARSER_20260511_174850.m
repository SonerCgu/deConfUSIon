...
% Fast + locked PPT export.
% Important: ask the small settings popup before expensive bundle collection.
% Also prevents GroupAnalysis double-callback from opening the popup twice.
pptPath = '';
busyKey = 'GA_EXPORT_PPT_BUSY_20260511';

try
    if isappdata(fig,busyKey) && isequal(getappdata(fig,busyKey),true)
        fprintf('[GroupAnalysis export] PPT export already running. Ignoring duplicate callback.\n');
        return;
    end
...
try
    % Ask first. This avoids waiting for slow table/bundle search before the popup appears.
    cfg = askPptCfgFast();
    if isempty(cfg)
        try, setappdata(fig,busyKey,false); catch, end
        return;
    end
    ...
    [G,rep] = makeGroupMeanG(B);
    fprintf('\n[GroupAnalysis export] Rendering FULL group mean PSC series: function B = collectBundles(fig)
...
function cfg = askPptCfgFast()
% Fast popup shown before slow bundle collection.
% Defaults are intentionally simple and SCM-like.
a = inputdlg({ ...
