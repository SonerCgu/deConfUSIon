...
ppt=[];
try
    ppt=Presentation(pptFile); open(ppt); slideW=13.333; slideH=7.5;
    slide=add(ppt,'Blank'); ga_ppt_pic(slide,summaryPng,0,0,slideW,slideH);
    for si=1:numel(slideSpecs)
        spec=slideSpecs{si}; slide=add(ppt,'Blank');
        ga_ppt_pic(slide,blackPng,0,0,slideW,slideH);
        ga_ppt_text(slide,spec.title,0.15,0.10,13.0,0.35,15,true,'center');
        ga_ppt_text(slide,spec.footer,3.6,7.10,9.5,0.25,8,true,'right');
        ga_ppt_pic(slide,cbPng,0.12,1.12,0.60,5.75);
        x0=1.15; xGap=0.22; yTop=0.92; yBot=4.02; tileW=3.85; tileH=2.58; labelH=0.25;
        for k=1:min(6,numel(spec.tilePNGs))
            if k<=3, cc=k-1; y=yTop; else, cc=k-4; y=yBot; end
            x=x0+cc*(tileW+xGap);
            ga_ppt_text(slide,spec.tileLbls{k},x,y,tileW,labelH,11,true,'center');
            ga_ppt_pic(slide,spec.tilePNGs{k},x,y+labelH,tileW,tileH);
        end
    end
    close(ppt);
catch ME
